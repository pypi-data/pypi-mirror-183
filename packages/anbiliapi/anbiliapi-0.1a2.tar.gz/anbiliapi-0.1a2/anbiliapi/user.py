...
class User(Bilibili):
    ...
    async def user_concern(self, fid: int, is_concern: bool = True):
        """
        改变关注状态
        :param fid:用户id
    ...
    async def user_block(self, fid: int, is_block: bool = True):
        """
        改变黑名单状态
        :param fid:
        :param is_block:
    ...
    async def get_card_info(self, mid: str = None, photo: bool = False):
        mid = self._mid if mid is None else mid
        kwargs = {
            "url": "https://api.bilibili.com/x/web-interface/card",
            "method": "GET",
            "data": {"mid": mid, "photo": photo},
        }
        res = await self._requests(**kwargs)
        return res.json()
