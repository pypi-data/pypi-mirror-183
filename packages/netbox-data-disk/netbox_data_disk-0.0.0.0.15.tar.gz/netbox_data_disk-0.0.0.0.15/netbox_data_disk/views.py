...
from netbox_data_disk.forms import BackupForm
from netbox_data_disk.git import GitBackup
from netbox_data_disk.models import Backup, BackupJob, BackupCommitTreeChange, BackupCommit, BackupObject
from netbox_data_disk.tables import BackupTable
from netbox_data_disk.utils import get_backup_tables, Differ
...
class BackupView(ObjectView):
    queryset = Backup.objects.all()
    template_name = 'netbox_data_disk/device.html'
...
class ConfigView(View):
    template_name = 'netbox_data_disk/config.html'
    ...
    def get(self, request, pk, current):
        ...
        return render(request, 'netbox_data_disk/config.html', {
            'object': backup,
...
class DiffView(View):
    template_name = 'netbox_data_disk/diff.html'
    ...
    def get(self, request, pk, current, previous=None):
        ...
        return render(request, 'netbox_data_disk/diff.html', {
            'object': backup,
            'diff': diff,
            'current': current,
            'previous': previous,
