...
function initGenerateKeyPair() {
  ...
  function handleOpen() {
    ...
    apiGetBase<APIKeyPair>('/api/plugins/netbox_data_disk/generate-rsa-key-pair').then(data => {
      if (!hasError(data)) {
...
function initLockUnlock() {
  ...
  function unlock(id: string | null) {
    ...
    if (typeof id === 'string' && id !== '') {
      apiGetBase<APISecret>(`/api/plugins/netbox_data_disk/secrets/${id}`).then(data => {
        if (!hasError(data)) {
...
function requestSessionKey(privateKey: string) {
  apiPostForm('/api/plugins/netbox_data_disk/get-session-key/', {
