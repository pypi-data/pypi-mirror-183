...
from netbox_data_disk.exceptions import InvalidKey
from netbox_data_disk.hashers import SecretValidationHasher
from netbox_data_disk.querysets import UserKeyQuerySet
from netbox_data_disk.utils import encrypt_master_key, decrypt_master_key, generate_random_key
...
class UserKey(models.Model):
    ...
    def clean(self):
        ...
        if self.public_key:
            ...
            if pubkey_length < settings.PLUGINS_CONFIG['netbox_data_disk']['public_key_size']:
                raise ValidationError({
                    'public_key': "Insufficient key length. Keys must be at least {} bits long.".format(
                        settings.PLUGINS_CONFIG['netbox_data_disk']['public_key_size']
                    )
                })
...
class SecretRole(OrganizationalModel):
    ...
    def get_absolute_url(self):
        return reverse('plugins:netbox_data_disk:secretrole', args=[self.pk])
...
class Secret(NetBoxModel):
    ...
    def get_absolute_url(self):
        return reverse('plugins:netbox_data_disk:secret', args=[self.pk])
