...
from netbox_data_disk.models import Backup
...
class ActionButtonsColumn(tables.TemplateColumn):
    ...
    template_code = """
    <a href="{% url 'plugins:netbox_data_disk:backup_config' pk=record.pk current=record.current.pk %}" class="btn btn-sm btn-outline-dark" title="View">
        <i class="mdi mdi-cloud-download"></i>
    </a>
    {% if record.previous %}
        <a href="{% url 'plugins:netbox_data_disk:backup_diff' pk=record.pk current=record.current.pk previous=record.previous.pk %}" class="btn btn-outline-dark btn-sm" title="Diff">
