...
from netbox_data_disk.forms import UserKeyForm, SecretRoleFilterForm
...
class SecretEditView(ObjectEditView):
    queryset = Secret.objects.all()
    form = SecretForm
    template_name = 'netbox_data_disk/secret_edit.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            return redirect('plugins:netbox_data_disk:userkey')
        if not uk.is_active():
...
class SecretBulkImportView(BulkImportView):
    ...
    table = SecretTable
    template_name = 'netbox_data_disk/secret_import.html'
...
class UserKeyView(LoginRequiredMixin, View):
    template_name = 'netbox_data_disk/userkey.html'
...
class UserKeyEditView(LoginRequiredMixin, View):
    ...
    template_name = 'netbox_data_disk/userkey_edit.html'
...
class SessionKeyDeleteView(ObjectDeleteView):
    ...
    def get(self, request):
        ...
        return render(request, 'netbox_data_disk/sessionkey_delete.html', {
            'object': sessionkey,
            'obj_type': sessionkey._meta.verbose_name,
            'form': form,
            'return_url': reverse('plugins:netbox_data_disk:userkey'),
        })
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            # Delete cookie
            response = redirect('plugins:netbox_data_disk:userkey')
