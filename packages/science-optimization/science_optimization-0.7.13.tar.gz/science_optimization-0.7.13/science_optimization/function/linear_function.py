...
class LinearFunction(BaseFunction):
    ...
    def parameter_check(self, c: np.ndarray, d):
        ...
        if len(c.shape) != 2 or c.shape[1] != 1:
            raise ValueError("Invalid format for 'c' parameter")

        # checking d parameter
        try:
            int(d)
        except (ValueError, TypeError):
            raise ValueError("'d' parameter must be a valid number")
    ...
    def __init__(self, c, d=0):
       ...
       """
        ...
        # set parameters
        self.parameters = {'c': sparse.COO(c),
                           'd': d}
    ...
    def gradient(self, x):
        ...
        if len(dim) == 1:
            ...
            dfdx = np.matlib.repmat(c.todense(), 1, dim[1])
