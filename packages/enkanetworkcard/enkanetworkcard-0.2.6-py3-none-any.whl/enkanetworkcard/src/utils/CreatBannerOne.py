...
from .FunctionsPill import imagSize,imgD,centrText
from .options import *
...
def weaponAdd(weaponRes,characters,lvlName):
    ...
    image = imagSize(link = characters.detail.icon.url,size = (114,121))
...
def create_picture(rezFrame,person,imgs,adapt,splash = None):
    if imgs:
        ...
        if splash:
            banner = imagSize(link = splash,size = (2048,1024))
        else:
            banner = imagSize(link = person.images.banner.url,size = (2048,1024))
...
def talants(talatsRes,characters):
    ...
    for key in characters.skills:
        ...
        imagesIconTalants = imgD(link = key.icon.url).resize((50,50))
...
def creatDopStat(infpart,rezArtDop):
    ...
    for key in infpart:
        ...
        if str(key.type) == "DigitType.PERCENT":
            v = f"{v}%"
        imageStats= imagSize(image = imageStats,fixed_width = 17) 
...
def creatArtifact(artifacResSave,infpart,imageStats):
    ...
    artimg = imagSize(link = infpart.detail.icon.url,size = (175,175))
