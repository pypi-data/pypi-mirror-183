...
class PowerMLLearnTopics:
    def __init__(
            ...
            num_subsamples=20,
            sample_size=20,
            create_model_name="unblocked/create-topics/v2",
            filter_model_name="unblocked/filter-topics"):
    ...
    def get_topics(self):
        ...
        prompt = self.__prompt_format(topic_type)
        # prompt_append = f'What are the {topic_type} that these messages discuss?\n\n1.'
        num_samples = len(self.messages)
        sample_index = 0
        ...
        for i in tqdm(range(self.num_subsamples)):
            ...
            output = self.model.predict(
                prompt, max_tokens=256, temperature=0.7, model=self.create_model_name)
    ...
    def __add_examples(self, examples, prompt, max_total_tokens=4000, max_output_tokens=256):
        ...
        for i, example in enumerate(examples):
            ...
            if self.__count_tokens(str(prompt_dict)) < max_total_tokens - max_output_tokens:
                continue
            else:
                break
        return prompt_dict
    ...
    def __filter_top_topics(self, top_topics):
        prompt = {
            "{{topics_in_list}}": f"{top_topics}", "{{topic_type}}": "technical system-level topics"}
        output = self.model.predict(
