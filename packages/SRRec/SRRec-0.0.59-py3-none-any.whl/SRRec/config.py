'''
Author: Pengyao PING
Date: 2022-04-01 11:36:55
LastEditors: Pengyao PING
LastEditTime: 2022-10-25 22:11:53
    ...
    def __init__(self, config_file, logger):
        ...
        if config_file:
            ...
            if conf.has_option("SourceInputData", "input_file"):
                ...
                logger.error("No input file setting in config file.")            
            ...
            if conf.has_option("SourceInputData", "correct_data"):
                ...
                logger.error("No input correct_data setting in config file when evaluation.")  
            ...
            # when the number of negative samples larger than preseting threshold, SRRec will downsample negative samples for training
            ...
            if conf.has_option("EmbeddingSetup", "read_type"):
                self.read_type = conf.getboolean("EmbeddingSetup", "read_type")
            else:
                self.read_type = 'DNA'

            # AmbiguousSetup
            if conf.has_option("AmbiguousSetup", "ambiguous_error_node_degree"):
                self.ambiguous_error_node_degree = conf.getfint("AmbiguousSetup", "ambiguous_error_node_degree")
            else:
            ...
            if conf.has_option("Amplicon", "amplicon_threshold_proba"):
                self.amplicon_threshold_proba = conf.getint("Amplicon", "amplicon_threshold_proba")
            else:
            ...
            if conf.has_option("Simulation", "error_rate"):
                self.error_rate = conf.getint("Simulation", "error_rate")
            else:
