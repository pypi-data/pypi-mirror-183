...
from SRRec.config import Config
import sys, getopt
from SRRec.data_generation import DataGneration
from SRRec.error_orrection import ErrorCorrection
from SRRec.data_analysis import DataAnalysis
import os
from SRRec.simulation import Simulation
from SRRec.utils import custom_logger, usage
from SRRec.data_preprocessing import DataProcessing

def main():
    argv = sys.argv[1:]
    # create logger
    logger = custom_logger("SRRec", debug_mode=False)
    ##############################################################
    try:
        ...
        if opts:
            ...
            if h_lst:
                ...
                if m_lst:
                    ...
                    logger.error("Not select any module, using command SRRec -h/--help for usage.")
                    sys.exit()
                ...
                if module_arg == "correction":
                    ...
                    if c_lst:
                        ...
                        if i_lst:
                            ...
                            #     logger.error('Input file does not exsit.')
                        if t_lst:
...
############################################################################################################################
                    ...
                    if i_lst and t_lst:
                        ...
                        logger.error("Must input raw and true datasets simultaneously or configuration file.")
                        sys.exit()
...
############################################################################################################################
                    ...
                    if i_lst and t_lst and r_lst:
                        ...
                        logger.error("Must input raw, true and rectified datasets simultaneously or configuration file.")
                        sys.exit()
...
############################################################################################################################
                    ...
                    logger.error("Input wrong module name, using command SRRec -h/--help for usage.")
                    sys.exit()
        else:
            logger.error("No valid arguments input, using command SRRec -h/--help for usage")
