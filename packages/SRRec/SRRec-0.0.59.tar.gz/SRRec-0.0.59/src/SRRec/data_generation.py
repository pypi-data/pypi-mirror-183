    ...
    def __init__(self, logger, config):
        ...
        Args:
            ...
            config (class): parameters below setting using configparser
                ...
                config.save_graph (bool, optional): If true, SRRec will save the construted graph to file. Defaults to False. 
                config.graph_visualization (bool, optional): If true, SRRec will draw the connected subgraphs of the construted  graph. Defaults to False.
                config.drawing_graph_num (int, optional): The number of subgraphs will be drawing. Defaults to 50.
                config.high_ambiguous (bool, optional): If true, SRRec will predict high ambiguous errors. Defaults to False.
                config.min_iters (int, optional): Minimum progress display update interval in iterations for the module tqdm. Defaults to 100.
                config.verbose (bool, optional): If true, SRRec will save the genuine, ambiguous errors and negative reads to csv.
        """
        self.logger = logger
        self.config = config
        if os.path.exists(self.config.result_dir):
            self.logger.warning("Directory '% s' already exists, SRRec will use it." % self.config.result_dir)
        else:
    ...
    def draw_graph(self, graph, sub_dir, drawing_graph_num = 50):
        ...
        for s in S:
            if nx.number_of_nodes(s) >= 10:
                ...
                nx.draw(s, pos = graphviz_layout(s, prog='sfdp', args="-Nshape=point"), labels=labels)
                plt.savefig(sub_dir + str(i) + ".png", format="PNG")
                i += 1
