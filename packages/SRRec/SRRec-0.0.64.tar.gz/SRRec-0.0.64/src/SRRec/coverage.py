...
class SequenceCoverage():
    ...
    def __init__(self, logger, config):
        ...
        if os.path.exists(self.config.result_dir):
            self.logger.info("Directory '% s' already exists, SRRec will use it" % self.config.result_dir)
        else:
    ...
    def build_index(self, reference):
        ...
        if os.path.exists(ref_index_dir):
            self.logger.info("Directory '% s' already exists, SRRec will use it" % ref_index_dir)
        else:
