...
def combine(parser: Parser, delimiter: str = "") -> Parser:
    return transform(parser, lambda items: delimiter.join(utils.stringify(utils.filter(utils.flatten(items)))))
...
def filter(parser: Parser) -> Parser:
    return transform(parser, lambda items: [*utils.filter(items)])
