...
def map(parser: Parser, function: Callable[[Any], Any]) -> Parser:
    @Parser
    def map_parser(stream: str, pos: int) -> Result:
        result = parser(stream, pos)
        if result:
...
def combine(parser: Parser, delimiter: str = "") -> Parser:
    return map(parser, lambda items: delimiter.join(utils.stringify(utils.filter(utils.flatten(items)))))


def flatten(parser: Parser) -> Parser:
    return map(parser, lambda items: [*utils.flatten(items)])


def filter(parser: Parser) -> Parser:
    return map(parser, lambda items: [*utils.filter(items)])
