...
from montecarlodata.common.data import EventProperties, AWSArn, DcResourceProperties, ResourceProperties, BucketRow
...
class CloudResourceService:
    ...
    def __init__(self, config: Config, user_service: Optional[UserService] = None,
                 ...
                 aws_region_override: Optional[str] = None, request_wrapper: Optional[GqlWrapper] = None):
        ...
        self._user_service = user_service or UserService(config=config, request_wrapper=self._request_wrapper)
    ...
    def _initialize_events(self, event_type: str, collector_props: Box,
    ...
    def create_event_topic(self, dc_id: Optional[str] = None, auto_yes: Optional[bool] = False, **kwargs):
        collector = self.get_and_validate_active_collector(dc_id=dc_id)
        event_details = self._populate_event_details_from_saved_config(collector, **kwargs)
        self._create_or_update_event_topic(event_details=event_details, skip_prompts=auto_yes)

    def create_bucket_side_event_infrastructure(self, dc_id: Optional[str] = None, auto_yes: Optional[bool] = False, **kwargs):
        collector = self.get_and_validate_active_collector(dc_id=dc_id)
        event_details = self._populate_event_details_from_saved_config(collector, **kwargs)
        self._create_topic_subscription(event_details=event_details, skip_prompts=auto_yes)
        self._create_event_notification(event_details=event_details, skip_prompts=auto_yes)

    def update_event_queue(self, dc_id: Optional[str] = None, auto_yes: Optional[bool] = False, **kwargs):
        collector = self.get_and_validate_active_collector(dc_id=dc_id)
        event_details = self._populate_event_details_from_saved_config(collector, collection_client=True, **kwargs)
        self._update_event_q(event_details=event_details, skip_prompts=auto_yes)
    ...
    def _get_clients_for_buckets(self, buckets, cust_resources_profile):
        ...
        for bucket in buckets:
            ...
            if region not in region_to_client:
                region_to_client[region] = AwsClientWrapper(profile_name=cust_resources_profile, region_name=region)
        ...
        **kwargs
        ...
        if response.data:
            ...
            if not collection_client:
                bucket_clients = self._get_clients_for_buckets(config['resource_properties'], cust_resources_profile)
            config['resource_properties'] = [
                ResourceProperties(
                    bucket_name=bucket['bucket_name'],
                    client=bucket_clients[bucket['bucket_name']] if not collection_client else None,
        ...
        if collection_client:
            aws_client = AwsClientWrapper(profile_name=collector_profile, region_name=collector_arn.region)
            collector_props.outputs = aws_client.get_stack_outputs(stack_id=collector_props.stackArn)
            aws_options = dict(
                collection_account_id=aws_client.get_caller_identity(),
                collection_client=aws_client,
                event_queue_arn=config.get('event_queue_arn') or self._get_relevant_q(event_type, collector_props.outputs)
            )
        ...
        return EventProperties(**event_args,)
    ...
    def _create_or_update_event_topic(self, event_details: EventProperties, skip_prompts: Optional[bool] = False) -> None:
        ...
        for region, resources in region_to_resources.items():
            ...
            if topic_arn is None:
                ...
                prompt = f'Create topic \'{name}\' in \'{region}\' ' \
            ...
            region_bucket_topic_policy_id = self._MCD_TOPIC_POLICY_SID_PATTERN.format(event_details.event_type, resources[0].topic_name)

            existing_bucket_policy = self._get_preexisting_topic_policy(topic_policy_id=region_bucket_topic_policy_id, topic_attributes=topic_attributes)

            if not existing_bucket_policy:
                new_statements.append(self._generate_sns_bucket_policy(sid=region_bucket_topic_policy_id, topic_arn=topic_arn, bucket_arns=region_bucket_arns))
            else:
                update_statements[region_bucket_topic_policy_id] = self._extend_sns_bucket_policy(existing_policy=existing_bucket_policy, bucket_arns=region_bucket_arns)

            if not self._is_preexisting_topic_policy(topic_policy_id=q_topic_policy_id, topic_attributes=topic_attributes):
                new_statements.append(self._generate_sns_q_policy(sid=q_topic_policy_id, event_details=event_details, topic_arn=topic_arn))

            # Update policy if necessary

            new_statement_count = len(new_statements) + len(update_statements)
            if new_statement_count > 0:
                click.echo(f'Adding statements (x{new_statement_count}) to \'{resources[0].topic_name}\' -')
                ...
                resources[0].client.set_topic_attributes(
                    arn=topic_arn,
                    name='Policy',
                    value=self._extend_topic_policy(topic_attributes=topic_attributes, new_statements=new_statements, update_statements=update_statements)
                )
            else:
    ...
    def _update_event_q(self, event_details: EventProperties, skip_prompts: Optional[bool] = False) -> None:
        ...
        topic_arns = list(OrderedDict.fromkeys([resource.topic_arn for resource in event_details.resource_properties]).keys())

        if not attributes.get('Attributes', {}).get('Policy'):
            # No policy set (e.g. is a new q). Create
            new_policy = self._generate_q_policy(event_details=event_details, topic_arns=topic_arns)
            ...
            new_policy = self._extend_q_policy(existing_policy=existing_policy, event_details=event_details, topic_arns=topic_arns)
    ...
    def _create_topic_subscription(self, event_details: EventProperties, skip_prompts: Optional[bool] = False) -> None:
        ...
        for resource in event_details.resource_properties:
            ...
            subscriptions = list(resource.client.list_topic_subscriptions(arn=resource.topic_arn) or [])
            if subscriptions:
            ...
            if should_continue:
                continue
            click.echo(f'Creating subscription for \'{event_details.event_queue_name}\' on \'{resource.topic_name}\'')
            prompt_connection(message='Please confirm', skip_prompt=skip_prompts)
    ...
    @staticmethod
    def _extend_topic_policy(topic_attributes: Box, new_statements: List[Dict], update_statements: Dict) -> str:
        """
        ...
        for statement in policy.Statement:
            if statement.Sid in update_statements:
    ...
    def _extend_sns_bucket_policy(existing_policy: Box, bucket_arns: List[str]) -> Dict:
        ...
        source_arn = new_policy.get('Condition', {}).get('StringEquals', {}).get('aws:SourceArn', [])
        if source_arn:
    ...
    @staticmethod
    def _extend_q_policy(existing_policy: Box, event_details: EventProperties, topic_arns: Optional[Iterable[str]]) -> Dict:
        """
        Merge new statements with existing
        """
        new_policy = Box(copy.deepcopy(existing_policy))
    ...
    def _merge_event_notifications(self, setting_id: str, existing_config: Dict, resource_details: ResourceProperties) -> Dict:
        """
    ...
    @staticmethod
    def _generate_bucket_to_topic_config(setting_id: str, resource_details: ResourceProperties) -> Dict:
        return dict(
            Id=setting_id,
        ...
        return {
            ...
            'Statement': list(filter(None, [
                ...
                },
                {
                    ...
                    'Condition': {
                        'ArnLike': {
                            'aws:SourceArn': list(topic_arns)
                        }
                    }
                } if topic_arns else None
            ]))
        }
