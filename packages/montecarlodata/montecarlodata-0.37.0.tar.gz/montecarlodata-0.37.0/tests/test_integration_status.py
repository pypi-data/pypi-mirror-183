...
class OnboardingStatusTest(TestCase):
    ...
    def test_display_integrations_with_connections(self):
        ...
        self._user_service_mock.warehouses = [
            {'name': 'Sample 1', 'connections': [{'uuid': sample_uuid, 'type': sample_type1, 'connectionIdentifiers': [sample_connection_identifier1, sample_connection_identifier2], 'createdOn': sample_time}]}]
        self._user_service_mock.bi_containers = [
            {'name': 'Sample 2', 'connections': [{'uuid': sample_uuid, 'type': sample_type2, 'connectionIdentifiers': [sample_connection_identifier2], 'createdOn': sample_time}]}]
