...
class DbtImportServiceTests(TestCase):
    ...
    @patch.object(DbtImporter, 'import_run')
    def test_import_run_with_no_connection_id_and_multiple_warehouses(self, mock_import_run: Mock, mock_echo: Mock):
        # given
        connection_id = None
    ...
    def test_import_run_with_invalid_connection_id(self, mock_import_run: Mock, mock_echo: Mock):
        ...
        mock_echo.assert_called_once_with(
            f'Error - Could not find a connection with id: {connection_id}',
            err=True
        )
