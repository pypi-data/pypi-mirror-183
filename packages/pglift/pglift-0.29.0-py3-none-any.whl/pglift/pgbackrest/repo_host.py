...
class Server:
    ...
    def args(self) -> List[str]:
        return [
            ...
            "server",
            f"--config-path={self.settings.configpath}",
        ]
...
def base_config(settings: PgBackRestSettings) -> configparser.ConfigParser:
    """Build the base configuration for pgbackrest on the database host.
    ...
    cp["global"].update(
        {
            "repo1-host-type": "tls",
            "repo1-host": s.host,
            "repo1-host-port": str(s.host_port),
            "repo1-host-config": str(s.host_config),
