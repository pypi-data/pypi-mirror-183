    ...
    def render(self, clean=False) -> None:
        ...
        with Progress() as progress:
            ...
            for page in self.route_list.values():
                progress.update(task_add_route, description=f"[blue]Adding[grey]Route: [blue]{page.slug}")
            ...
            # Parse SubCollection
            task_render_subcollection = progress.add_task("[blue]Rendering SubCollections", total=len(self.subcollections))
            for tag, subcollection in self.subcollections.items():
                progress.update(task_render_subcollection, description=f"[blue]Rendering[grey]SubCollection: [blue]{tag}")
                page = Page()
                page.title = tag
