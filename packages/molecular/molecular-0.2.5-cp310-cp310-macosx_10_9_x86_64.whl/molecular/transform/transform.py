...
def fit(a, b):
    """
    Fit Trajectory `a` to Trajectory `b` in a pairwise fashion.
    ...
    a_xyz = (a.xyz - center(a)).to_numpy().reshape(*a.shape)
    b_xyz = (b.xyz - center(b)).to_numpy().reshape(*b.shape)

    # Transpose a
    a_xyz_transpose = np.transpose(a_xyz, axes=[0, 2, 1])

    # Compute covariance matrix
    covariance_matrix = np.matmul(a_xyz_transpose, b_xyz)

    # Get rotation matrix
    rotation_matrix = _get_rotation_matrix(covariance_matrix)

    # Perform optimal rotation
    a_xyz = np.matmul(a_xyz, rotation_matrix)

    # Move `a` to `b` center
    a_xyz = a_xyz + b.center()
...
def _xyz_numpy(a):
    ...
    """
    return a.xyz.to_numpy().reshape(*a.shape)