...
class DatosCROM(TablasVC):
    ...
    def __check_id(self, id):
        ...
        if not isinstance(id,tipos_fijos):
            raise TypeError(f'El parámetro "id" {id} es {type(id)}, pero se esperaba alguno de {tipos_fijos}')
        else:
            try:
                return int(id)
            except:
                raise Exception(f'No se pudo convertir el ID {id} a el tipo de datos int')
        
    def __check_datos_10s(self,nemo,equipo,fi,ff):
        ...
        if self.datos_seg.index.min() > fi:
            raise ValueError(f'Los datos diez segundales no contienen el período requerido para analizar la incidencia deseada ID = {id}\n' +
                             f'La fecha de inicio de los datos diez segundales es {self.datos_seg.index.min()} y la de la incidencia es {fi}')

        if self.datos_seg.index.max() > fi:
            raise ValueError(f'Los datos diez segundales no contienen el período requerido para analizar la incidencia deseada ID = {id}\n' +
                             f'La fecha de inicio de los datos diez segundales es {self.datos_seg.index.max()} y la de la incidencia es {ff}')
    
    def __estimar_ENS_una_incidencia(self,id,muestra=.33):
        
        if self.incidencias.query(f'ID == {id}').empty:
            raise ValueError(f'No se encontró la incidencia con ID {id}')

        if muestra <=0 or muestra >1:
            raise ValueError(f'El parámetro muestra debe ser un valor entre (0 , 1]')

        incidencia = self.incidencias.query(f'ID == {id}').iloc[0]
        ...
        self.__check_datos_10s(p,e,fi,ff)
    ...
    def estimar_ENS(self,ids,muestra=.33):
        
        tipos_iterables = (list,tuple,set,np.ndarray,pd.Series)
        tipos_fijos = (int,str)
        tipos_permitidos = tipos_iterables + tipos_fijos
        if not isinstance(ids,tipos_permitidos):
            raise TypeError(f'El parámetro "ids" es {type(ids)}, pero se esperaba alguno de {tipos_permitidos}')
        else:
            ids = [self.__check_id(id) for id in ids]
        
        for id in ids:
            self.modificar_incidencia(
                id = id,
                ENS = self.__estimar_ENS_una_incidencia(id,muestra=muestra))
        
    def estimar_ENS_incidencias_abiertas(self,muestra=.33):
        
        if not self.incidencias.query('Status == "ABIERTA"').empty:
            self.estimar_ENS(
                ids = self.incidencias.query('Status == "ABIERTA"').ID,
                muestra = muestra)
        else:
