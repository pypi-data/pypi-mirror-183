...
__version__ = "1.1.3"
__all__ = ["P", "sh", "re", "typer", "sys", "os", "__monitescript_check_returncodes__"]

PIPE = subprocess.PIPE
DEVNULL = subprocess.DEVNULL
__last_bash_cmd_returncode__ = 0
__monitescript_check_returncodes__ = MutableBool(True)
...
def sh(
    ...
    **kwargs,
    ...
    result = subprocess.run(cmd, shell=True, text=True, check=__monitescript_check_returncodes__.value, **kwargs)
...
def add_hook():
    ...
    hook = import_hook.create_hook(
        ...
        extensions=[".monitescript"],
    )
    return hook


def parse_argv(argv: T.List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="monitescript")
...
def main(argv: T.Optional[T.List[str]] = None) -> None:
    ...
    builtins.__monitescript_check_returncodes__ = __monitescript_check_returncodes__  # type: ignore

    if args.script is None:
        from ideas import console

        console.configure(transform_source=transform_source)
        console.start(prompt=">>> ", banner=f"monitescript Console [Python version: {sys.version}]")
        exit(0)
        ...
        new_source = "from monitescript import *\n" + new_source
        ...
        module_name = args.script[: -len(".monitescript")]
