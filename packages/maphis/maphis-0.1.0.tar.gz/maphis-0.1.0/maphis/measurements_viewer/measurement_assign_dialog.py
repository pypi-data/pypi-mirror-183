...
class PropertyAssignment(QObject):
    ...
    def from_dict(_dict: typing.Dict[str, typing.Any], lab_hierarchy: LabelHierarchy) -> 'PropertyAssignment':
        ...
        prop_comp = [prop_comp for prop_comp in prop_comps if prop_comp.info.key == _dict['computation_key']]
        if len(prop_comp) == 0:
...
class MeasurementAssignDialog(QDialog):
    ...
    def _save_current_assignment(self):
        result: typing.Tuple[QUrl, typing.Any] = QFileDialog.getSaveFileUrl(self, 'Save configuration as...')
        if result[0].isEmpty():
            return
        ...
        save_path = pathlib.Path(result[0].toLocalFile())
        with open(save_path, 'w') as f:
            json.dump(assignments_json, f, indent=2)

    def _load_assignments(self):
        result: typing.Tuple[QUrl, typing.Any] = QFileDialog.getOpenFileUrl(self, 'Open configuration as...')
        if result[0].isEmpty():
            return
        load_path = pathlib.Path(result[0].toLocalFile())
        with open(load_path) as f:
            assignments_dicts = json.load(f)
        # TODO clear current assignments
        for assignment_dict in assignments_dicts:
            _prop_assignment: PropertyAssignment = PropertyAssignment.from_dict(assignment_dict, self.state.storage.get_label_hierarchy2('Labels'))
    ...
    def _populate_label_tree(self):
        ...
        hierarchy = self.state.storage.get_label_hierarchy2(self.current_label_name)
