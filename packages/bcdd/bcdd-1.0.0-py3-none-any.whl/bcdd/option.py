...
class FileSelector:
    ...
    @typing.overload
    def get(self, multi: Literal[False]) -> Optional["path.Path"]:
        ...
    
    @typing.overload
    def get(self, multi: Literal[True]) -> Optional[list["path.Path"]]:
        ...
    
    @typing.overload
    def get(self) -> Optional["path.Path"]:
        ...

    def get(self, multi: bool = False) -> Union[Optional["path.Path"], Optional[list["path.Path"]]]:
        if multi:
    ...
    def save(self, default: Optional["path.Path"] = None) -> Optional["path.Path"]:
        pt = filedialog.asksaveasfilename(title=self.title, filetypes=self.filetypes, initialfile=default.get_file_name() if default else None)
        if not pt:
            return None
        return path.Path(pt)

class FolderSelector:
    def __init__(self, title: str, default: Optional["path.Path"] = None):
        self.title = title
    ...
    def get(self) -> "path.Path":
        pt = filedialog.askdirectory(title=self.title, initialdir=self.default.path if self.default else None)
        return path.Path(pt)
