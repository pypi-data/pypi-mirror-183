...
def _compute_hash_and_end_bytes(path: str) -> Tuple[str, List[int]]:
    """
    ...
    Returns
    -------
    A tuple containing hash and end_bytes for the file.
    """
    hash = utils.get_hash(file_name=path)
    end_bytes = utils.get_end_bytes(path=path)
    return hash, end_bytes
...
class Manifest:
    ...
    def _set_database(self) -> None:
        ...
        if job_data is None:
            raise JobDataNotFoundException(f"No job data in {self._manifest_file}")
        ...
        root_id = self._database.get_file_id(path=self._hyperthought_root)

        if root_id is None:
            self._create_hyperthought_root()
        else:
            root = self._database.get_file(file_id=root_id)
            self._hyperthought_root_id_path = (
                f"{root['hyperthought_id_path']}" f"{root['hyperthought_id']},"
            )
    ...
    def _validate_job_data(self) -> None:
        ...
        if not self._workspace_alias or not isinstance(self._workspace_alias, str):
            raise ValueError("workspace_id must be a non-empty string")

        if self._ignore_path is not None and not os.path.isdir(self._ignore_path):
            raise ValueError("ignore_path_prefix must be a directory if provided")

        if not self._hyperthought_root or not isinstance(self._hyperthought_root, str):
    ...
    def common_metadata(self) -> List[MetadataItem]:
        return ht.metadata.from_api_format(self._database.get_common_metadata())
    ...
    def add_path(self, path: str) -> None:
        ...
        # Compute file info.
        # Do this before creating any records in case there's an error.
        ...
        if is_folder:
            ...
            file_hash, end_bytes = _compute_hash_and_end_bytes(path=path)
        ...
        if ancestor_found:
            ...
            ht_id_path = (
                f"{ancestor['hyperthought_id_path']}" f"{ancestor['hyperthought_id']},"
            )
        else:
...
def from_json(manifest_data: Dict, manifest_database_file: str) -> Manifest:
    ...
    for metadata_item in manifest_data["metadata"]["fileSpecific"]:
        metadata = ht.metadata.from_api_format(metadata=metadata_item["metadata"])
        apply_to_file_ids = [
            json_id_to_database_id[id_] for id_ in metadata_item["applyToFileIds"]
        ]
