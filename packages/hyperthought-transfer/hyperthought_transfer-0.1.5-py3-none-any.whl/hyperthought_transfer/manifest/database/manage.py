...
class DatabaseManager:
    ...
    def create_file(
        ...
        if end_bytes is not None:
            ...
            if invalid_chars:
                raise ValueError(f"Invalid chars in end_bytes: {invalid_chars}")
    ...
    def create_or_update_common_metadata(
        ...
        for item in metadata:
            if not isinstance(item, ht.metadata.MetadataItem):
                raise ValueError("all metadata items must be instances of MetadataItem")
    ...
    def bulk_load_files(
        ...
        for item in file_data:
            ...
            if missing_keys:
                raise ValueError(f"Element in file_data missing keys: {missing_keys}")
    ...
    def bulk_load_parsers(
        ...
        for item in parser_data:
            ...
            if missing_keys:
                raise ValueError(f"Element in parser_data missing keys: {missing_keys}")
    ...
    def bulk_load_metadata(
        ...
        for item in metadata_data:
            if not isinstance(item, dict):
                raise ValueError("All elements of metadata_data must be dicts.")
    ...
    def has_other_errors(self) -> bool:
        ...
        cursor.execute(
            sql,
            {
                ...
                "malware_detected_status_id": (FileStatus.MALWARE_DETECTED.value),
