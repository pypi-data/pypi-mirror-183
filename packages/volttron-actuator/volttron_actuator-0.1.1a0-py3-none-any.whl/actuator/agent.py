...
def initialize_agent(config_path, **kwargs):
    ...
    return ActuatorAgent(heartbeat_interval, schedule_publish_interval, preempt_grace_time,
                         driver_vip_identity, allow_no_lock_write, **kwargs)
...
class ActuatorAgent(Agent):
    ...
    def _setup_schedule(self, preempt_grace_time, initial_state=None):
        ...
        self._schedule_manager = ScheduleManager(preempt_grace_time,
                                                 now=now,
    ...
    def _update_device_state_and_schedule(self, now, device_only=None, publish=True):
        ...
        self._update_event = self.core.schedule(new_update_event_time, self._update_schedule_state,
    ...
    def handle_set(self, peer, sender, bus, topic, headers, message):
        ...
        if sender == 'pubsub.compat':
            message = unpack_legacy_message(headers, message)
    ...
    def get_point(self, topic, point=None, **kwargs):
        ...
        return self.vip.rpc.call(self.driver_vip_identity, 'get_point', path, point_name,
                                 **kwargs).get()
    ...
    def _set_point(self, sender, topic, value, point=None, **kwargs):
        topic = topic.strip('/')
        _log.debug('handle_set: {topic},{sender}, {value}'.format(topic=topic,
                                                                  sender=sender,
                                                                  value=value))
        ...
        if self._check_lock(path, sender):
            result = self.vip.rpc.call(self.driver_vip_identity, 'set_point', path, point_name,
                                       value, **kwargs).get()
    ...
    def scrape_all(self, topic):
        ...
        """
        topic = topic.strip('/')
        return self.vip.rpc.call(self.driver_vip_identity, 'scrape_all', topic).get()
    ...
    def get_multiple_points(self, topics, **kwargs):
        ...
        for device, point_names in devices.items():
            r, e = self.vip.rpc.call(self.driver_vip_identity, 'get_multiple_points', device,
                                     point_names, **kwargs).get()
            results.update(r)
            errors.update(e)
    ...
    def set_multiple_points(self, requester_id, topics_values, **kwargs):
        ...
        for device in devices:
            if not self._check_lock(device, requester_id):
                raise LockError("caller ({}) does not lock for device {}".format(
                    requester_id, device))

        for device, point_names_values in devices.items():
            r = self.vip.rpc.call(self.driver_vip_identity, 'set_multiple_points', device,
    ...
    def _revert_point(self, sender, topic, point=None, **kwargs):
        topic = topic.strip('/')
        _log.debug('handle_revert: {topic},{sender}'.format(topic=topic, sender=sender))
        ...
        if self._check_lock(path, sender):
            self.vip.rpc.call(self.driver_vip_identity, 'revert_point', path, point_name,
                              **kwargs).get()
    ...
    def _check_lock(self, device, requester):
        _log.debug('_check_lock: {device}, {requester}'.format(device=device, requester=requester))
        device = device.strip('/')
    ...
    def handle_schedule_request(self, peer, sender, bus, topic, headers, message):
        ...
        _log.debug('handle_schedule_request: {topic}, {headers}, {message}'.format(
        ...
        if request_type == SCHEDULE_ACTION_NEW:
            try:
                ...
                self._request_new_schedule(requester_id, task_id, priority, requests)
            except Exception as ex:
    ...
    def request_new_schedule(self, requester_id, task_id, priority, requests):
        ...
        """
        rpc_peer = self.vip.rpc.context.vip_message.peer
        return self._request_new_schedule(rpc_peer,
                                          task_id,
                                          priority,
                                          requests,
    ...
    def _request_new_schedule(self, sender, task_id, priority, requests, publish_result=True):
        ...
        _log.debug("Got new schedule request: {}, {}, {}, {}".format(sender, task_id, priority,
                                                                     requests))
    ...
    def _handle_unknown_schedule_error(self, ex, headers, message):
        _log.error('bad request: {header}, {request}, {error}'.format(header=headers,
                                                                      request=message,
                                                                      error=str(ex)))
        results = {
            'result': "FAILURE",
