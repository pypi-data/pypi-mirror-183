...
from hydroqc.types import DPCPeakDataTyping
...
class DPCPeakHandler:
    ...
    def today_morning_peak(self) -> Peak | None:
        ...
        now = _now()
        peaks: list[Peak] = [
            p for p in self.peaks if p.day == now.date() and p.start_date.hour < 12
        ]
        if len(peaks) > 1:
    ...
    def tomorrow_morning_peak(self) -> Peak | None:
        ...
        peaks: list[Peak] = [
            p
            for p in self.peaks
            if p.day == now.date() + datetime.timedelta(days=1)
