...
class TinyB(object):
    ...
    async def create_workspace_branch(self, workspace: Dict[str, Any], branch: str):
        return await self._req(f"/v0/workspaces/{workspace['id']}/branch/?branch_name={branch}", method='POST',
                               data=b'')

    async def branch_workspace_data(self, workspace_id: str, other_workspace: str):
        params = {} if not other_workspace else {'origin': other_workspace}
        url = f"/v0/workspaces/{workspace_id}/data_branch?{urlencode(params)}"
        return await self._req(url, method='POST', data=b'')
