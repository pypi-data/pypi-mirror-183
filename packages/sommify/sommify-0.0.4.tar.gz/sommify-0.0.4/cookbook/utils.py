...
from ..cookbook import regex as re_
...
def P_duplicates(phrase):
    return re.sub(rf"({re_.UNIT}) \1\b", r"\1", str(phrase))


def P_multi_size_fix(phrase):
    return re.sub(rf"({re_.Q} {re_.SIZE}) or {re_.Q} {re_.SIZE}", "\1", phrase)
...
def P_missing_multiplier_symbol_fix(phrase):
    pattern = rf"^(?:(?P<multiplier>{re_.Q} )(?P<quantity>{re_.RANGE}|{re_.NUMBER})[- ]?(?P<unit>{re_.UNIT})) (?P<misc>{re_.U_MISC})"
    return re.sub(pattern, r"\g<multiplier>x \g<quantity> \g<unit>", phrase)


def P_quantity_dash_unit_fix(phrase):
    pattern = rf"(?P<quantity>{re_.RANGE}|{re_.NUMBER})-(?P<unit>{re_.UNIT}) (?P<misc>{re_.U_MISC})"
...
def Q_to_number(val):
    ...
    def range_to_number(string):
        lower, upper = re.split(re_.R_SEP, string)
    ...
    multiplier = 1
    if re.match(rf"^({re_.Q}) ?[x\*][ 0-9]", val):
        match = re.match(rf"^({re_.Q}) ?[x\*](?=[0-9 ])(.*)", val)
        multiplier = Q_to_number(match.group(1))
        val = match.group(2).strip()

    if re.match(rf"^{re_.N_WORD}$", val):
        val = word_number_to_number(val)

    elif re.match(rf"^{re_.N_WHOLE}$", val) or re.match(rf"^{re_.N_DECIMAL}$", val):
        val = float(val)

    elif re.match(rf"^{re_.RANGE}$", val):
        val = range_to_number(val)

    elif re.match(rf"^{re_.N_FRACTION}$", val):
        val = fraction_to_number(val)

    elif re.match(rf"^{re_.N_COMPOSED}$", val):
...
def Q_unit_split(quantity):
    quantity, unit = re.search(rf"({re_.Q})?(.*)?", quantity).groups()
    return pd.Series([quantity, unit])


def P_quantity_unit(phrase):
    # match = re.search(rf'({re_.QUANTITY})(.+)', phrase)
    # if not match:
    #     print(phrase)
    #     return pd.Series(['']*4)

    quantity, ingredient = re.search(rf"({re_.QUANTITY})?(.+)?", phrase).groups()
    ingredient = re.sub(rf"^ ?{re_.N_PREP} ", "", ingredient).strip()

    pre_mods, ingredient = re.match(rf"^({re_.MOD})?(.*)?", ingredient).groups()
    ...
    match = re.search(re_.UNIT, quantity)
    unit = match.group() if match else ""
    unit = re.sub(rf"^{re_.N_PREP} ", "", unit).strip()
    quantity = re.sub(re_.UNIT, "", quantity).strip()

    match = re.match(re_.SIZE, ingredient)
    ...
    ingredient = re.sub(rf"^{re_.SIZE} ", "", ingredient)
    quantity = re.sub(rf"^{re_.SIZE} ", "", quantity)
...
def P_juice_zest_fix(phrase):
    ...
    phrase = re.sub(
        rf"(?:^(?:the )?(juice and zest|zest and juice)(?: from| of)?.*?(?P<quantity>{re_.Q})).+(?P<citrus>{r'|'.join(citrus_list)})",
        "\g<quantity> \g<citrus>",
        phrase,
    )

    return re.sub(
        rf"(?:^(?:the )?(?P<part>juice|zest|peel|rind)(?: from| of)?.*?(?P<quantity>{re_.Q})).+(?P<citrus>{r'|'.join(citrus_list)})",
...
def I_label_protein(ingredient):
    if not re.search(re_.PROTEIN, ingredient):
        return ingredient
...
def ingredient_fixer(ingredient, more_frequent_ingredients):
    longest_match = ""
    most_frequent_match = ""

    for more_frequent in more_frequent_ingredients:
        # arugula --> arugula leaf
        if re.search(rf"\b{re.escape(ingredient)}$", more_frequent):
            if not len(most_frequent_match):
                most_frequent_match = more_frequent

        # tasty tomato --> tomato
        if re.search(rf"\b{re.escape(more_frequent)}$", ingredient):
            if len(more_frequent) > len(longest_match):
                longest_match = more_frequent
