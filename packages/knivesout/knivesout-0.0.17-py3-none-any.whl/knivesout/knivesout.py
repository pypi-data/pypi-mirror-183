...
class Config(BaseModel):
    ...
    start_time: Optional[str] = pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')
...
class AbstractStateSwitcher(ABC):
    ...
    def reset_states(self, config: Config, cur_state: str, next_state: str) -> Config:
    ...
    async def stop_execute(self, config: Config):
        logger.info(f"Stop running [{config.command}]")
        pids = await self.get_pids(config)
        config = self.reset_states(config, _ProgramState.stopped, _ProgramState.stopped)
    ...
    async def restart_execute(self, config: Config):
        ...
        logger.info(f"restarting [{config.command}]")
        pids = await self.get_pids(config)
        for pid in pids:
            os.system(f"kill -9 {pid}")
