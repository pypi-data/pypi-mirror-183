...
class M3U8Download(M3U8Parse, M3U8AsyncDownloader):
    ...
    def load(self, url, file_name=None, file_path=None, open_download=True, delete=True, **kwargs):
        ...
        self._run_m3u8(open_download, delete)
    ...
    def _run_m3u8(self, open_download, delete):
        self._initialization()
        self.delete = delete
        segments = self.run_m3u8parae()
        # 显示解析信息 不下载
        ...
        if segments:
            ...
            if completion(self.target) and delete:
                self.delete = True
            ffmpeg(self.target, self.file_name, self.delete)
