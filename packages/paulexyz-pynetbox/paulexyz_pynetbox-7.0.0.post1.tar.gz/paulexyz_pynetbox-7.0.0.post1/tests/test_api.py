...
class ApiTestCase(unittest.TestCase):
    ...
    def test_get(self, *_):
        api = paulexyz_pynetbox.api(host, **def_kwargs)
        self.assertTrue(api)
    ...
    def test_sanitize_url(self, *_):
        api = paulexyz_pynetbox.api("http://localhost:8000/", **def_kwargs)
        self.assertTrue(api)
...
class ApiCreateTokenTestCase(unittest.TestCase):
    ...
    def test_create_token(self, *_):
        api = paulexyz_pynetbox.api(host)
        token = api.create_token("user", "pass")
        self.assertTrue(isinstance(token, paulexyz_pynetbox.core.response.Record))
