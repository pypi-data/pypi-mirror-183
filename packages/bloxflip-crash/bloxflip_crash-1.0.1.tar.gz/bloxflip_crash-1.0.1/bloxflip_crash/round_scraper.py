class scrape_rounds:
    ...
    def start(times):
     # If the number of rounds to gather data for is 0
     if times == int(0):
        # Start an infinite loop
        def loop():
         while True:
            # Gather data for one round
            scrape_rounds.gather_data(1)
        scrape_rounds.threading(loop).start()
     # If the number of rounds to gather data for is not 0
     else:
        # Gather data for the specified number of rounds
        scrape_rounds.gather_data(times)
        scrape_rounds.threading(scrape_rounds.gather_data, args=(times,)).start()
