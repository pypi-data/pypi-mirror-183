...
def supertrend(df,atr_periodsuper,factorsuper):
    ...
    return seniales
def bandas_bollinger(datos, periodo, desviaciones):
    datos['media'] = datos['close'].rolling(window=periodo).mean()
    media_movil = datos['close'].rolling(window=periodo).mean()
    desviacion_estandar = datos['close'].rolling(window=periodo).std(ddof=0)
    banda_superior = media_movil.astype(float) + desviaciones * desviacion_estandar.astype(float)
    banda_inferior = media_movil.astype(float) - desviaciones * desviacion_estandar.astype(float)
