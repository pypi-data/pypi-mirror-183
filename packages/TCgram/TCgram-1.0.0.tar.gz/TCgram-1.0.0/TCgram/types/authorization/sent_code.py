...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~TCgram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~TCgram.Client.sign_in` or :meth:`~TCgram.Client.sign_up`).

        next_type (:obj:`~TCgram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~TCgram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~TCgram.Client.resend_code`.
    """
