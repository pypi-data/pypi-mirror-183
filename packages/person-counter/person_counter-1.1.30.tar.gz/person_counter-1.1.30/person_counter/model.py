      ...
      for i, det in enumerate(pred):
        ...
        if len(self.outputs[i]) > 0:
          for j, (output, conf) in enumerate(zip(self.outputs[i], det[:, 4])):
            bboxes = output[0:4]
            id = int(output[4])
            if not id in self.__mapping:
               self.__mapping[id] = self.__mapping.get(id, len(self.__mapping) + 1)


            cls = output[5]

            detections.append({"bboxes": [int(x) for x in bboxes], "id":self.__mapping[id], "class": self.model.names[int(cls)], "confidence": float(conf) })
