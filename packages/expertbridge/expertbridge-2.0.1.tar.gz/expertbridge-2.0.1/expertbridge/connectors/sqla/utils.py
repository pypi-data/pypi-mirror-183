...
from expertbridge.sql_parse import has_table_query, insert_rls, ParsedQuery
...
def get_virtual_table_metadata(dataset: "SqlaTable") -> List[ResultSetColumnType]:
    ...
    try:
        with closing(engine.raw_connection()) as conn:
            ...
            result_set = ExpertbridgeResultSet(result, cursor.description, db_engine_spec)
            cols = result_set.columns
    except Exception as ex:
        raise ExpertbridgeGenericDBErrorException(message=str(ex)) from ex
    return cols
...
def validate_adhoc_subquery(
    ...
    :raise ExpertbridgeSecurityException if sql contains sub-queries or
    ...
    for statement in sqlparse.parse(sql):
        if has_table_query(statement):
            if not is_feature_enabled("ALLOW_ADHOC_SUBQUERY"):
                raise ExpertbridgeSecurityException(
                    ExpertbridgeError(
                        error_type=ExpertbridgeErrorType.ADHOC_SUBQUERY_NOT_ALLOWED_ERROR,
