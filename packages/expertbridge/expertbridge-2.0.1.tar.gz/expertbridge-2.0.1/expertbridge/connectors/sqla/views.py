...
from expertbridge.connectors.base.views import DatasourceModelView
...
class TableColumnInlineView(
    CompactCRUDMixin, ExpertbridgeModelView
...
class RowLevelSecurityListWidget(
    ...
    template = "expertbridge/models/rls/list.html"
...
class RowLevelSecurityFiltersModelView(
    ExpertbridgeModelView, DeleteMixin
):  # pylint: disable=too-many-ancestors
    datamodel = SQLAInterface(models.RowLevelSecurityFilter)

    list_widget = cast(ExpertbridgeListWidget, RowLevelSecurityListWidget)
...
class TableModelView(  # pylint: disable=too-many-ancestors
    ...
    description_columns = {
        ...
        "sql": _(
            "This fields acts a Expertbridge view, meaning that Expertbridge will "
    ...
    def edit(self, pk: str) -> FlaskResponse:
        ...
        if isinstance(resp, str):
            return resp
        return redirect("/expertbridge/explore/table/{}/".format(pk))
