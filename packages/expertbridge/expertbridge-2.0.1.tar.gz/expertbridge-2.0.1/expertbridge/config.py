...
# under the License.
"""The main config file for Expertbridge

All configuration in this file can be overridden by providing a expertbridge_config
in your PYTHONPATH as there is a ``from expertbridge_config import *``
...
from expertbridge.advanced_data_type.plugins.internet_address import internet_address
from expertbridge.advanced_data_type.plugins.internet_port import internet_port
from expertbridge.advanced_data_type.types import AdvancedDataType
from expertbridge.constants import CHANGE_ME_SECRET_KEY
from expertbridge.jinja_context import BaseTemplateProcessor
from expertbridge.stats_logger import DummyStatsLogger
from expertbridge.expertbridge_typing import CacheConfig
from expertbridge.utils.core import is_test, parse_boolean_string
from expertbridge.utils.encrypt import SQLAlchemyUtilsAdapter
from expertbridge.utils.log import DBEventLogger
from expertbridge.utils.logging_configurator import DefaultLoggingConfigurator
...
EXPERTBRIDGE_LOG_VIEW = True

BASE_DIR = pkg_resources.resource_filename("expertbridge", "")
if "EXPERTBRIDGE_HOME" in os.environ:
    DATA_DIR = os.environ["EXPERTBRIDGE_HOME"]
else:
    DATA_DIR = os.path.expanduser("~/.expertbridge")

# ---------------------------------------------------------
# Expertbridge specific config
# ---------------------------------------------------------
VERSION_INFO_FILE = pkg_resources.resource_filename(
    "expertbridge", "static/version_info.json"
)
PACKAGE_JSON_FILE = pkg_resources.resource_filename(
    "expertbridge", "static/assets/package.json"
)
...
# actually running Expertbridge, we will have already installed,
...
EXPERTBRIDGE_WEBSERVER_PROTOCOL = "http"
EXPERTBRIDGE_WEBSERVER_ADDRESS = "0.0.0.0"
EXPERTBRIDGE_WEBSERVER_PORT = 8088

# This is an important setting, and should be lower than your
# [load balancer / proxy / envoy / kong / ...] timeout settings.
# You should also make sure to configure your WSGI server
# (gunicorn, nginx, apache, ...) timeout setting to be <= to this setting
EXPERTBRIDGE_WEBSERVER_TIMEOUT = int(timedelta(minutes=1).total_seconds())

# this 2 settings are used by dashboard period force refresh feature
# When user choose auto force refresh frequency
# < EXPERTBRIDGE_DASHBOARD_PERIODICAL_REFRESH_LIMIT
# they will see warning message in the Refresh Interval Modal.
# please check PR #9886
EXPERTBRIDGE_DASHBOARD_PERIODICAL_REFRESH_LIMIT = 0
EXPERTBRIDGE_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE = None

EXPERTBRIDGE_DASHBOARD_POSITION_DATA_LIMIT = 65535
CUSTOM_SECURITY_MANAGER = None
SQLALCHEMY_TRACK_MODIFICATIONS = False
# ---------------------------------------------------------

# Your App secret key. Make sure you override it on expertbridge_config.py.
# Use a strong complex alphanumeric string and use a tool to help you generate
# a sufficiently random sequence, ex: openssl rand -base64 42"
SECRET_KEY = CHANGE_ME_SECRET_KEY

# The SQLAlchemy connection string.
SQLALCHEMY_DATABASE_URI = "sqlite:///" + os.path.join(DATA_DIR, "expertbridge.db")
...
WTF_CSRF_EXEMPT_LIST = [
    "expertbridge.views.core.log",
    "expertbridge.views.core.explore_json",
    "expertbridge.charts.data.api.data",
]
...
# Expertbridge allows server-side python stacktraces to be surfaced to the
...
APP_NAME = "Expertbridge"

# Specify the App icon
APP_ICON = "/static/assets/images/expertbridge-logo-horiz.png"

# Specify where clicking the logo would take the user
# e.g. setting it to '/' would take the user to '/expertbridge/welcome/'
...
# Enables SWAGGER UI for expertbridge openapi spec
...
BABEL_DEFAULT_FOLDER = "expertbridge/translations"
...
# overwritten by those specified under FEATURE_FLAGS in expertbridge_config.py
# For example, DEFAULT_FEATURE_FLAGS = { 'FOO': True, 'BAR': False } here
# and FEATURE_FLAGS = { 'BAR': True, 'BAZ': True } in expertbridge_config.py
# will result in combined feature flags of { 'FOO': True, 'BAR': True, 'BAZ': True }
DEFAULT_FEATURE_FLAGS: Dict[str, bool] = {
    # allow dashboard to use sub-domains to send chart request
    # you also need ENABLE_CORS and
    # EXPERTBRIDGE_WEBSERVER_DOMAINS for list of domains
    ...
    # With Expertbridge 2.0, we are updating the default so that the legacy datasource
    # editor no longer shows. Currently this is set to false so that the editor
    # option does show, but we will be depreciating it.
    "DISABLE_LEGACY_DATASOURCE_EDITOR": True,
    # For some security concerns, you may need to enforce CSRF protection on
    # all query request to explore_json endpoint. In Expertbridge, we use
    ...
    # See `PR 7935 <https://github.com/apache/expertbridge/pull/7935>`_ for more details.
    ...
    # encrypted before being saved into expertbridge metastore.
...
# Feature flags may also be set via 'EXPERTBRIDGE_FEATURE_' prefixed environment vars.
DEFAULT_FEATURE_FLAGS.update(
    {
        k[len("EXPERTBRIDGE_FEATURE_") :]: parse_boolean_string(v)
        for k, v in os.environ.items()
        if re.search(r"^EXPERTBRIDGE_FEATURE_\w+", k)
    }
)
...
# THEME_OVERRIDES is used for adding custom theme to expertbridge
...
# Default cache for Expertbridge objects
...
# next available socket. PR #5039 is trying to allow domain sharding for Expertbridge,
# and this feature will be enabled by configuration only (by default Expertbridge
# doesn't allow cross-domain request).
EXPERTBRIDGE_WEBSERVER_DOMAINS = None
...
# time grains in expertbridge/db_engine_specs.builtin_time_grains).
# For example: to disable 1 second time grain:
# TIME_GRAIN_DENYLIST = ['PT1S']
TIME_GRAIN_DENYLIST: List[str] = []

# Additional time grains to be supported using similar definitions as in
# expertbridge/db_engine_specs.builtin_time_grains.
...
DEFAULT_MODULE_DS_MAP = OrderedDict(
    [
        ("expertbridge.connectors.sqla.models", ["SqlaTable"]),
    ]
)
...
FILENAME = os.path.join(DATA_DIR, "expertbridge.log")
ROLLOVER = "midnight"
INTERVAL = 1
BACKUP_COUNT = 30
...
# Additional static HTTP headers to be served by your Expertbridge server. Note
...
#         return 'tmp_expertbridge_schema'
...
# from CSV files. For example, 'companyname-expertbridge'
...
# Roles that are controlled by the API / Expertbridge and should not be changes
# by humans.
ROBOT_PERMISSION_ROLES = ["Public", "Gamma", "Alpha", "Admin", "sql_lab"]

CONFIG_PATH_ENV_VAR = "EXPERTBRIDGE_CONFIG_PATH"
...
SMTP_STARTTLS = True
SMTP_SSL = False
SMTP_USER = "expertbridge"
SMTP_PORT = 25
SMTP_PASSWORD = "expertbridge"
SMTP_MAIL_FROM = "expertbridge@expertbridge.com"
...
MACHINE_AUTH_PROVIDER_CLASS = "expertbridge.utils.machine_auth.MachineAuthProvider"
...
# Slack API token for the expertbridge reports, either string or callable
...
# Send user to a link where they can read more about Expertbridge
...
# in `expertbridge/db_engine_specs/`.
...
# Some sqlalchemy connection strings can open Expertbridge to security risks.
...
# (expertbridge.connectors.sqla.models.SqlaTable), we call this hook
...
# Guest token audience for the embedded expertbridge, either string or callable
...
if CONFIG_PATH_ENV_VAR in os.environ:
    ...
    try:
        ...
        override_conf = imp.load_source("expertbridge_config", cfg_path)
        ...
        raise
elif importlib.util.find_spec("expertbridge_config") and not is_test():
    try:
        # pylint: disable=import-error,wildcard-import,unused-wildcard-import
        import expertbridge_config
        from expertbridge_config import *  # type:ignore

        print(f"Loaded your LOCAL configuration at [{expertbridge_config.__file__}]")
    except Exception:
        logger.exception("Found but failed to import local expertbridge_config")
        raise
