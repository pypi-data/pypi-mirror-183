...
from expertbridge import cache_manager, is_feature_enabled
from expertbridge.common.db_query_status import QueryStatus
from expertbridge.databases.utils import make_url_safe
from expertbridge.db_engine_specs.base import BaseEngineSpec, ColumnTypeMapping
from expertbridge.errors import ExpertbridgeErrorType
from expertbridge.exceptions import ExpertbridgeTemplateException
from expertbridge.models.sql_lab import Query
from expertbridge.models.sql_types.presto_sql_types import (
    Array,
    Date,
    Interval,
    Map,
...
from expertbridge.result_set import destringify
from expertbridge.sql_parse import ParsedQuery
from expertbridge.expertbridge_typing import ResultSetColumnType
from expertbridge.utils import core as utils
from expertbridge.utils.core import ColumnSpec, GenericDataType
...
class PrestoEngineSpec(BaseEngineSpec):  # pylint: disable=too-many-public-methods
    ...
    custom_errors: Dict[Pattern[str], Tuple[str, ExpertbridgeErrorType, Dict[str, Any]]] = {
        COLUMN_DOES_NOT_EXIST_REGEX: (
            __(
                'We can\'t seem to resolve the column "%(column_name)s" at '
                "line %(location)s.",
            ),
            ExpertbridgeErrorType.COLUMN_DOES_NOT_EXIST_ERROR,
            {},
        ),
        TABLE_DOES_NOT_EXIST_REGEX: (
            ...
            ExpertbridgeErrorType.TABLE_DOES_NOT_EXIST_ERROR,
            {},
        ),
        SCHEMA_DOES_NOT_EXIST_REGEX: (
            ...
            ExpertbridgeErrorType.SCHEMA_DOES_NOT_EXIST_ERROR,
            {},
        ),
        CONNECTION_ACCESS_DENIED_REGEX: (
            __('Either the username "%(username)s" or the password is incorrect.'),
            ExpertbridgeErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            {},
        ),
        CONNECTION_INVALID_HOSTNAME_REGEX: (
            __('The hostname "%(hostname)s" cannot be resolved.'),
            ExpertbridgeErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
            {},
        ),
        CONNECTION_HOST_DOWN_REGEX: (
            ...
            ExpertbridgeErrorType.CONNECTION_HOST_DOWN_ERROR,
            {},
        ),
        CONNECTION_PORT_CLOSED_REGEX: (
            __('Port %(port)s on hostname "%(hostname)s" refused the connection.'),
            ExpertbridgeErrorType.CONNECTION_PORT_CLOSED_ERROR,
            {},
        ),
        CONNECTION_UNKNOWN_DATABASE_ERROR: (
            __('Unable to connect to catalog named "%(catalog_name)s".'),
            ExpertbridgeErrorType.CONNECTION_UNKNOWN_DATABASE_ERROR,
            {},
        ),
    }
    ...
    def convert_dttm(
        ...
        Expertbridge only defines time zone naive `datetime` objects, though this method
    ...
    def latest_partition(
        ...
        if not indexes:
            raise ExpertbridgeTemplateException(
    ...
    def latest_sub_partition(
        ...
        for k in kwargs.keys():  # pylint: disable=consider-iterating-dictionary
            if k not in k in part_fields:  # pylint: disable=comparison-with-itself
                ...
                raise ExpertbridgeTemplateException(msg)
