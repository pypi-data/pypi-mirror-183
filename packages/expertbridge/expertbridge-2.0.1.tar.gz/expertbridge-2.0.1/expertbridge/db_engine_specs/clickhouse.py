...
from expertbridge.db_engine_specs.exceptions import ExpertbridgeDBAPIDatabaseError
...
class ClickHouseEngineSpec(BaseEngineSpec):  # pylint: disable=abstract-method
    ...
    def get_dbapi_exception_mapping(cls) -> Dict[Type[Exception], Type[Exception]]:
        return {NewConnectionError: ExpertbridgeDBAPIDatabaseError}

    @classmethod
    def get_dbapi_mapped_exception(cls, exception: Exception) -> Exception:
        new_exception = cls.get_dbapi_exception_mapping().get(type(exception))
        if new_exception == ExpertbridgeDBAPIDatabaseError:
            return ExpertbridgeDBAPIDatabaseError("Connection failed")
