...
from expertbridge.commands.exceptions import CommandInvalidError
from expertbridge.commands.importers.v1.examples import ImportExamplesCommand
from expertbridge.commands.importers.v1.utils import METADATA_FILE_NAME
...
def load_examples_from_configs(
    ...
    """
    Load all the examples inside expertbridge/examples/configs/.
    """
...
def load_contents(load_test_data: bool = False) -> Dict[str, Any]:
    ...
    resource_names = resource_listdir("expertbridge", str(root))
    ...
    while queue:
        ...
        if resource_isdir("expertbridge", str(path_name)):
            queue.extend(
                path_name / child_name
                for child_name in resource_listdir("expertbridge", str(path_name))
            )
        elif path_name.suffix.lower() in YAML_EXTENSIONS:
            if load_test_data and test_re.search(str(path_name)) is None:
                continue
            contents[path_name] = (
                resource_stream("expertbridge", str(path_name)).read().decode("utf-8")
            )
