...
from expertbridge.commands.exceptions import (
    CommandException,
    CommandInvalidError,
    CreateFailedError,
    DeleteFailedError,
...
class DatabaseTestConnectionFailedError(ExpertbridgeErrorsException):
    status = 422
    message = _("Connection failed, please check your connection settings")
...
class DatabaseTestConnectionUnexpectedError(ExpertbridgeErrorsException):
    status = 422
    message = _("Unexpected error occurred, please check your logs for details")


class NoValidatorConfigFoundError(ExpertbridgeErrorException):
    status = 422
    message = _("no SQL validator is configured")


class NoValidatorFoundError(ExpertbridgeErrorException):
    status = 422
    message = _("No validator found (configured for the engine)")


class ValidatorSQLError(ExpertbridgeErrorException):
    status = 422
...
class ValidatorSQL400Error(ExpertbridgeErrorException):
    status = 400
    message = _("Was unable to check your query")
...
class InvalidEngineError(ExpertbridgeErrorException):
    status = 422


class DatabaseOfflineError(ExpertbridgeErrorException):
    status = 422


class InvalidParametersError(ExpertbridgeErrorsException):
    status = 422
