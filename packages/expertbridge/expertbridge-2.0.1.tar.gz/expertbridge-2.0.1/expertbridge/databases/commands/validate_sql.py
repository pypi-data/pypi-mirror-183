...
from expertbridge.sql_validators.base import BaseSQLValidator
...
class ValidateSQLCommand(BaseCommand):
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(ex)
            expertbridge_error = ExpertbridgeError(
                message=__(
            ...
            if re.search(r"([\W]|^)4\d{2}([\W]|$)", str(ex)):
                raise ValidatorSQL400Error(expertbridge_error) from ex
            raise ValidatorSQLError(expertbridge_error) from ex
