...
from expertbridge.exceptions import (
    ...
    ExpertbridgeGenericDBErrorException,
...
class SynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    ...
    def execute(
        ...
        try:
            ...
            raise ExpertbridgeGenericDBErrorException(
        ...
        if data.get("status") == QueryStatus.FAILED:  # type: ignore
            ...
            if data["errors"]:  # type: ignore
                raise ExpertbridgeErrorsException(
                    [ExpertbridgeError(**params) for params in data["errors"]]  # type: ignore
                )
            # old string-only error message
            raise ExpertbridgeGenericDBErrorException(data["error"])  # type: ignore
    ...
    def execute(
        ...
        try:
            ...
            error = ExpertbridgeError(
                message=message,
                error_type=ExpertbridgeErrorType.ASYNC_WORKERS_ERROR,
            ...
            raise ExpertbridgeErrorException(error) from ex
        self._query_dao.update_saved_query_exec_info(query_id)
