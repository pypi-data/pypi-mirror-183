...
from expertbridge import app, db, security_manager
from expertbridge.extensions import celery_app
from expertbridge.models.core import Log
from expertbridge.models.dashboard import Dashboard
from expertbridge.models.slice import Slice
from expertbridge.models.tags import Tag, TaggedObject
from expertbridge.utils.date_parser import parse_human_datetime
from expertbridge.utils.machine_auth import MachineAuthProvider
...
def get_url(chart: Slice, dashboard: Optional[Dashboard] = None) -> str:
    ...
    with app.test_request_context():
        ...
        url = f"{baseurl}expertbridge/warm_up_cache/?slice_id={chart.id}"
        if dashboard:
...
class Strategy:  # pylint: disable=too-few-public-methods
    ...
    be fetched from the `/expertbridge/warm_up_cache/` endpoint.

    Strategies can be configured in `expertbridge/config.py`:
