...
from expertbridge.exceptions import ExpertbridgeVizException
...
from expertbridge.views.utils import get_datasource_info, get_viz
...
def load_explore_json_into_cache(  # pylint: disable=too-many-locals
    ...
    try:
        ...
        if viz_obj.has_error(payload):
            raise ExpertbridgeVizException(errors=payload["errors"])
        ...
        result_url = f"/expertbridge/explore_json/data/{cache_key}"
        async_query_manager.update_job(
            job_metadata,
            async_query_manager.STATUS_DONE,
            result_url=result_url,
        ...
        if isinstance(ex, ExpertbridgeVizException):
