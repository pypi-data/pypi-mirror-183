...
from expertbridge.views.base import BaseExpertbridgeView

logger = logging.getLogger(__name__)


class R(BaseExpertbridgeView):  # pylint: disable=invalid-name
    ...
    def _validate_url(url: Optional[str] = None) -> bool:
        if url and (
            url.startswith("//expertbridge/dashboard/")
            or url.startswith("//expertbridge/explore/")
        ):
            return True
        return False
    ...
    def index(self, url_id: int) -> FlaskResponse:
        ...
        if url and url.url:
            explore_url = "//expertbridge/explore/?"
