...
from expertbridge.models.sql_lab import Query, SavedQuery, TableSchema, TabState
from expertbridge.expertbridge_typing import FlaskResponse
from expertbridge.utils import core as utils

from .base import BaseExpertbridgeView, DeleteMixin, json_success, ExpertbridgeModelView
...
class TabStateView(BaseExpertbridgeView):
    @has_access_api
    @expose("/", methods=["POST"])
...
class TableSchemaView(BaseExpertbridgeView):
    @has_access_api
    @expose("/", methods=["POST"])
    def post(self) -> FlaskResponse:  # pylint: disable=no-self-use
...
class SqlLab(BaseExpertbridgeView):
