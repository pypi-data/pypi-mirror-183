...
from expertbridge.commands.exceptions import CommandException, CommandInvalidError
from expertbridge.connectors.sqla import models
from expertbridge.datasets.commands.exceptions import get_dataset_exist_error_msg
from expertbridge.db_engine_specs import get_available_engine_specs
from expertbridge.db_engine_specs.gsheets import GSheetsEngineSpec
...
from expertbridge.exceptions import (
    ...
    ExpertbridgeErrorsException,
    ExpertbridgeException,
    ExpertbridgeSecurityException,
)
from expertbridge.models.helpers import ImportExportMixin
from expertbridge.models.reports import ReportRecipientType
from expertbridge.expertbridge_typing import FlaskResponse
from expertbridge.translations.utils import get_language_pack
from expertbridge.utils import core as utils

from .utils import bootstrap_user_data

FRONTEND_CONF_KEYS = (
    "EXPERTBRIDGE_WEBSERVER_TIMEOUT",
    "EXPERTBRIDGE_DASHBOARD_POSITION_DATA_LIMIT",
    "EXPERTBRIDGE_DASHBOARD_PERIODICAL_REFRESH_LIMIT",
    "EXPERTBRIDGE_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE",
    ...
    "DEFAULT_VIZ_TYPE",
    "SQL_MAX_ROW",
    "EXPERTBRIDGE_WEBSERVER_DOMAINS",
...
config = expertbridge_app.config
...
def api(f: Callable[..., FlaskResponse]) -> Callable[..., FlaskResponse]:
    ...
    def wraps(self: "BaseExpertbridgeView", *args: Any, **kwargs: Any) -> FlaskResponse:
        try:
...
def handle_api_exception(
    ...
    """
    A decorator to catch expertbridge exceptions. Use it after the @api decorator above
    so expertbridge exception handler is triggered before the handler for generic
    exceptions.
    """
    ...
    def wraps(self: "BaseExpertbridgeView", *args: Any, **kwargs: Any) -> FlaskResponse:
        try:
            ...
            logger.warning("ExpertbridgeSecurityException", exc_info=True)
            return json_errors_response(
                errors=[ex.error], status=ex.status, payload=ex.payload
            )
        except ExpertbridgeErrorsException as ex:
            logger.warning(ex, exc_info=True)
            return json_errors_response(errors=ex.errors, status=ex.status)
        except ExpertbridgeErrorException as ex:
            logger.warning("ExpertbridgeErrorException", exc_info=True)
...
class BaseExpertbridgeView(BaseView):
    @staticmethod
...
def menu_data() -> Dict[str, Any]:
    ...
    return {
        ...
        "navbar_right": {
            ...
            "show_watermark": ("expertbridge-logo-horiz" not in appbuilder.app_icon),
            ...
            else f"/expertbridge/profile/{g.user.username}",
...
# ExpertbridgeErrorException or ExpertbridgeErrorsException
@expertbridge_app.errorhandler(ExpertbridgeErrorException)
def show_expertbridge_error(ex: ExpertbridgeErrorException) -> FlaskResponse:
    logger.warning("ExpertbridgeErrorException", exc_info=True)
    return json_errors_response(errors=[ex.error], status=ex.status)


@expertbridge_app.errorhandler(ExpertbridgeErrorsException)
def show_expertbridge_errors(ex: ExpertbridgeErrorsException) -> FlaskResponse:
    logger.warning("ExpertbridgeErrorsException", exc_info=True)
    return json_errors_response(errors=ex.errors, status=ex.status)


# Redirect to login if the CSRF token is expired
@expertbridge_app.errorhandler(CSRFError)
...
@expertbridge_app.errorhandler(HTTPException)
def show_http_exception(ex: HTTPException) -> FlaskResponse:
    ...
    if (
        ...
        path = resource_filename("expertbridge", f"static/assets/{ex.code}.html")
...
# CommandException it should be fixed to map it to ExpertbridgeErrorException
# or ExpertbridgeErrorsException, with a specific status code and error type
@expertbridge_app.errorhandler(CommandException)
def show_command_errors(ex: CommandException) -> FlaskResponse:
    logger.warning("CommandException", exc_info=True)
    if "text/html" in request.accept_mimetypes and not config["DEBUG"]:
        path = resource_filename("expertbridge", "static/assets/500.html")
    ...
    return json_errors_response(
        errors=[
            ExpertbridgeError(
                message=ex.message,
                error_type=ExpertbridgeErrorType.GENERIC_COMMAND_ERROR,
...
@expertbridge_app.errorhandler(Exception)
...
@expertbridge_app.context_processor
...
class ExpertbridgeListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    template = "expertbridge/fab_overrides/list.html"


class ExpertbridgeModelView(ModelView):
    page_size = 100
    list_widget = ExpertbridgeListWidget
...
class ListWidgetWithCheckboxes(ListWidget):  # pylint: disable=too-few-public-methods
    ...
    template = "expertbridge/fab_overrides/list_with_checkboxes.html"
...
def check_ownership(obj: Any, raise_if_false: bool = True) -> bool:
    ...
    security_exception = ExpertbridgeSecurityException(
        ExpertbridgeError(
            error_type=ExpertbridgeErrorType.MISSING_OWNERSHIP_ERROR,
...
@expertbridge_app.after_request
