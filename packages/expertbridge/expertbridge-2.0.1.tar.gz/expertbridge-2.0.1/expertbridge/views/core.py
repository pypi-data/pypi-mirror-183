...
from expertbridge.charts.commands.exceptions import ChartNotFoundError
from expertbridge.charts.dao import ChartDAO
from expertbridge.common.chart_data import ChartDataResultFormat, ChartDataResultType
from expertbridge.common.db_query_status import QueryStatus
from expertbridge.connectors.base.models import BaseDatasource
from expertbridge.connectors.connector_registry import ConnectorRegistry
from expertbridge.connectors.sqla.models import (
    AnnotationDatasource,
    SqlaTable,
    SqlMetric,
    TableColumn,
)
from expertbridge.dashboards.commands.importers.v0 import ImportDashboardsCommand
from expertbridge.dashboards.dao import DashboardDAO
from expertbridge.dashboards.permalink.commands.get import GetDashboardPermalinkCommand
from expertbridge.dashboards.permalink.exceptions import DashboardPermalinkGetFailedError
from expertbridge.databases.commands.exceptions import DatabaseInvalidError
from expertbridge.databases.dao import DatabaseDAO
from expertbridge.databases.filters import DatabaseFilter
from expertbridge.databases.utils import make_url_safe
from expertbridge.datasets.commands.exceptions import DatasetNotFoundError
from expertbridge.errors import ErrorLevel, ExpertbridgeError, ExpertbridgeErrorType
from expertbridge.exceptions import (
    CacheLoadError,
    CertificateException,
    DatabaseNotFound,
    SerializationError,
    ExpertbridgeCancelQueryException,
    ExpertbridgeErrorException,
    ExpertbridgeException,
    ExpertbridgeGenericErrorException,
    ExpertbridgeSecurityException,
    ExpertbridgeTimeoutException,
)
from expertbridge.explore.form_data.commands.get import GetFormDataCommand
from expertbridge.explore.form_data.commands.parameters import CommandParameters
from expertbridge.explore.permalink.commands.get import GetExplorePermalinkCommand
from expertbridge.explore.permalink.exceptions import ExplorePermalinkGetFailedError
from expertbridge.extensions import async_query_manager, cache_manager
from expertbridge.jinja_context import get_template_processor
from expertbridge.models.core import Database, FavStar, Log
from expertbridge.models.dashboard import Dashboard
from expertbridge.models.datasource_access_request import DatasourceAccessRequest
from expertbridge.models.slice import Slice
from expertbridge.models.sql_lab import Query, TabState
from expertbridge.models.user_attributes import UserAttribute
from expertbridge.queries.dao import QueryDAO
from expertbridge.security.analytics_db_safety import check_sqlalchemy_uri
from expertbridge.sql_lab import get_sql_results
from expertbridge.sql_parse import ParsedQuery
from expertbridge.sql_validators import get_validator_by_name
from expertbridge.sqllab.command import CommandResult, ExecuteSqlCommand
from expertbridge.sqllab.command_status import SqlJsonExecutionStatus
from expertbridge.sqllab.exceptions import (
    QueryIsForbiddenToAccessException,
    SqlLabException,
)
from expertbridge.sqllab.execution_context_convertor import ExecutionContextConvertor
from expertbridge.sqllab.limiting_factor import LimitingFactor
from expertbridge.sqllab.query_render import SqlQueryRenderImpl
from expertbridge.sqllab.sql_json_executer import (
    ASynchronousSqlJsonExecutor,
    SqlJsonExecutor,
    SynchronousSqlJsonExecutor,
)
from expertbridge.sqllab.sqllab_execution_context import SqlJsonExecutionContext
from expertbridge.sqllab.utils import apply_display_max_row_configuration_if_require
from expertbridge.sqllab.validators import CanAccessQueryValidatorImpl
from expertbridge.expertbridge_typing import FlaskResponse
from expertbridge.tasks.async_queries import load_explore_json_into_cache
from expertbridge.utils import core as utils, csv
from expertbridge.utils.async_query_manager import AsyncQueryTokenException
from expertbridge.utils.cache import etag_cache
from expertbridge.utils.core import apply_max_row_limit, ReservedUrlParameters
from expertbridge.utils.dates import now_as_float
from expertbridge.utils.decorators import check_dashboard_access
...
from expertbridge.views.utils import (
...
from expertbridge.viz import BaseViz
...
class Expertbridge(BaseExpertbridgeView):  # pylint: disable=too-many-public-methods
    """The base views for Expertbridge!"""
    ...
    def request_access(self) -> FlaskResponse:
        ...
        if has_access_:
            return redirect("/expertbridge/dashboard/{}".format(dashboard_id))
        ...
        return self.render_template(
            "expertbridge/request_access.html",
    ...
    def slice(self, slice_id: int) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        if not slc:
            abort(404)
        endpoint = "/expertbridge/explore/?form_data={}".format(
    ...
    def explore_json(
        ...
        if (
            ...
            and not security_manager.can_access("can_csv", "Expertbridge")
        ):
    ...
    def import_dashboards(self) -> FlaskResponse:
            ...
            try:
                ...
                flash(
                    _(
                        ...
                        "Please contact your Expertbridge administrator"
                    ),
                    "danger",
                )
            if success:
        ...
        return self.render_template(
            "expertbridge/import_dashboards.html", databases=databases
        )
    ...
    def explore(
        ...
        slice_download_perm = security_manager.can_access("can_csv", "Expertbridge")
        ...
        except (ExpertbridgeException, SQLAlchemyError):
    ...
    def filter(  # pylint: disable=no-self-use
        ...
        :raises ExpertbridgeSecurityException: If the user cannot access the resource
        """
    ...
    def available_domains(self) -> FlaskResponse:  # pylint: disable=no-self-use
        """
        Returns the list of available Expertbridge Webserver domains (if any)
        defined in config. This enables charts embedded in other apps to
        leverage domain sharding if appropriately configured.
        """
        return Response(
            json.dumps(conf.get("EXPERTBRIDGE_WEBSERVER_DOMAINS")), mimetype="text/json"
        )
    ...
    def fave_dashboards(self, user_id: int) -> FlaskResponse:
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                dash["creator_url"] = "/expertbridge/profile/{}/".format(user.username)
            payload.append(dash)
        return json_success(json.dumps(payload, default=utils.json_int_dttm_ser))
        ...
        if config["ENABLE_ACCESS_REQUEST"]:
            for datasource in dashboard.datasources:
                ...
                if datasource and not security_manager.can_access_datasource(
                    ...
                    return redirect(
                        f"/expertbridge/request_access/?dashboard_id={dashboard.id}"
                    )

        dash_edit_perm = check_ownership(
            dashboard, raise_if_false=False
        ) and security_manager.can_access("can_save_dash", "Expertbridge")
        edit_mode = (
    ...
    def dashboard_permalink(  # pylint: disable=no-self-use
        ...
        url = f"/expertbridge/dashboard/{dashboard_id}?permalink_key={key}"
    ...
    def sqllab_viz(self) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        try:
            ...
            raise ExpertbridgeGenericErrorException(
                __(
        ...
        if not database:
            raise ExpertbridgeErrorException(
                ExpertbridgeError(
                    message=__("The database was not found."),
                    error_type=ExpertbridgeErrorType.DATABASE_NOT_FOUND_ERROR,
    ...
    def estimate_query_cost(  # pylint: disable=no-self-use
        ...
        except ExpertbridgeTimeoutException as ex:
            logger.exception(ex)
    ...
    def theme(self) -> FlaskResponse:
        return self.render_template("expertbridge/theme.html")
        ...
        if not results_backend:
            raise ExpertbridgeErrorException(
                ExpertbridgeError(
                    ...
                    error_type=ExpertbridgeErrorType.RESULTS_BACKEND_NOT_CONFIGURED_ERROR,
                    level=ErrorLevel.ERROR,
                )
            )
        ...
        if not blob:
            raise ExpertbridgeErrorException(
                ExpertbridgeError(
                    ...
                    error_type=ExpertbridgeErrorType.RESULTS_BACKEND_ERROR,
                    level=ErrorLevel.ERROR,
                ),
                status=410,
            )
        ...
        try:
            ...
            raise ExpertbridgeErrorException(
                ExpertbridgeError(
                    ...
                    error_type=ExpertbridgeErrorType.QUERY_SECURITY_ACCESS_ERROR,
                    level=ErrorLevel.ERROR,
                ),
                status=403,
            ) from ex
    ...
    def stop_query(self) -> FlaskResponse:
        ...
        if not sql_lab.cancel_query(query):
            raise ExpertbridgeCancelQueryException("Could not cancel query")
    ...
    def _create_sql_json_command(
        ...
        query_dao = QueryDAO()
        sql_json_executor = Expertbridge._create_sql_json_executor(
    ...
    def csv(  # pylint: disable=no-self-use,too-many-locals
        ...
        logger.debug(
            "CSV exported: %s", event_rep, extra={"expertbridge_event": event_info}
        )
        return response
    ...
    def search_queries(self) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        page /expertbridge/sqllab#search.
    ...
    def show_traceback(self) -> FlaskResponse:  # pylint: disable=no-self-use
        return (
            render_template("expertbridge/traceback.html", error_msg=get_error_msg()),
            500,
        )
    ...
    def welcome(self) -> FlaskResponse:
        ...
        if not g.user or not g.user.get_id():
            if conf["PUBLIC_ROLE_LIKE"]:
                return self.render_template("expertbridge/public_welcome.html")
    ...
    def sqllab(self) -> FlaskResponse:
        ...
        return self.render_template(
            "expertbridge/basic.html", entry="sqllab", bootstrap_data=bootstrap_data
        )
    ...
    def schemas_access_for_file_upload(self) -> FlaskResponse:
        ...
        try:
            ...
            return json_error_response(
                ...
                "Please contact your Expertbridge Admin!"
            )
