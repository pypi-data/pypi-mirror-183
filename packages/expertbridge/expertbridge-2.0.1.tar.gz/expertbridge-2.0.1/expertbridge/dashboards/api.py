...
from expertbridge import is_feature_enabled, thumbnail_cache
from expertbridge.charts.schemas import ChartEntityResponseSchema
from expertbridge.commands.importers.exceptions import NoValidFilesFoundError
from expertbridge.commands.importers.v1.utils import get_contents_from_bundle
from expertbridge.constants import MODEL_API_RW_METHOD_PERMISSION_MAP, RouteMethod
from expertbridge.dashboards.commands.bulk_delete import BulkDeleteDashboardCommand
from expertbridge.dashboards.commands.create import CreateDashboardCommand
from expertbridge.dashboards.commands.delete import DeleteDashboardCommand
...
from expertbridge.dashboards.commands.export import ExportDashboardsCommand
from expertbridge.dashboards.commands.importers.dispatcher import ImportDashboardsCommand
from expertbridge.dashboards.commands.update import UpdateDashboardCommand
from expertbridge.dashboards.dao import DashboardDAO
from expertbridge.dashboards.filters import (
...
from expertbridge.dashboards.schemas import (
...
from expertbridge.extensions import event_logger
from expertbridge.models.dashboard import Dashboard
from expertbridge.models.embedded_dashboard import EmbeddedDashboard
from expertbridge.tasks.thumbnails import cache_dashboard_thumbnail
from expertbridge.utils.cache import etag_cache
from expertbridge.utils.screenshots import DashboardScreenshot
from expertbridge.utils.urls import get_url_path
from expertbridge.views.base import generate_download_headers
from expertbridge.views.base_api import (
    BaseExpertbridgeModelRestApi,
    RelatedFieldFilter,
    requires_form_data,
    requires_json,
    statsd_metrics,
)
from expertbridge.views.filters import FilterRelatedOwners

logger = logging.getLogger(__name__)


def with_dashboard(
    f: Callable[[BaseExpertbridgeModelRestApi, Dashboard], Response]
) -> Callable[[BaseExpertbridgeModelRestApi, str], Response]:
    """
    ...
    def wraps(self: BaseExpertbridgeModelRestApi, id_or_slug: str) -> Response:
        try:
...
class DashboardRestApi(BaseExpertbridgeModelRestApi):
    ...
    def __repr__(self) -> str:
        ...
        return "Expertbridge.dashboards.api.DashboardRestApi@v{}{}".format(
    ...
    def thumbnail(self, pk: int, digest: str, **kwargs: Any) -> WerkzeugResponse:
        ...
        dashboard_url = get_url_path(
            "Expertbridge.dashboard", dashboard_id_or_slug=dashboard.id
        )
