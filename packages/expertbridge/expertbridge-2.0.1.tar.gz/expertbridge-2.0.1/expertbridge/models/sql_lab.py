...
from expertbridge.models.tags import QueryUpdater
from expertbridge.sql_parse import CtasMethod, ParsedQuery, Table
from expertbridge.sqllab.limiting_factor import LimitingFactor
from expertbridge.utils.core import QueryStatus, user_label
...
class Query(Model, ExtraJSONMixin):
    ...
    # Could be configured in the expertbridge config.
...
class SavedQuery(Model, AuditMixinNullable, ExtraJSONMixin, ImportExportMixin):
    ...
    def pop_tab_link(self) -> Markup:
        return Markup(
            f"""
            <a href="/expertbridge/sqllab?savedQueryId={self.id}">
    ...
    def url(self) -> str:
        return "/expertbridge/sqllab?savedQueryId={0}".format(self.id)
