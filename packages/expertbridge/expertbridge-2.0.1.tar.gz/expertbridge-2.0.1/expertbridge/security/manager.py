...
from expertbridge import sql_parse
from expertbridge.connectors.connector_registry import ConnectorRegistry
from expertbridge.constants import RouteMethod
...
from expertbridge.security.guest_token import (
    GuestToken,
    GuestTokenResources,
...
from expertbridge.utils.core import DatasourceName, RowLevelSecurityFilterType
from expertbridge.utils.urls import get_url_host

if TYPE_CHECKING:
    from expertbridge.common.query_context import QueryContext
    ...
    from expertbridge.models.sql_lab import Query
    from expertbridge.sql_parse import Table
...
class ExpertbridgeSecurityListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    """
    Redeclaring to avoid circular imports
    """

    template = "expertbridge/fab_overrides/list.html"


class ExpertbridgeRoleListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    """
    Role model view from FAB already uses a custom list widget override
    So we override the override
    """

    template = "expertbridge/fab_overrides/list_role.html"
...
UserModelView.list_widget = ExpertbridgeSecurityListWidget
RoleModelView.list_widget = ExpertbridgeRoleListWidget
PermissionViewModelView.list_widget = ExpertbridgeSecurityListWidget
PermissionModelView.list_widget = ExpertbridgeSecurityListWidget
...
class ExpertbridgeSecurityManager(  # pylint: disable=too-many-public-methods
    SecurityManager
):
    ...
    SQLLAB_PERMISSION_VIEWS = {
        ("can_csv", "Expertbridge"),
        ("can_read", "SavedQuery"),
        ("can_read", "Database"),
        ("can_sql_json", "Expertbridge"),
        ("can_sqllab_viz", "Expertbridge"),
        ("can_sqllab_table_viz", "Expertbridge"),
        ("can_sqllab", "Expertbridge"),
        ("menu_access", "SQL Lab"),
    ...
    def request_loader(self, request: Request) -> Optional[User]:
        ...
        from expertbridge.extensions import feature_flag_manager

        if feature_flag_manager.is_feature_enabled("EMBEDDED_EXPERTBRIDGE"):
    ...
    def get_schema_perm(  # pylint: disable=no-self-use
        ...
        :param database: The Expertbridge database or database name
        :param schema: The Expertbridge schema name
        :return: The database specific schema permission
        """
    ...
    def can_access_all_datasources(self) -> bool:
        """
        Return True if the user can fully access all the Expertbridge datasources, False
        otherwise.

        :returns: Whether the user can fully access all Expertbridge datasources
        """
    ...
    def can_access_all_databases(self) -> bool:
        """
        Return True if the user can fully access all the Expertbridge databases, False
        otherwise.

        :returns: Whether the user can fully access all Expertbridge databases
        """
    ...
    def can_access_database(self, database: "Database") -> bool:
        """
        Return True if the user can fully access the Expertbridge database, False otherwise.

        Note for Druid the database is akin to the Druid cluster.

        :param database: The Expertbridge database
        :returns: Whether the user can fully access the Expertbridge database
        """
    ...
    def can_access_schema(self, datasource: "BaseDatasource") -> bool:
        """
        Return True if the user can fully access the schema associated with the Expertbridge
        ...
        :param datasource: The Expertbridge datasource
    ...
    def can_access_datasource(self, datasource: "BaseDatasource") -> bool:
        """
        Return True if the user can fully access of the Expertbridge datasource, False
        otherwise.

        :param datasource: The Expertbridge datasource
        :returns: Whether the user can fully access the Expertbridge datasource
        """

        try:
            self.raise_for_access(datasource=datasource)
        except ExpertbridgeSecurityException:
            return False
    ...
    def get_datasource_access_error_msg(datasource: "BaseDatasource") -> str:
        """
        Return the error message for the denied Expertbridge datasource.

        :param datasource: The denied Expertbridge datasource
    ...
    def get_datasource_access_link(  # pylint: disable=unused-argument
        ...
        """
        Return the link for the denied Expertbridge datasource.
    ...
    def get_datasource_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied Expertbridge datasource.

        :param datasource: The denied Expertbridge datasource
        :returns: The error object
        """
        return ExpertbridgeError(
            error_type=ExpertbridgeErrorType.DATASOURCE_SECURITY_ACCESS_ERROR,
    ...
    def get_table_access_error_object(self, tables: Set["Table"]) -> ExpertbridgeError:
        """
        ...
        """
        return ExpertbridgeError(
            error_type=ExpertbridgeErrorType.TABLE_SECURITY_ACCESS_ERROR,
    ...
    def create_custom_permissions(self) -> None:
        ...
        self.add_permission_view_menu("can_share_dashboard", "Expertbridge")
        self.add_permission_view_menu("can_share_chart", "Expertbridge")
    ...
    def create_missing_perms(self) -> None:
        ...
        # pylint: disable=import-outside-toplevel
        from expertbridge.models import core as models
    ...
    def sync_role_definitions(self) -> None:
        """
        Initialize the Expertbridge application with security roles and such.
        """
    ...
    def raise_for_access(
        ...
        :param table: The Expertbridge table (requires database)
        ...
        from expertbridge.extensions import feature_flag_manager
        from expertbridge.sql_parse import Table
        from expertbridge.views.utils import is_owner
        ...
        if database and table or query:
            ...
            if denied:
                raise ExpertbridgeSecurityException(
    ...
    def raise_for_user_activity_access(user_id: int) -> None:
        ...
        if not user or (
            ...
            raise ExpertbridgeSecurityException(
                ExpertbridgeError(
                    error_type=ExpertbridgeErrorType.USER_ACTIVITY_SECURITY_ACCESS_ERROR,
    ...
    def raise_for_dashboard_access(self, dashboard: "Dashboard") -> None:
        ...
        """
        # pylint: disable=import-outside-toplevel
        from expertbridge import is_feature_enabled
        from expertbridge.dashboards.commands.exceptions import DashboardAccessDeniedError
        from expertbridge.views.base import is_user_admin
        from expertbridge.views.utils import is_owner
    ...
    def can_access_based_on_dashboard(datasource: "BaseDatasource") -> bool:
        ...
        from expertbridge.dashboards.filters import DashboardAccessFilter
        from expertbridge.models.dashboard import Dashboard
    ...
    def validate_guest_token_resources(resources: GuestTokenResources) -> None:
        # pylint: disable=import-outside-toplevel
        from expertbridge.embedded.dao import EmbeddedDAO
        from expertbridge.embedded_dashboard.commands.exceptions import (
    ...
    def is_guest_user(user: Optional[Any] = None) -> bool:
        ...
        if not is_feature_enabled("EMBEDDED_EXPERTBRIDGE"):
            return False
        if not user:
            user = g.user
