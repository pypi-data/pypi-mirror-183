...
from expertbridge.dataframe import df_to_records
from expertbridge.db_engine_specs import BaseEngineSpec
from expertbridge.errors import ErrorLevel, ExpertbridgeError, ExpertbridgeErrorType
from expertbridge.exceptions import ExpertbridgeErrorException, ExpertbridgeErrorsException
from expertbridge.extensions import celery_app
from expertbridge.models.core import Database
from expertbridge.models.sql_lab import Query
from expertbridge.result_set import ExpertbridgeResultSet
from expertbridge.sql_parse import CtasMethod, insert_rls, ParsedQuery
from expertbridge.sqllab.limiting_factor import LimitingFactor
from expertbridge.utils.celery import session_scope
...
from expertbridge.utils.decorators import stats_timing
...
def handle_query_error(
    ...
    if isinstance(ex, ExpertbridgeErrorException):
        errors = [ex.error]
    elif isinstance(ex, ExpertbridgeErrorsException):
        errors = ex.errors
    else:
...
def execute_sql_statement(  # pylint: disable=too-many-arguments,too-many-statements
    ...
    if not db_engine_spec.is_readonly_query(parsed_query) and not database.allow_dml:
        raise ExpertbridgeErrorException(
            ExpertbridgeError(
                ...
                error_type=ExpertbridgeErrorType.DML_NOT_ALLOWED_ERROR,
    ...
    try:
        ...
        raise ExpertbridgeErrorException(
            ExpertbridgeError(
                ...
                error_type=ExpertbridgeErrorType.SQLLAB_TIMEOUT_ERROR,
                level=ErrorLevel.ERROR,
            )
        ) from ex
    except Exception as ex:
    ...
    return ExpertbridgeResultSet(data, cursor_description, db_engine_spec)
...
def _serialize_and_expand_data(
    result_set: ExpertbridgeResultSet,
    ...
    if (
        ...
        raise ExpertbridgeErrorException(
            ExpertbridgeError(
                ...
                error_type=ExpertbridgeErrorType.INVALID_CTAS_QUERY_ERROR,
                level=ErrorLevel.ERROR,
            )
        )
    if (
        ...
        raise ExpertbridgeErrorException(
            ExpertbridgeError(
                ...
                error_type=ExpertbridgeErrorType.INVALID_CVAS_QUERY_ERROR,
                level=ErrorLevel.ERROR,
            )
        )
