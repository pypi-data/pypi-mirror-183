...
def compute_thumbnails(
    ...
    from expertbridge.tasks.thumbnails import (
    ...
    def compute_generic_thumbnail(
        ...
        for i, model in enumerate(dashboards):
            ...
            if friendly_type == "chart":
                url = get_url_path(
                    "Expertbridge.slice", slice_id=model.id, standalone="true"
                )
            else:
                url = get_url_path("Expertbridge.dashboard", dashboard_id_or_slug=model.id)
