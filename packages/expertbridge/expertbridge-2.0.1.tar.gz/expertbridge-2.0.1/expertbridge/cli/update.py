...
import expertbridge.utils.database as database_utils
from expertbridge.extensions import db
from expertbridge.utils.core import override_user
from expertbridge.utils.encrypt import SecretsMigrator
...
def sync_tags() -> None:
    ...
    # pylint: disable=import-outside-toplevel
    from expertbridge.common.tags import add_favorites, add_owners, add_types
...
def update_api_docs() -> None:
    ...
    expertbridge_dir = os.path.abspath(os.path.dirname(__file__))
    openapi_json = os.path.join(
        expertbridge_dir, "..", "..", "docs", "static", "resources", "openapi.json"
    )
    api_version = "v1"
