...
class AbstractEventLogger(ABC):
    ...
    def log_with_context(  # pylint: disable=too-many-locals
        ...
        **payload_override: Optional[Dict[str, Any]],
        ...
        from expertbridge.views.core import get_form_data
...
def get_event_logger_from_cfg_value(cfg_value: Any) -> AbstractEventLogger:
    ...
    if inspect.isclass(cfg_value):
        logging.warning(
            textwrap.dedent(
                """
                In expertbridge private config, EVENT_LOGGER has been assigned a class
                ...
                expertbridge.utils.log.AbstractEventLogger.
                """
            )
        )
    ...
    if not isinstance(result, AbstractEventLogger):
        raise TypeError(
            ...
            "of expertbridge.utils.log.AbstractEventLogger."
        )
...
class DBEventLogger(AbstractEventLogger):
    """Event logger that commits logs to Expertbridge DB"""
