...
# under the License.
"""Utility functions used across Expertbridge"""
...
from expertbridge.constants import (
    EXTRA_FORM_DATA_APPEND_KEYS,
    EXTRA_FORM_DATA_OVERRIDE_EXTRA_KEYS,
    EXTRA_FORM_DATA_OVERRIDE_REGULAR_MAPPINGS,
)
from expertbridge.errors import ErrorLevel, ExpertbridgeErrorType
...
from expertbridge.utils.database import get_example_database
from expertbridge.utils.dates import datetime_to_epoch, EPOCH
from expertbridge.utils.hashing import md5_sha_from_dict, md5_sha_from_str
...
if TYPE_CHECKING:
    from expertbridge.connectors.base.models import BaseColumn, BaseDatasource
...
class ReservedUrlParameters(str, Enum):
    """
    Reserved URL parameters that are used internally by Expertbridge. These will not be
...
def validate_json(obj: Union[bytes, bytearray, str]) -> None:
    if obj:
        try:
            ...
            raise ExpertbridgeException("JSON is not valid") from ex
...
class SigalrmTimeout:
    ...
    def handle_timeout(  # pylint: disable=unused-argument
        ...
        raise ExpertbridgeTimeoutException(
            error_type=ExpertbridgeErrorType.BACKEND_TIMEOUT_ERROR,
...
def notify_user_about_perm_udate(  # pylint: disable=too-many-arguments
    ...
    logger.info(msg)
    subject = __(
        "[Expertbridge] Access to the datasource %(name)s was granted",
...
def is_test() -> bool:
    return strtobool(os.environ.get("EXPERTBRIDGE_TESTENV", "false"))
