...
class DashboardSession(object):
    ...
    def process_project(self, project_name: str, reprocess: bool=False) -> Project:
        ...
        """
        if isinstance(project_name, list):
            # Pass function arguments to `process_projects()`.
            # Argument 0 is `self` so we'll pass arguments 1 and on
            func_args = list(locals().values())[1:]
            project = self.process_projects(*func_args)
    ...
    def process_projects(self, project_list, reprocess=False):
        ...
        """
        # Store function arguments in case we need to pass them to `process_project()`.
        # Argument 0 is `self` so we'll pass arguments 1 and on
        func_args = list(locals().values())[1:]

        if isinstance(project_list, (str, unicode, Project)):
            project = self.process_project(*func_args)

            return project
        elif isinstance(project_list, list):
            # If a list was passed, argument 0 is the project list
            # So we'll pass arguments 1 and on
            func_args = func_args[1:]

            if len(project_list) == 1:
                project_name = project_list[0]
                project = self.process_project(project_name, *func_args)
                ...
                for i, project_name in enumerate(project_list):
                    ...
                    print('*******************************************************')
                    self.process_project(project_name, *func_args)
        else:
            raise TypeError("`project_list` must be a string or Python list! Please reformat input and try again.")
    ...
    def export_project(self, project_name: str, server: str='dev', save_pickle: bool=True, *args) -> None:
        """
        ...
        Optional Args:
            These allow the user to export only certain parts of the project.
            
            <b>Options:</b>
            <br>
                <code>'excel'</code>: Exports dataframes to Excel dashboard template.
            <br>
                <code>'df'</code>: Export dataframe to the `Dataframes` folder of the project's
                [project_directory][ccrenew.dashboard.project.Project.project_directory],
                S3, and Bartertown PostgreSQL database.
            <br>
                <code>'summary'</code>: Exports monthly summary data to S3 and
                Bartertown PostgreSQL database.
            <br>
                <code>'snow'</code>: Exports [snow_data][ccrenew.dashboard.project.Project.snow_data] and
        ...
        if isinstance(project_name, list):
            ...
            # Argument 0 is `self` so we'll pass arguments 1 and on
            func_args = list(locals().values())[1:]
            project = self.export_projects(func_args)
        elif isinstance(project_name, (str, Project)):
            project = self.process_project(project_name)
            project.export(server, save_pickle, *args)
            print(f"{project.project_name} successfully exported.")
        else:
            raise TypeError("`project_name` must be a string or Project object! Please reformat input and try again.")

    
    def export_projects(self, project_list: str, server: str='dev', save_pickle: bool=True, *args) -> None:
        ...
        Optional Args:
            See [export_project()][ccrenew.dashboard.session.DashboardSession.export_project].
        """
        ...
        if isinstance(project_list, (str, unicode, Project)):
            project = self.export_project(*func_args)
        elif isinstance(project_list, list):
            # If a list was passed, argument 0 is the project list
            # So we'll pass arguments 1 and on
            func_args = func_args[1:]
