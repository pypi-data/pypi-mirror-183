...
class Bdb(object):
    ...
    def _set_break_module(self, module, filename, lineno, aug_id):
        ...
        if not status:
            ...
            for cobj in _GetModuleCodeObjects(module):
                if isinstance(cobj, types.GeneratorType):
                    inner_cobj = cobj.gi_code
                elif isinstance(cobj, tuple):
                    inner_cobj = None
                    for elem in cobj:
                        if isinstance(elem, types.CodeType):
                            inner_cobj = elem
                else:
                    inner_cobj = cobj.__code__
    ...
    def _set_code_object(self, func, code_object):
        if isinstance(func, types.GeneratorType):
            func.gi_code = code_object
        else:
            # TODO: handle tuples
            func.__code__ = code_object

    def _set_break_code_object(self, func, filename, lineno, aug_id):
        try:
            code_object = self._func_to_original_co_code[id(func)]
        except KeyError:
            code_object = self._get_code_object(func)
        ...
        if sys.version_info >= (3, 8, 0):
            ...
            self._set_code_object(func, new_code_object)
        else:
        ...
        self._cookies_to_func[cookie] = func
        if id(func) not in self._func_to_original_co_code:
            self._func_to_original_co_code[id(func)] = code_object
    ...
    def clear_break(self, aug_id):
        try:
            ...
            func = self._cookies_to_func[cookie]
