...
_BFS_IGNORE_TYPES = (types.ModuleType, type(None), bool, float, six.binary_type,
                     ...
                     types.BuiltinMethodType) + six.integer_types
...
def GetCodeObjectAtLine(module, line):
  ...
  for code_object in _GetModuleCodeObjects(module):
    if isinstance(code_object, types.GeneratorType):
      lines = sorted(list(_GetLineNumbers(code_object.gi_code)))
    elif isinstance(code_object, types.FrameType):
      continue
    elif isinstance(code_object, tuple):
      lines = []
      for elem in code_object:
        if isinstance(elem, types.CodeType):
          lines = sorted(list(_GetLineNumbers(elem)))
    else:
      lines = sorted(list(_GetLineNumbers(code_object.__code__)))
...
def _GetModuleCodeObjects(module):
  ...
  while current:
    ...
    new_current = []
    for code_object in current:
      if isinstance(code_object, tuple):
        continue
      if isinstance(code_object, types.FrameType):
        continue
      elif isinstance(code_object, types.GeneratorType):
        new_current.append(code_object.gi_code.co_consts)
      else:
        new_current.append(code_object.__code__.co_consts)
    ...
    for current_obj in current:
      ...
      for obj in referents:
        try:
          ...
          if isinstance(obj, types.CodeType) and not isinstance(current_obj, dict):
            code_objects.add(current_obj)
          else:
