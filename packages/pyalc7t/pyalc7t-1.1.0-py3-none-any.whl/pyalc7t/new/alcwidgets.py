...
class myDoubleValidator(QtGui.QDoubleValidator):
   ...
   def validate(self, inputstring, pos):
      if not inputstring or inputstring == ".":
         return (QtGui.QValidator.Intermediate,inputstring,0)
      try:
         wert=float(inputstring)
      except ValueError:
         return (QtGui.QValidator.Invalid,inputstring,0)
      if wert < self.bottom or wert > self.top:
         return (QtGui.QValidator.Invalid,inputstring,0)
      return (QtGui.QValidator.Acceptable,inputstring,0)
#
...
#
      ...
      self.validator_Nennkapazitaet= myDoubleValidator(0.0,99.9,2)
      self.lineEdit_Nennkapazitaet.setValidator(self.validator_Nennkapazitaet)
      ...
      if self.kanalnr == KANAL1 or self.kanalnr == KANAL2:
         self.validator_Ladestrom=myDoubleValidator(0.01,3.5,2)
      else:
         self.validator_Ladestrom=myDoubleValidator(0.01,1.0,2)
      self.lineEdit_Ladestrom.setValidator(self.validator_Ladestrom)
      self.gridLayout2.addWidget(self.lineEdit_Ladestrom,5,1)

      self.lineEdit_Entladestrom = QtWidgets.QLineEdit()
      if self.kanalnr == KANAL1 or self.kanalnr == KANAL2:
         self.validator_Entladestrom=myDoubleValidator(0.01,3.5,2)
      else:
         self.validator_Entladestrom=myDoubleValidator(0.01,1.0,2)
      self.lineEdit_Entladestrom.setValidator(self.validator_Entladestrom)
...
##
      self.UNenn=dict_akku_spannung[self.config['AKTyp']]* self.config['AnzZellen']
#     Max Anzahl Zellen: maximale Nominalspannung 12V
...
#
   def do_neu(self):
      print("do neu")
      self.config= { 'AKTyp' :AKKU_TYP_NICD_NIMH, 'AnzZellen' : 1, 'CNenn' :  0.01, 'ILad' : 0.01, 'IEntl': 0.01, 'Beschreibung': "" }
      self.UNenn= dict_akku_spannung[self.config['AKTyp']]* self.config['AnzZellen']
      self.geprueft= False
      self.update_gui()
#
