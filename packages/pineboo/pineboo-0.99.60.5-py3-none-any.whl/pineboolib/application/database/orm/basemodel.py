...
class BaseModel(object):
    ...
    def _flush(
        self, relations: List[str] = [], only: List[str] = [], ignore_foreign_key: bool = False
    ) -> None:
        """Flush data."""
        ...
        if self._session is None:
            ...
            if not only or "check_integrity" in only:
                self._check_integrity(ignore_foreign_key=ignore_foreign_key)
    ...
    def _check_integrity(self, ignore_foreign_key: bool = False) -> bool:
