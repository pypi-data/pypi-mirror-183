...
class BaseModel(object):
    ...
    def _flush(
        ...
        if self._session is None:
            ...
            if not only or "flush" in only:
                for relation in relations:
                    list_objects = getattr(self, relation, [])
                    print("OJO4 %s" % list_objects)

                    for list_object in list_objects:
                        LOGGER.info("Lanzando _flush referenciado a %s" % list_object)
                        list_object._flush(
                ...
                try:
                    ...
                    for relation in relations:
                        list_objects = getattr(self, relation, [])
                        print("OJO5 %s" % list_objects)
                        for list_object in list_objects:
                            LOGGER.info("Añadiendo a _flush instancia a %s" % list_object)
    ...
    def _before_flush(self) -> None:
        ...
        try:
            ...
            print("OJO 1.1", getattr(self, "lineas", None))
            ...
            print("OJO 1.2", getattr(self, "lineas", None))
        except Exception as error:
            self._error_manager("_before_flush", error)

        try:
            print("OJO 1.3", getattr(self, "lineas", None))
            self.before_flush()
        ...
        if mode == 0:  # insert
            try:
                print("OJO 1.4", getattr(self, "lineas", None))
                self.before_new()
            ...
            try:
                print("OJO 1.5", getattr(self, "lineas", None))
                self.before_change()
                self._validate_cursor()
