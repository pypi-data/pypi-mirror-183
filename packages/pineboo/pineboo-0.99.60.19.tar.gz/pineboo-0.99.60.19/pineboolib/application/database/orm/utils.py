...
def do_flush(session: "Session", model_obj: Any) -> None:
    ...
    LOGGER.info(
        "BEFORE_DO_FLUSH de %s en session %s. NEW: %s, DIRTY: %s, LINEAS: %s"
        % (model_obj, session, session.new, session.dirty, getattr(model_obj, "lineas", []))
    )
    session.flush(model_obj if isinstance(model_obj, list) else [model_obj])
    LOGGER.info(
        "AFTER_DO_FLUSH de %s en session %s. NEW: %s, DIRTY: %s, LINEAS: %s"
        % (model_obj, session, session.new, session.dirty, getattr(model_obj, "lineas", []))
    )
