...
class Yolov7:
    ...
    def __init__(self, classes_path: str, VOCdevkit_path: str = os.path.join('yolovxx', 'yolov7pytorch', 'VOCdevkit'),
                 anchors_path: str = os.path.join('yolovxx', 'yolov7pytorch', 'model_data', 'yolo_anchors.txt'),
                 phi: str = "l", input_shape: list = [640, 640], anchors_mask: list = [[6, 7, 8], [3, 4, 5], [0, 1, 2]]) -> None:
    ...
    def train(self, cuda: bool = True, distributed: bool = False, sync_bn: bool = False,
              fp16: bool = False, pretrained: bool = False, model_path: str = "", save_dir: str = 'logs',
              save_period: int = 10, eval_flag: bool= True, eval_period: int = 10, num_workers: int = 4,
              epoch: int = 300, optimizer_type: str = 'sgd', batch_size: int = 4, freeze_train: bool = True) -> None:
        ...
        #   创建yolo模型
        #------------------------------------------------------#
        model = YoloBody(anchors_mask, num_classes, self.phi, pretrained=pretrained)
        if not pretrained:
            weights_init(model)
        if model_path != '':
            #------------------------------------------------------#
        ...
        #   获得损失函数
        #----------------------#
        yolo_loss    = YOLOLoss(anchors, num_classes, self.input_shape, anchors_mask, label_smoothing)
        #----------------------#
        #   记录Loss
        #----------------------#
        if local_rank == 0:
        ...
        if local_rank == 0:
            show_config(
                classes_path = self.classes_path, anchors_path = self.anchors_path, anchors_mask = anchors_mask, model_path = model_path, input_shape = self.input_shape, \
    ...
    def predict(self, cuda: bool = True, model_path: str = "", mode: str = "predict",
                ...
                count: bool = False,  test_interval: int = 100, fps_image_path: str = "",
                simplify: bool= True, onnx_save_path: str = "", confidence: float = 0.5,
                nms_iou: float = 0.3, letterbox_image: bool = True):
        

        yolo = YOLO(model_path=model_path, classes_path=self.classes_path, anchors_path=self.anchors_path,
        ...
        if mode == "predict":
            ...
            try:
                ...
                r_image, detection_result = yolo.detect_image(image, crop = crop, count=count)
                r_image.show()
    ...
    def compute_map(self, model_path: str = '', cuda: bool = True, map_mode: int = 0, min_overlap: float = 0.5, confidence: float = 0.001,
        ...
        image_ids = open(os.path.join(self.VOCdevkit_path, "VOC2007/ImageSets/Main/test.txt")).read().strip().split()
        ...
        if map_mode == 0 or map_mode == 3:
            print("Get map.")
            get_map(min_overlap, True, score_threhold = score_threhold, path = map_out_path)
            print("Get map done.")
...
def main():
    yolo = Yolov7(classes_path=os.path.join('yolovxx', 'yolov7pytorch', 'model_data', 'voc_classes.txt'))
    yolo.voc_annotation()
    yolo.train()
