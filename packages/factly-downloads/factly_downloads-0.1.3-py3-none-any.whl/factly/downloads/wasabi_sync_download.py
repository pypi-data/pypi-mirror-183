...
def get_access_keys():
    ...
    try:
        ...
        if "File" in str(e):
            print("please configure wasabi_keys.cfg with 'key' and 'secret' key words")
...
def sync_files(project_bucket_path, remote_directory_name, local_file_list, local_path):
    ...
    if len(list(project_bucket_path.objects.filter(Prefix=remote_directory_name))) != 0:
        for obj in project_bucket_path.objects.filter(Prefix=remote_directory_name):
            ...
            if str(obj.key).split(f"{remote_directory_name}")[-1] in local_file_list:
                logging.info(" - File exist in local")
            else:
                logging.info(" - File does not exist in local")
                output_file = get_directory(obj.key, local_path, remote_directory_name)
...
def wasabi_sync_download(bucket_name, wasabi_path, local_path):
    ...
    s3 = session.resource("s3", endpoint_url="https://s3.eu-central-1.wasabisys.com")
