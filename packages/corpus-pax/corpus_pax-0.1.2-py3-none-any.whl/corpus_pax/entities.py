...
class PracticeArea(Persons, TableConfig):
    __tablename__ = "areas"
...
class PersonCategory(Persons, TableConfig):
...
class Individual(Persons, Profile, IndividualBio, TableConfig):
    ...
    def list_members_repo(cls):
        return gh.fetch_contents("members")
    ...
    def set_membership_rows(self, c: Connection) -> Table | None:
        ...
        if self.members:
            for member in self.members:
                ...
                if email and (_e := EmailStr(email)):
                    obj = OrgMember(
                        org_id=self.id,
                        individual_id=None,
...
class OrgMember(Persons, TableConfig):
    ...
    def on_insert_add_member_id(cls, c: Connection) -> sqlite3.Cursor:
        """Since the original data doesn't contain the `member id` yet, we need to setup up trigger. The trigger will ensure that, on insert of a `OrgMember` row, the email address contained in the row can be used to fetch the member id and include it in the `OrgMember` row just inserted."""
