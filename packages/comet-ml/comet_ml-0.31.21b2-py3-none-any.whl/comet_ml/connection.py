...
import comet_ml
import comet_ml.json_encoder as json
...
def get_backend_address(comet_config=None):
    # type: (Optional[Config]) -> str
    if comet_config is None:
        comet_config = get_config()

    return sanitize_url(comet_config["comet.url_override"])


def get_comet_root_url(comet_config):
    # type: (Config) -> str

    return get_root_url(
        sanitize_url(comet_config.get_string(None, "comet.url_override"))
    )
...
def get_optimizer_address(comet_config):
    # type: (Config) -> str
    optimizer_url = comet_config.get_string(None, "comet.optimizer_url")

    if optimizer_url is None:
        return url_join(get_comet_root_url(comet_config), "optimizer/")
    else:
...
def format_messages_for_ws(messages):
    ...
    data = json.dumps(messages_arr, ignore_nan=False, allow_nan=False)
