...
class Streamer(BaseStreamer):
    ...
    def _process_rest_api_send(
        ...
        try:
            ...
            LOGGER.info("Failed 1 to process REST API request, reason: %r", exc)
        except (ConnectionError, RequestException) as ce:
            LOGGER.info("Failed 2 to process REST API request, reason: %r", ce)
            LOGGER.exception("Failed 2 to process REST API request, reason: %r", ce)
            self._has_server_connection = False
        except Exception:
...
class OfflineStreamer(BaseStreamer):
    ...
    def __init__(
        ...
        self.file = open(
            os.path.join(self.tmp_dir, OFFLINE_EXPERIMENT_MESSAGES_JSON_FILE_NAME),
            mode="wt",
            encoding="utf-8",
        )
