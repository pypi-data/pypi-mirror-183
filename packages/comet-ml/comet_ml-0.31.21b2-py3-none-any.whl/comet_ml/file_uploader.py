...
def preprocess_asset_folder(
    ...
    for asset_folder, subs, files in os.walk(dispatched.user_input):
        for file_name in files:
            full_asset_path = os.path.join(asset_folder, file_name)
            if not os.path.isfile(full_asset_path):
                continue

            dispatched = dispatch_user_file_upload(full_asset_path)
            if logical_path:
                full_asset_path = os.path.join(logical_path, full_asset_path)
            asset_file = preprocess_asset_file(
                dispatched=dispatched,
                upload_type=upload_type,
                file_name=full_asset_path,
                metadata=metadata,
                overwrite=overwrite,
