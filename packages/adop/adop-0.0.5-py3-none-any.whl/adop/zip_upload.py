...
def upload(
    file: str, config: str, cwd: str, remote: list[str], install: str, deploy: bool
):
    """
    ...
    def gen(self, install_data, remote_data, requires_data: dict, deploy: bool):
        ...
        for root, shasum in requires_data.items():
            ...
            for remote_data_item in remote_data:
                if uploaders.api_check_is_uploaded(root, shasum, tag, remote_data_item):
                    yield f"{root} already uploaded to {remote_data_item['url']}"
                else:
                    yield from uploaders.api_upload(
                        cache_file, root, remote_data_item, headers, deploy
                    )
