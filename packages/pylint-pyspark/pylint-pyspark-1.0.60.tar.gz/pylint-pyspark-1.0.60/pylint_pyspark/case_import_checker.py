...
class CaseImportChecker(BaseChecker):
    ...
    def _check_import(self, node: nodes.Import):
        if node.names[0][0] == "pyspark.sql.function" and node.names[0][1] == 'f':
            self.add_message(
                "lower-function-case", node=node
            )
        if node.names[0][0] == "pyspark.sql.types" and node.names[0][1] == 't':
            self.add_message(
                "lower-type-case", node=node
            )

    def visit_import(self, node):
        self._check_import_stack.append(node)
