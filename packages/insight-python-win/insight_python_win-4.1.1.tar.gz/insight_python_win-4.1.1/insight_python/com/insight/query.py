...
# 查询指标排行榜数据
def get_billboard(type, market, trading_day=None):
    '''
    ...
    :param trading_day: 开始日期 结束日期 datetime类型 列表类型 不填默认返回当天数据
    :return: pandas.DataFrame
    '''
    ...
    result = get_billboard_handle(type, market, trading_day)
    return result
...
# 可转债转股结果
def get_con_bond_2_shares(htsc_code=None, pub_date=None, exer_start_date=None, exer_end_date=None, convert_code=None, exchange=None):
    '''
    :param htsc_code: 华泰证券ID
    :param pub_date: 信息发布日期范围
    :param exer_start_date: 行权起始日范围
    ...
    if exer_start_date:
        if isinstance(exer_start_date, list):
            if len(exer_start_date) > 2:
                return "end_date format is not [start_date, end_date]"
            if len(exer_start_date) == 1:
                exer_start_date = [exer_start_date[0], exer_start_date[0]]
            if not isinstance(exer_start_date[0], datetime) or not isinstance(exer_start_date[1], datetime):
                return "start_date,end_date format is not datetime"
        else:
            return "exer_start_date format is not list"
    ...
    result = get_con_bond_2_shares_handle(htsc_code, pub_date, exer_start_date, exer_end_date, convert_code, exchange)
    return result


# 利润表
def get_income_statement(htsc_code=None, date=None, period=None):
    '''
    :param htsc_code: 华泰证券ID
    :param date: 限定时间范围
    ...
    if date:
        if isinstance(date, list):
            ...
            if len(date) == 1:
                date = [date[0], date[0]]
            if not isinstance(date[0], datetime) or not isinstance(date[1], datetime):
                return "start_date,end_date format is not datetime"
        else:
            return "date format is not list"

    result = get_income_statement_handle(htsc_code, date, period)
    return result


# 资产负债表
def get_balance_sheet(htsc_code=None, date=None, period=None):
    '''
    :param htsc_code: 华泰证券ID
    :param date: 限定时间范围
    :param period: 报表类型，Q1，Q2，Q3，Q4
    ...
    result = get_balance_sheet_handle(htsc_code, date, period)
    return result


# 现金流量表
def get_cashflow_statement(htsc_code=None, date=None, period=None):
    '''
    :param htsc_code: 华泰证券ID
    :param date: 限定时间范围
    :param period: 报表类型，Q1，Q2，Q3，Q4
    ...
    result = get_cashflow_statement_handle(htsc_code, date, period)
    return result


# 财务指标
def get_fin_indicator(htsc_code=None, date=None, period=None):
    '''
    :param htsc_code: 华泰证券ID
    :param date: 限定时间范围
    :param period: 报表类型，Q1，Q2，Q3，Q4
    ...
    result = get_fin_indicator_handle(htsc_code, date, period)
    return result
...
def get_additional_share(htsc_code=None, listing_date=None):
    ...
    if listing_date:
        ...
        listing_date = [datetime.today(), datetime.today()]
...
# 复权因子
def get_adj_factor(htsc_code=None, trading_day=None):
    '''
    :param htsc_code: 华泰证券ID
    :param trading_day: 时间范围
    :return: pandas.DataFrame
    ...
    result = get_adj_factor_handle(htsc_code, trading_day)
    return result
