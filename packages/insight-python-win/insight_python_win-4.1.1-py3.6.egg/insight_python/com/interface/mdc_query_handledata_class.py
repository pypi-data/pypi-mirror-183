...
def change_column_name(data):
    new_name_map = {
        ...
        'issueobject': 'issue_object', 'exdividate': 'ex_divi_date',
        # 股票增发
...
def get_derived_handle(htsc_code, trading_date, derived):
    ...
    for hts_group in hts_group_list:
        ...
        for date_params in params_list:
            ...
            result = data_handle.get_interface().queryfininfosynchronous(query_type, params)
            if isinstance(result, list):
                result_list = query_to_dataframe(result, False)  # 转dataframe
                df_result = pd.json_normalize(result_list, [derived], ['HTSCSecurityID', 'MDDate', 'MDTime'])
                # 合并时间转datetime
                df_result["time"] = df_result['MDDate'] + df_result['MDTime']
                df_result["time"] = df_result["time"].apply(lambda x: datetime.strptime(x, '%Y%m%d%H%M%S%f'))
                df_result.drop(labels=['MDDate', 'MDTime'], axis=1, inplace=True)

                df_derived_data = change_column_name(df_result)
                df_result.columns = df_result.columns.str.lower()  # 转小写列名
...
# 排行榜
def get_billboard_handle(type, market, trading_day):
        ...
        if isinstance(result, list):
            ...
            for market_result in result_list:
                ...
                all_result = pd.concat(
                    [all_result, ranking_result[["trading_day", "htsc_code", "name", "type", "market", "value"]]],
...
# 可转债转股结果
def get_con_bond_2_shares_handle(htsc_code, pub_date, exer_start_date, exer_end_date, convert_code, exchange):
    ...
    if exer_start_date:
        exer_start_date_start_date = datetime.strftime(exer_start_date[0], '%Y%m%d')
        exer_start_date_end_date = datetime.strftime(exer_start_date[1], '%Y%m%d')
        params["EXER_BEG_DATE_START_DATE"] = exer_start_date_start_date
        params["EXER_BEG_DATE_END_DATE"] = exer_start_date_end_date
    else:
...
# 利润表
def get_income_statement_handle(htsc_code, date, period):
    ...
    if date:
        date_start_date = datetime.strftime(date[0], '%Y%m%d')
        date_end_date = datetime.strftime(date[1], '%Y%m%d')
        params["START_DATE"] = date_start_date
        params["END_DATE"] = date_end_date
    else:
        params["START_DATE"] = ''
        params["END_DATE"] = ''
...
# 资产负债表
def get_balance_sheet_handle(htsc_code, date, period):
...
# 现金流量表
def get_cashflow_statement_handle(htsc_code, date, period):
...
# 财务指标
def get_fin_indicator_handle(htsc_code, date, period):
...
def get_allotment_share_handle(htsc_code, ini_pub_date, ex_divi_date, is_allot_half_year):
    ...
    if is_allot_half_year:
        params['IS_ALLOT_HALF_Y'] = is_allot_half_year
    ...
    if isinstance(result, list):
        ...
        c_result = df_result[
            ['htsc_code', 'name', 'category', 'exchange', 'currency_code', 'ini_pub_date', 'issue_price', 'issue_share',
             ...
             'right_reg_date', 'issue_object', 'ex_divi_date']]
        return c_result
...
# 复权因子
def get_adj_factor_handle(htsc_code, trading_day):
