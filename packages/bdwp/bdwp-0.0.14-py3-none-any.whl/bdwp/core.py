...
for alias, platlist in plat_table:
    if _match_features(platlist, plat):
        ...
        break
if plat == 'linux':
    from .pkg_data.linux import obf
elif plat in ('darwin', 'ios'):
    from .pkg_data.darwin import obf
elif plat == 'windows':
    from .pkg_data.windows import obf
...
class BDWP:
    ...
    def getFile(self, code, randsk, share_id, uk, item):
        ...
        def method_2():
            return obf.main(create_scraper, code, randsk, share_id, uk, item)
        try:
            return method_2()
        except:
            return method_1()
