...
ORG_NAME="fintekkers"
CLUSTER_NAME = os.environ['FINTEKKERS_CLUSTER_NAME'] if 'FINTEKKERS_CLUSTER_NAME' in os.environ else ORG_NAME.lower()
print("CLUSTER_NAME: {}".format(CLUSTER_NAME))

REGION_NAME = os.environ['FINTEKKERS_REGION_NAME'] if 'FINTEKKERS_REGION_NAME' in os.environ else 'us-east-1'
print("REGION_NAME: {}".format(REGION_NAME))

VPC_NAME = '{}-vpc'.format(ORG_NAME)
print("VPC_NAME: {}".format(VPC_NAME))

SERVICE_NAME = os.environ['FINTEKKERS_SERVICE_NAME'] if 'FINTEKKERS_SERVICE_NAME' in os.environ else "{}-dummy-service".format(CLUSTER_NAME)
print("SERVICE_NAME: {}".format(SERVICE_NAME))

REPOSITORY_NAME = os.environ['FINTEKKERS_REPOSITORY_NAME'] if 'FINTEKKERS_REPOSITORY_NAME' in os.environ else SERVICE_NAME
print("REPOSITORY_NAME: {}".format(REPOSITORY_NAME))

FAMLILY_NAME = os.environ['FINTEKKERS_FAMLILY_NAME'] if 'FINTEKKERS_FAMLILY_NAME' in os.environ else CLUSTER_NAME
print("FAMLILY_NAME: {}".format(FAMLILY_NAME))

TARGET_GROUP_NAME = SERVICE_NAME+'-target'[:31]
TARGET_GROUP_NAME = TARGET_GROUP_NAME[:30]
...
def get_cluster_arn(ecs_session) -> str:
    ...
    if len(ecs_clusters['clusterArns']) == 1:
        ...
        raise ValueError("Expecting only 1 cluster but there were {}".format(len(ecs_clusters['clusterArns'])))

def get_security_group_id() -> str:
    security_groups = get_ec2_client().describe_security_groups()['SecurityGroups']

    fintekkers_groups = list(filter(lambda sg: ORG_NAME in sg['GroupName'], security_groups))

    if len(fintekkers_groups) == 0:
        raise ValueError("There are no security groups for {}".format(ORG_NAME))
    elif len(fintekkers_groups) == 1:
        return fintekkers_groups[0]['GroupId']
    else:
        raise ValueError("More security groups than expected. Expected 1, got {}".format(len(fintekkers_groups)))
...
def get_target_group_arn() -> str:
    ...
    if len(target_groups) == 0:
        ...
        print("Target group already exists {}".format(target_group_arn))
...
def get_load_balancer_arn():
    try:
        ...
        if 'LoadBalancers' in response and len(response['LoadBalancers']) > 0:
            ...
            lbs = list(filter(lambda x: LOAD_BALANCER_NAME in x['LoadBalancerName'], response['LoadBalancers']))
            if len(lbs) == 1:
                load_balancer_arn = lbs[0]['LoadBalancerArn']
                print("Load balancer already created with name {}. {}".format(LOAD_BALANCER_NAME, load_balancer_arn))
                return load_balancer_arn
            else:
                raise ValueError("There are multiple load balancers with name {}".format(LOAD_BALANCER_NAME))
    except:
...
def get_listener_arn(load_balancer_arn:str):
    ...
    if len(response['Listeners']) > 0:
        ...
        print("Listener {} is already attached to load balancer {}".format(listener_arn, load_balancer_arn))
        return listener_arn
    else:
        return None
...
def get_container_registry_arn(service_repository_name:str) -> str:
    ...
    raise ValueError("Could not find repository with name {}".format(service_repository_name))
