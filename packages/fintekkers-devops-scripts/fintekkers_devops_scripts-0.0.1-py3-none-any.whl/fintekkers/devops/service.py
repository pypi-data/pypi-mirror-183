...
from fintekkers.devops.docker_image import *

# SERVICE_AND_TASK_NAME = get_image_tag_name()
DEFAULT_PORT= 5001 if 'FINTEKKERS_DEFAULT_PORT' not in os.environ else int(os.environ['FINTEKKERS_DEFAULT_PORT'])
print("DOCKER_FILE: {}".format(DOCKER_FILE))

TASK_COUNT = 1
print("TASK_COUNT: {}".format(TASK_COUNT))

PROTOCOL = 'HTTP' if 'FINTEKKERS_PROTOCOL' not in os.environ else os.environ['FINTEKKERS_PROTOCOL']
print("PROTOCOL: {}".format(PROTOCOL))

IS_GPRC = False if 'FINTEKKERS_IS_GRPC' not in os.environ else ast.literal_eval(os.environ['FINTEKKERS_IS_GRPC'])
...
def create_task_definition():
    ...
    print("Revision {} created of {}".format(response['taskDefinition']['revision'], FAMLILY_NAME))
...
def create_target_group():
    ...
    if target_group_arn is not None:
        ...
        print("Creating target group called {} in VPC: {}".format(TARGET_GROUP_NAME, vpc_id))

        print("Target group will receive traffic via {} on port {}. The health check "
              "will be checked against the path {} via {} on port {}".format(
                    protocol_version, DEFAULT_PORT, healthcheck_path, 'HTTP', healthcheck_path))
...
def get_cert_arn():
    ...
    raise ValueError("Could not find certificate for {}. There were {} certs".format(DOMAIN_NAME, len(certs)))

def attach_load_balancer_and_target_group(target_group_arn:str, load_balancer_arn:str):
...
def await_load_balancer(load_balancer_arn):
    ...
    print("Will call {} repeatedly for {}+ seconds to ensure service was up and running the whole time. Will only print errors" \
          .format(url, sleep_time*tries))
    ...
    for i in range(tries):
        ...
        if lb['State']['Code'] != 'active':
            print("Load balancer is not in an active state yet ({})".format(lb['State']['Code']))
            break
        ...
        try:
            ...
            print("Failure connecting to load balancer, may not be running yet {}".format(request_url))




def update_service(task_defintion_arn:str, subnets:list):
    print("Updating {} service on {} cluster, with task definition arn {}".format(CLUSTER_NAME, SERVICE_NAME, task_defintion_arn))
...
def create_service(task_defintion_arn:str, subnets:list, target_group_arn:str):
    ...
    print("Utilizing cluster '{}' to create service {}".format(CLUSTER_NAME, SERVICE_NAME))
    ...
    if len(response['services']) >= 2:
        raise ValueError("There are multiple services running. The DevOps scripts needs to be extended to support this. Erroring out")
    ...
    if len(response['services']) >= 1 and response['services'][0]['status'] == 'ACTIVE':
        if response['services'][0]['runningCount'] < response['services'][0]['desiredCount']:
            print("There are not enough tasks running in the service, it might be in a bad state. Will continue to update anyway.")

        print("Service {} already exists. Arn for service {}".format(SERVICE_NAME, response['services'][0]['serviceArn']))
...
def check_task_count(ecs_session, families):
    ...
    if len(tasks['taskArns']) < TASK_COUNT:
        print("It seems that there aren't enough tasks running for {}; {}".format(CLUSTER_NAME, SERVICE_NAME))
    ...
    if service['status'] != 'ACTIVE':
        raise ValueError("Service {} is not in an ACTIVE state {}".format(SERVICE_NAME, service['status']))
    ...
    if service['runningCount'] < TASK_COUNT:
        raise ValueError("Service is running {} tasks, but desired count is {}. There could be an issue" \
                         .format(service['runningCount'], TASK_COUNT))


def delete_listener(load_balancer_arn):
    listner_arn = get_listener_arn(load_balancer_arn)

    if listner_arn is None:
        print("There is no listener to delete")
        return

    get_elb_client().delete_listener(
...
def delete_load_balancer(load_balancer_arn):
    ...
    get_elb_client().delete_load_balancer(
        LoadBalancerArn=load_balancer_arn
    )
...
def delete_target_group():
    ...
    get_elb_client().delete_target_group(
        TargetGroupArn=target_group_arn
    )
...
def full_build(create:bool=False, teardown:bool=True):
    ecs_session = get_ecs_client()
    ...
    if create and is_any_clusters(ecs_session):
        ...
        # time.sleep(60)
        listener_arn = attach_load_balancer_and_target_group(target_group_arn, load_balancer_arn)
