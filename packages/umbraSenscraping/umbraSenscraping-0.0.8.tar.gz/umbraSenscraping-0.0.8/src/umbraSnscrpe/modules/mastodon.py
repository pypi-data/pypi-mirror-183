...
class _MastodonCommonScraper(umbraSnscrpe.base.Scraper):
	...
	def _entries_to_items(self, entries, url):
		for entry in entries:
			...
			userKwargs['avatarUrl'] = urllib.parse.urljoin(url, userLink.find('img', class_ = 'u-photo')['umbraSnscrpe'])
	...
	def _display_name(self, strong, url):
		...
		for child in strong.children:
			if isinstance(child, bs4.element.NavigableString):
				...
				hasCustomEmoji = True
				outPlain.append(child['alt'])
				outFull.append(CustomEmoji(shortName = child['alt'], url = urllib.parse.urljoin(url, child['umbraSnscrpe'])))
			else:
