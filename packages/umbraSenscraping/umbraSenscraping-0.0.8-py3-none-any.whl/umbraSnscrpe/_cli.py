...
def _dump_stack_and_locals(trace, exc = None):
	with tempfile.NamedTemporaryFile('w', prefix = 'umbra_scrape_sn_locals_', delete = False) as fp:
		if exc is not None:
			fp.write('Exception:\n')
		...
		for frameRecord in trace:
			...
			if not module.__name__.startswith('src.') and module.__name__ != 'src':
				continue
...
class CitationAction(argparse.Action):
	...
	def __call__(self, parser, namespace, values, optionString):
		try:
			m = importlib.metadata.metadata('src')
		except importlib.metadata.PackageNotFoundError:
			print('Error: could not find src installation. --citation does not work without the package being installed.', file = sys.stderr)
			parser.exit(1)
...
def parse_args():
	...
	parser.add_argument('--version', action = 'version', version = f'src {src.version.__version__}')
	...
	classes = src.base.Scraper.__subclasses__()
	scrapers = {}
	for cls in classes:
