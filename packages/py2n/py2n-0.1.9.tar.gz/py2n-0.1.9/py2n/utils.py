...
async def get_info(
    ...
    try:
        async with aiohttp_session.get(
            f"http://{options.host}{API_SYSTEM_INFO}",
        ...
        _LOGGER.debug("host %s: error: %r", options.host, error)
...
async def get_status(
    ...
    try:
        async with aiohttp_session.get(
            f"http://{options.host}{API_SYSTEM_STATUS}",
            timeout=HTTP_CALL_TIMEOUT,
            auth=options.auth,
        ) as response:
...
async def restart(
    ...
    try:
        async with aiohttp_session.get(
            f"http://{options.host}{API_SYSTEM_RESTART}",
            timeout=HTTP_CALL_TIMEOUT,
            auth=options.auth,
        ) as response:
            if response.status == 401:
...
async def get_switches(
    ...
    try:
        async with aiohttp_session.get(
            f"http://{options.host}{API_SWITCH_STATUS}",
            timeout=HTTP_CALL_TIMEOUT,
            auth=options.auth,
        ) as response:
            if response.status == 401:
...
async def test_audio(
    ...
    try:
        async with aiohttp_session.get(
            f"http://{options.host}{API_AUDIO_TEST}",
            timeout=HTTP_CALL_TIMEOUT,
            auth=options.auth,
        ) as response:
            if response.status == 401:
