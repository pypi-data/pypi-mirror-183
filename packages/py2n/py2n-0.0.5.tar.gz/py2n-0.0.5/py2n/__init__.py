...
class Py2NDevice:
    ...
    async def update(self) -> Py2NDeviceData:
        ...
        try:
            ...
            self.data = Py2NDeviceData()
            self.data.name = self._info["deviceName"]
            self.data.model = self._info["variant"]
            self.data.serial = self._info["serialNumber"]
            self.data.firmware = self._info["swVersion"]
            self.data.uptime = datetime.now() - timedelta(seconds=self._status["upTime"])
