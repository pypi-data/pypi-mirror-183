...
class DatabaseLock(DistributedLock):
    ...
    def can_execute_task(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Process: {os.getpid()} {self.lock_owner} is not lock owner, but still trying "
    ...
    def _do_acquire_lock(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"lock acquired by {self.lock_owner}, "
                         f"fencing_token_id: {sl.fencing_token_id}")
            return True
        except IntegrityError as e:
            logger.debug(f"Error when {self.lock_owner} tried to acquire the lock, lock already "
    ...
    def try_acquire_lock(self) -> bool:
        ...
        try:
            ...
            if record.valid_until < LockUtils.get_epochtime_in_millis():
                ...
                logger.debug(f'valid lock held by, OWNER: {record.owner}, EXPIRY: {record.valid_until}')
                return False
        except Exception as e:
    ...
    def _do_renew_lease(self) -> bool:
        ...
        try:
            ...
            logger.error(f'Trying to renew a lock which does not exist {os.getpid()}, Error: {e}')
            return False
