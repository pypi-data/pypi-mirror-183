...
class Message(Object, Update):
    ...
    async def _parse(
        ...
        from_id = ramutils.get_raw_peer_id(message.from_id)
        peer_id = ramutils.get_raw_peer_id(message.peer_id)
        ...
        if isinstance(message, raw.types.MessageService):
            ...
            if isinstance(action, raw.types.MessageActionChatAddUser):
                ...
                new_chat_members = [types.User._parse(client, users[ramutils.get_raw_peer_id(message.from_id)])]
                service_type = enums.MessageServiceType.NEW_CHAT_MEMBERS
            ...
            parsed_message = Message(
                id=message.id,
                date=ramutils.timestamp_to_datetime(message.date),
                ...
                migrate_to_chat_id=ramutils.get_channel_id(migrate_to_chat_id) if migrate_to_chat_id else None,
        ...
        if isinstance(message, raw.types.Message):
            ...
            if forward_header:
                forward_date = ramutils.timestamp_to_datetime(forward_header.date)

                if forward_header.from_id:
                    raw_peer_id = ramutils.get_raw_peer_id(forward_header.from_id)
                    peer_id = ramutils.get_peer_id(forward_header.from_id)
            ...
            parsed_message = Message(
                ...
                media=media_type,
                edit_date=ramutils.timestamp_to_datetime(message.edit_date),
    ...
    def link(self) -> str:
        if (
            ...
            return f"https://t.me/c/{ramutils.get_channel_id(self.chat.id)}/{self.id}"
