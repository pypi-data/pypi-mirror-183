...
def difference_datetimezone7_by_seconds_from_between(_dtz_first, _dtz_last):
    ...
    print('- - - result check difference_datetimezone7_by_seconds_from_between :',diff)
    return diff
...
def maxdatetime_lstdict(_lst_dict=[]):
    ...
    try:
        if len(_lst_dict)!=0:
            _lst_t = []
            for t in _lst_dict:
                print('maxdatetime_lstdict --> append',datetime.strptime(t,'%Y-%m-%dT%H:%M:%S.%f%z'))
                _lst_t.append(datetime.strptime(t,'%Y-%m-%dT%H:%M:%S.%f%z'))
            mxtime = max(_lst_t)
    except:
        mxtime = None
    print('maxdatetime_lstdict --> result', mxtime)
    return mxtime