...
class sanbox():
    ...
    def getValue_kwargs_variables(self, kwargs, _nameOfKwargs, _typeOfKwargs, _defaultValue = None):
    ...
    def getLst_log_pod_by_pattern_andTarget(self, _sincelast, _pod, _namespace, _pattern, _lst_target,**kwargs):
        lst_oput = [] # イニシャライズ
        try:
            threadNumber, bparam=self.getValue_kwargs_variables(kwargs,'_argThreadNumber','int', 0)
            if bparam==True:
                idAirtable, bparam = self.getValue_kwargs_variables(kwargs,'_argIdAirtable','str', None)
                if bparam==True: 
                    DiffMinSecondsSilentLogger_forUpdate, bparam = self.getValue_kwargs_variables(kwargs,'_argDiffMinSecondsSilentLogger_forUpdate','str',0)

            showCommandkubectl, bparam=self.getValue_kwargs_variables(kwargs,'_argShowCommandkubectl','bool', False)
            # if "'int'" in str(type(threadNumber)):
            #     threadNumber = kwargs.get("_argThreadNumber") 
            #     if '_argIdAirtable' in kwargs:
            #         idAirtable = kwargs.get("_argIdAirtable") 
            ...
            #     if "'bool'" in str(type(kwargs.get("_argShowCommandkubectl") )):
