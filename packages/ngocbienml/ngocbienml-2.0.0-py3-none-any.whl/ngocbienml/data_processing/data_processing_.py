...
class FeatureSelection(BaseEstimator, TransformerMixin):
    ...
    def feature_selection(self, X):
        ...
        def var_selection(df):
            print('start to var selection with threshold=%s' % self.threshold)
            list_drop = []
            for var in tqdm(df.columns):
                if df[var].std() < self.threshold:
                    list_drop.append(var)
            return list_drop

        def correlation_feat_selection(df):
            list_drop = []
            print('start to correlation selection feature with threshold=%s...' % (1 - self.threshold))
            corr = df.corr()
            for var1 in corr.index:
                for var2 in corr.columns:
                    condition = (var1 != var2) and (corr.loc[var1][var2] > 1 - self.threshold) \
