...
def __regression_score__(true, pred, key, message=None):
    score = mean_absolute_error(true, pred)
    if np.min(true) > 0:
        mape = mean_absolute_percentage_error(true, pred)
        mape = str(np.round(mape * 100, 2))
    else:
        mape = "undefined"
    r2 = r2_score(true, pred)
    r2 = np.round(r2 * 100, 2)
    r2 = f"{r2} %"
    if score > 1e6:
        score = int(score / 1000) * 1_000
    elif score > 1e3:
        score = int(score)
    else:
...
def regression_score(model, X1, y1, X2, y2):
    from ngocbienml.utils.utils_ import show_str_in_columns
    pred1 = model.predict(X1)
    pred2 = model.predict(X2)
    str_ = __regression_score__(y1, pred1, "train")
    str_ += __regression_score__(y2, pred2, 'test')
    show_str_in_columns(str_)


def regression_kfold_score(models, cv, X, y):
    from ngocbienml.utils.utils_ import show_str_in_columns
    def __regression_score__(true, pred):
        mae = mean_absolute_error(true, pred)
        r2 = r2_score(true, pred)
        mape = mean_absolute_percentage_error(true, pred)
        len_data = len(X)
        return mae, r2, mape, len_data

    count = 0
    for model, (index_train, index_test) in zip(models, cv.split(X, y)):
        maes_train, r2s_train, mapes_train, len_trains = [], [], [], []
        maes_test, r2s_test, mapes_test, len_tests = [], [], [], []
        train, test = X.iloc[index_train], X.iloc[index_test]
        y_train, y_test = y.iloc[index_train], y.iloc[index_test]
        pred_train = model.predict(train)
        pred_test = model.predict(test)
        mae, r2, mape, len_data = __regression_score__(y_train, pred_train)
        maes_train.append(mae)
        r2s_train.append(r2)
        mapes_train.append(mape)
        len_trains.append(len_data)

        mae, r2, mape, len_data = __regression_score__(y_test, pred_test)
        maes_test.append(mae)
        r2s_test.append(r2)
        mapes_test.append(mape)
        len_tests.append(len_data)
        count += 1
    # maes_train = sum(maes_train)/count
    # r2s_train = sum(r2s_train)/count
    # mapes_train = sum(mapes_train)/count
    # len_trains = int(sum(len_trains)/count)
    # maes_test = sum(maes_test)/count
    # r2s_test = sum(r2s_test)/count
    # mapes_test = sum(mapes_test)/count
    # len_tests = int(sum(len_tests)/count)
    str1 = to_show_acc(maes_train, r2s_train, mapes_train, len_trains, "train")
    str1 += to_show_acc(maes_test, r2s_test, mapes_test, len_tests, "test")
    show_str_in_columns(str1)



def to_show_acc(mae, r2, mape, len_, key):
    def format_mae(score):
        if score > 1e6:
            score = int(score / 1000) * 1_000
        elif score > 1e3:
            score = int(score)
    ...
    r2 = [c*100 for c in r2]
    mape = [c*100 for c in mape]
    c = len(mae)
    mae_ = sum(mae)/c
    r2_ = sum(r2)/c
    mape_ = sum(mape)/c
    len__ = int(sum(len_)/c)
    err_mae = np.std(mae)
    err_r2 = np.std(r2)
    err_mape = np.std(mape)
    key = key.upper()
    mae_, err_mae = format_mae(mae_), format_mae(err_mae)
    str_mape = "{}%+/-{}%".format(np.round(mape_, 2),np.round(err_mape, 2))
    str_r2 = "{}%+/-{}%".format(np.round(r2_, 2), np.round(err_r2, 2))
    str_mae = "{}%+/-{}%".format(mae_, err_mae)
    final_str = f"KEY={key}|MAE={str_mae}|MAPE={str_mape}|R2={str_r2}|len data={len__}\n"
    return final_str


if __name__ == "__main__":
    t = mean_absolute_percentage_error([0, 0, 1], [0, 0, 1])

    print(t, mean_absolute_error([0, 0, 1], [1, 1, 1]))
    print(np.std([1,2,3]))
