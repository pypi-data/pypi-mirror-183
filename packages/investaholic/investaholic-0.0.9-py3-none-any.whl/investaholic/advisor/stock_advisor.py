...
class StockAdvisor(Advisor):
    ...
    def _modify_position(self, ticker: str, n_proposal: int, quantity=None, closing_date=None):
        ...
        if response.status_code != 200:
            raise ConnectionError(f'Position modification [PUT] returned {response.status_code}:{response.text}')
    ...
    def _validate_position(self, ticker: str, n_proposal: int):
        ...
        if response.status_code != 200:
            raise ConnectionError(f'Position retrival [GET] returned {response.status_code}:{response.text}')

        # Belonging validation
        if not isinstance(response.json(), list): 
            user_proposal: Proposal = ProposalRepresentation.as_object(response.json())
            if user_proposal.code != n_proposal:
                raise ValueError('Position not belonging to user')
            return

        user_proposals: List[Proposal] = [ProposalRepresentation.as_object(x) for x in response.json() if x['code'] == n_proposal]
        if not len(user_proposals) == 1 or not (ticker in [position.ticker.symbol for position in user_proposals[0].positions]):
            raise ValueError('Position not belonging to user')
    ...
    def remove_position(self, ticker: str, n_proposal: int):
        ...
        if response.status_code != 200:
            raise ConnectionError(f'User deletion [DELETE] returned {response.status_code}:{response.text}')
