...
from investaholic.exceptions.exceptions import AssociationError
...
class Advisor(ABC):
    ...
    def _get_user_proposals(self):
        ...
        if response.status_code != 200:
            raise ConnectionError(f'Proposals retrieval [GET] returned {response.status_code}:{response.text}')
    ...
    def _get_customer(self, customer_id: str) -> User:
        ...
        if response.status_code != 200:
            raise ConnectionError(f'Customer retrieval returned {response.status_code}:{response.text}')
    ...
    def _add_customer(self):
        response = requests.get(f'{self._url}/users/{self.customer.id}')
    ...
    def delete_associated_customer(self):
        ...
        response = requests.delete(f'{self._url}/users/{self.customer.id}')
        if response.status_code != 200:
            raise ConnectionError(f'Customer deletion [DELETE] returned {response.status_code}:{response.text}')
    ...
    def delete_proposal(self, n_proposal: int):
        ...
        if response.status_code != 200:
            raise ConnectionError(f'Proposal deletion [DELETE] returned {response.status_code}:{response.text}')
