...
def check_latest_version():
    ...
    try:
        ...
        # this happens when in dev mode, message could be removed
        click.secho('Could not determine package version, continue nevertheless.', file=sys.stderr)
        pass
...
@click.option(
    ...
    help='Source of config files',
    )
@click.option(
    '--target-stage', '-t', required=True,
    type=click.Choice(STAGES, case_sensitive=True),
...
@click.version_option(version=__version__)
def promote(verbose, dry_run, clean_up, working_dir, source_stage, target_stage):
...
@click.option(
    '--source-domain', '-d', required=True,
    help='Source domain of config files',
    )
@click.option(
    '--source-stage', '-s', required=True,
    type=click.Choice(STAGES, case_sensitive=True),
...
@click.option(
    '--target-domain', '-t', required=True,
    help='Target domain of config files',
    )
@click.option('--clean-up', '-c', is_flag=True, default=False,
    help='Clean up files on target side that do not exist as source',
    show_default=True
...
@click.version_option(version=__version__)
def deploy(verbose, dry_run, clean_up, working_dir, source_domain, source_stage, target_domain):
