...
class PmMysqlBaseClass:
    ...
    @classmethod
    def instance(cls, host, port, user, password, database, pool_name, pm_logger, pool_size=10, pool_max_size=10,
                 pre_create_num=2):
        if cls._instance is None:
            ...
            pool = pymysqlpool.ConnectionPool(size=pool_size, maxsize=pool_max_size, pre_create_num=pre_create_num,
                                              name=pool_name, **db_config)
    ...
    def _get_connection_conn_cursor_helper(self, retry_num=10, retry_interval=0.1):
        try:
            conn = self.pool.get_connection(retry_num=retry_num, retry_interval=retry_interval)
            cursor = conn.cursor()
        except mysql.connector.Error as err:
