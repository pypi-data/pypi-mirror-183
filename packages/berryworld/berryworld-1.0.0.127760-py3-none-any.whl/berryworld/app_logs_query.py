    ...
    def query_exceptions(self, query=None, days=None):
        ...
        if query is None:
            default_kusto_query = f"""
              ...
              | where timestamp > ago({days}d) 
              | order by timestamp desc
              | project timestamp, type, severityLevel, details[0].message, customDimensions.ProjectName, 
                        customDimensions.Pipeline, customDimensions.IpAddress, customDimensions.RequestUrl
            """
        else:
        ...
        if query is None:
            logs_df = logs_df.rename(
                columns={'timestamp': 'Datetime', 'type': 'ErrorType', 'severityLevel': 'ErrorSeverity',
                         'details_0_message': 'ErrorMessage', 'customDimensions_ProjectName': 'ProjectName',
                         'customDimensions_Pipeline': 'Pipeline', 'customDimensions_IpAddress': 'IpAddress',
                         'customDimensions_RequestUrl': 'RequestUrl'}).drop_duplicates()
