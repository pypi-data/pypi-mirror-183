...
class CalculateResultsBase:
    ...
    def __init__(self, groups, dataset, samples=500, log_dir="."):
        self.groups = groups
        ...
        self.logger_results = Logger(
            "results", dataset=dataset, to_file=True, log_dir=log_dir
        )
    ...
    def calculate_metrics_for_individual_group(self,
        ...
        self.logger_results.info(f"metrics for individual group 1 {mem:.3f} GB of RAM")
        ...
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for individual group 2 {mem:.3f} GB of RAM")
        error_metrics['mase'][f'{group_name}_ind'] = np.round(self.mase(y=y,
                                                                        f=predictions_mean), 3)
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for individual group 3 {mem:.3f} GB of RAM")
        error_metrics['mase'][f'{group_name}'] = np.round(np.mean(error_metrics['mase'][f'{group_name}_ind']), 3)
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for individual group 4 {mem:.3f} GB of RAM")
        error_metrics['rmse'][f'{group_name}_ind'] = np.round(mean_squared_error(y_p,
                                                              ...
                                                              3)
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for individual group 5 {mem:.3f} GB of RAM")
        error_metrics['rmse'][f'{group_name}'] = np.round(np.mean(error_metrics['rmse'][f'{group_name}_ind']), 3)

        if predictions_std is not None:
            mem = process.memory_info().rss / (1024 ** 3)
            self.logger_results.info(f"metrics for individual group 6 {mem:.3f} GB of RAM")
            error_metrics['CRPS'][f'{group_name}'] = ps.crps_ensemble(y_p,
                                                                      ...
                                                                      .reshape((self.h,
                                                                                ...
                                                                                -1))).mean()
            mem = process.memory_info().rss / (1024 ** 3)
            self.logger_results.info(f"metrics for individual group 7 {mem:.3f} GB of RAM")
            error_metrics['CRPS'][f'{group_name}_ind'] = ps.crps_ensemble(y_p,
                                                                          ...
                                                                          .reshape((self.h,
                                                                                    ...
                                                                                    -1))).mean(axis=0)
            mem = process.memory_info().rss / (1024 ** 3)
            self.logger_results.info(f"metrics for individual group 8 {mem:.3f} GB of RAM")
        else:
        ...
        process = psutil.Process(os.getpid())
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for individual group {mem:.3f} GB of RAM")
...
class CalculateResultsBottomUp(CalculateResultsBase):
    ...
    def __init__(self, pred_samples, groups, store_prediction_samples, store_prediction_points, dataset):
        ...
        process = psutil.Process(os.getpid())
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"Instantiating class {mem:.3f} GB of RAM")
    ...
    def compute_error_for_every_group(self, error_metrics):
        ...
        for group in list(self.groups['predict']['groups_names'].keys()):
            ...
            for idx, name in enumerate(self.groups['predict']['groups_names'][group]):
                ...
                process = psutil.Process(os.getpid())
                mem = process.memory_info().rss / (1024 ** 3)
                self.logger_results.info(f"metrics for each group {mem:.3f} GB of RAM")
        ...
        process = psutil.Process(os.getpid())
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for every group {mem:.3f} GB of RAM")
        return error_metrics
    ...
    def bottom_up(self, level, error_metrics):
        ...
        process = psutil.Process(os.getpid())
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for bottom_up {mem:.3f} GB of RAM")
    ...
    def calculate_metrics(self):
        ...
        process = psutil.Process(os.getpid())
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for bottom {mem:.3f} GB of RAM")

        error_metrics = self.bottom_up('total', error_metrics)
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for total {mem:.3f} GB of RAM")

        error_metrics = self.bottom_up('groups', error_metrics)
        mem = process.memory_info().rss / (1024 ** 3)
        self.logger_results.info(f"metrics for groups {mem:.3f} GB of RAM")
