...
class GPF:
    ...
    def __init__(
        ...
        self.inducing_points = (
            torch.rand(int(inducing_points_perc * self.n_train), dtype=torch.double)
            * self.n_train
        )
    ...
    def train(
        ...
        if self.gp_type == "ngd":
            variational_ngd_optimizer = gpytorch.optim.NGD(
                model.variational_parameters(), num_data=self.train_y.size(0), lr=0.1
            )

            hyperparameter_optimizer = torch.optim.Adam(
                [{"params": model.hyperparameters()}], lr=0.01
            )
            scheduler = None
        else:
    ...
    def store_metrics(
        ...
        with open(
            f"{self.input_dir}results/results_gp_{self.gp_type}_cov_{self.dataset}_{self.model_version}.pickle",
            "wb",
        ) as handle:
            if track_mem:
                process = psutil.Process(os.getpid())
