...
class GPF:
    ...
    def _build_cov_matrices(self, ratio: float):
        """
    ...
    def _apply_mixture_cov_matrices(self, ratio: float):
        """
        ...
        """
        covs, known_mixtures, n_groups = self._build_cov_matrices(ratio)
    ...
    def _build_model(
        self, x: torch.Tensor, y: torch.Tensor, ratio: float
        ...
        """
        mixed_covs = self._apply_mixture_cov_matrices(ratio)
        n_changepoints = 4
    ...
    def train(
        ...
        likelihood_list, model_list = self._build_model(
            x=self.train_x, y=self.train_y, ratio=ratio
        )
            ...
            for i in range(epochs):
                ...
                if early_stopping:
                    if i % 5 == 0:
                        model.eval()
                        likelihood.eval()
                        val_loss = self.validate(model, ratio)
    ...
    def validate(self, model: IndependentModelList, ratio: float) -> float:
        """
        Validate the model.
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            likelihood_list_val, model_list_val = self._build_model(
                x=self.test_x, y=self.test_y, ratio=ratio
            )
    ...
    def store_metrics(
        ...
        with open(
            f"{self.input_dir}results_gp_{self.gp_type}_cov_{self.dataset}_{self.model_version}.pickle",
            "wb",
        ) as handle:
            if track_mem:
                process = psutil.Process(os.getpid())
