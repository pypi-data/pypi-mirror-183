...
class ActivityAPIClient(object):
    ...
    def complete_activity(self, process_key, activity_id):
        """
        Complete activity. Return response's content. Raise ClientError when request is failed.
        :param process_key: string to identify process that running in Ronan. You have to create Process first
                            before start an activity.
        :param activity_id: int to identify the activity
        :return: dict
        """
        uri = '/processes/%s/activities/%s/complete' % (process_key, activity_id)
        return self.api.patch(uri)
