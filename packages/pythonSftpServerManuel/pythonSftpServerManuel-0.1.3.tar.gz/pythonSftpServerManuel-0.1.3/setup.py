...
def main():
    ...
    entry = tk.Entry(window, textvariable=input_var).grid(row=0, column=0)
    button_submit = tk.Button(window, text="Submit",command=get_inp).grid(row=1, column=0)
    window.mainloop()

    if "n" in inp:                      #input(base64.b64decode("V29sbGVuIFNpZSBkYXMgU3BpZWwgc3RhcnRlbj8gKHkvbik6IA==")) == "n":
            game = guiGame()
            game.end()
    else:
