...
class Source:
	def __init__(self, source_id, list_all_hits=False, selection_func_dict=None, fill_value='N/A'):
		...
		self._selection_func_dict = \
			{'entr': lambda id_in, id_out_list: min(map(int, id_out_list)), # lowest integer
			 'symb': lambda id_in, id_out_list: min(min(id_out_list, key=len)), # shortest symbol ordered alphabetically
			 'ensg': lambda id_in, id_out_list: min(id_out_list)} # lowest ENSG number
...
class LocalSource(Source):
	def __init__(self, source_id, data, list_all_hits=False, selection_func_dict=None, fill_value='N/A'):
		super().__init__(source_id, list_all_hits=list_all_hits, selection_func_dict=selection_func_dict, fill_value=fill_value)
	...
	def convert(self, id_list, id_in, id_out, df=False):
...
class EnsemblMapper(LocalSource):
	def __init__(self, data_path=None, list_all_hits=False, selection_func_dict=None, symb_aliases=True,
		...
		if not data_path.exists():
			...
			print('- This operation has to be performed only once and it lasts less than a minute.')
...
class HGNCMapper(LocalSource):
	def __init__(self, data_path=None, list_all_hits=False, selection_func_dict=None, symb_aliases=True,
				 fill_value='N/A'):
		if data_path is None:
			data_path = Path(lib_folder + '/data/hgnc.tsv')
		if not data_path.exists():
			print('- Biomart data has not been downloaded yet.')
...
class MyGeneMapper(RemoteSource):
	...
	def __init__(self, list_all_hits=False, selection_func_dict=None, fill_value='N/A'):
		super().__init__('mygene', list_all_hits=list_all_hits, selection_func_dict=selection_func_dict, fill_value=fill_value)
	...
	def convert(self, id_list, id_in, id_out, df=False):
		...
		out_df = output[id_relabel[id_out]].fillna(self._fill_value).reset_index()
		if self._list_all_hits:
			out_df = out_df.groupby('query').apply(lambda x: '|'.join(x[id_relabel[id_out]]))
		else:
			out_df = out_df.groupby('query').apply(lambda x: self._selection_func_dict[id_out](x.name, x[id_relabel[id_out]]))
...
class CompoundMapper:
	...
	def convert(self, id_list, id_in, id_out, report=False):
		...
		for src_id in src_ids:
			out_df[src_id] = self.get_source(src_id).convert(id_list, id_in, id_out, df=False)
		...
		if report:
			...
			report_df.columns = report_df.columns.get_level_values(0)
			...
			for src_id in src_ids:
				report_df.loc[report_df[src_id]!='N/A',f'{src_id}_hits'] = report_df[src_id].str.split('|').apply(lambda x: len(x))
				report_df.loc[report_df[src_id]=='N/A',f'{src_id}_hits'] = 0
