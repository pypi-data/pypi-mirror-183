...
class TicketOrderChecker(Checker):
    ...
    def get_checkdata_problems(self, obj, fix=False):
        if obj.order is None or obj.site_id is None:
            return
        if obj.site.ref is None:
            return
        if obj.site.company == settings.SITE.site_config.site_company:
            return
        yield (True, _("Must fill order field after upgrade to Noi 22.12"))
        if fix:
            # Not tested. We'll just get it work after the upgrade
            Subscription = rt.models.subscriptions.Subscription
            Journal = rt.models.Journal.Journal
            VoucherTypes = rt.models.Journal.VoucherTypes
            sub = Subscription.objects.filter(subject=obj.site.ref).first()
            if sub is None:
                ...
                if jnl is None:
                    ...
                    jnl.full_clean()
                    jnl.save()
                sub = Subscription(partner=obj.site.company, jnl=jnl, subject=obj.site.ref)
                sub.full_clean()
                sub.save()
            obj.order = sub
            obj.full_clean()
