...
class Storage(base.Storage):
    ...
    def presignedGetObject(self, key, expires=3600, bucket=None):
        bucket = bucket or self.bucket
        url = self.client.presigned_get_object(bucket, key, expires=timedelta(seconds=expires))
        return url

    def presignedPutObject(self, key, expires=3600, bucket=None):
        bucket = bucket or self.bucket
        url = self.client.presigned_put_object(bucket, key, expires=timedelta(seconds=expires))
        return url

    def getObject(self, key, bucket=None, **kwargs):
        logger.debug(f"Get object from {key}")
        bucket = bucket or self.bucket
        response = None
        try:
            response = self.autoRefreshToken(self.client.get_object)(bucket, key, **kwargs)
    ...
    def putObject(self, key, data, bucket=None, **kwargs):
        logger.debug(f"Put object to {key}")
        bucket = bucket or self.bucket
        length = kwargs.get('length')
        part_size = kwargs.get('part_size')
        if not length and not part_size:
            length = -1
        ...
        result = self.autoRefreshToken(self.client.put_object)(
            bucket, key, data, length=length, part_size=part_size, **kwargs)
        return result
