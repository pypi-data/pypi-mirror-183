...
class Storage(base.Storage):
    ...
    def presignedGetObject(self, key, expires=3600, bucketOrBucketName=None, slash_safe=False):
        bucket = self.getBucket(bucketOrBucketName)
        url = bucket.sign_url('GET', key, expires, slash_safe=slash_safe)
        return url

    def presignedPutObject(self, key, expires=3600, bucketOrBucketName=None, slash_safe=False):
        bucket = self.getBucket(bucketOrBucketName)
        url = bucket.sign_url('PUT', key, expires, slash_safe=slash_safe)
        return url
    ...
    def getObject(self, key, bucket=None, **kwargs):
        ...
        object_stream = self.autoRefreshToken(bucket.get_object)(key, **kwargs)
    ...
    def putObject(self, key, data, bucket=None, **kwargs):
        ...
        result = self.autoRefreshToken(bucket.put_object)(key, data, **kwargs)
        return result
