...
class QueryData:
    ...
    def normal_query_select(self, cursor, table_name:str, declare_column:list = [], alias_column:list = [] 
                            , where_column:list = [], where_value:list = [], limit:int = [], offset:int = [], grouping_column:list = []
                            , ordering_column:str = [], ordering:str = "asc", like_column:list = [], or_column:list = []
                            , or_value:list = [], lowercase_column:list = []):
            ...
            for idx, data_where in enumerate(where_column):
                ...
                if idx == 0:
                    if str(data_where).lower() in list(map(lambda x: str(x).lower(), like_column)):
                        ...
                        __datavalue.append(where_value[idx])
                else:
