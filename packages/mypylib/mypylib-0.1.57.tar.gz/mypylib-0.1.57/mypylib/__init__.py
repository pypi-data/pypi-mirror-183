...
def get_taifex_weight_list():
    ...
    try:
        for x in df[0].to_dict('index').values():
            list_weight.append({'symbol': x['證券名稱'], 'weight': Decimal(x['市值佔 大盤比重'][:-1])/100})
            list_weight.append({'symbol': x['證券名稱.2'], 'weight': Decimal(x['市值佔 大盤比重.1'][:-1])/100})
