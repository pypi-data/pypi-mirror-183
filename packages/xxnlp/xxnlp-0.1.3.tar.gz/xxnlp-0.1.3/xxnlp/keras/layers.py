...
from xxnlp.backend import keras, K, is_tf_keras
from xxnlp.backend import align, sequence_masking
from xxnlp.backend import recompute_grad
from xxnlp.backend import attention_normalize
from xxnlp.backend import sinusoidal_embeddings
from xxnlp.backend import apply_rotary_position_embeddings
...
if (not is_tf_keras) or tf.__version__ < '1.15':
    ...
    if not is_tf_keras:
        ...
        NodeBase = keras.engine.node.Node
    else:
        from tensorflow.python.keras.engine import base_layer
        NodeBase = base_layer.node_module.Node

    class Node(NodeBase):
        """修改Node来修复keras下孪生网络的bug
        注意：这是keras的bug，并不是xnlp的bug，但keras已经不更新了，
    ...
    if not is_tf_keras:
        keras.engine.node.Node = Node
    else:
        base_layer.node_module.Node = Node
