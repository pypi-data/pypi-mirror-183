...
def main():
    ...
    parser.add_argument('-v', '--version', action='version', version='%(prog)s 1.1.3.7')
    ...
    # using dirloc, which is the directory location of the csv files
    file_list = glob.glob(dirloc + '*.csv')
    print(file_list)

    # try df = pd.read_csv(file_list[file_number], encoding='cp949'), and if there isn't any file, then show the error message and stop the program
    try:
