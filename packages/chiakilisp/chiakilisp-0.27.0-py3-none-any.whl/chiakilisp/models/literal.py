...
class Literal(LiteralType):
    ...
    def wrapped(self) -> str:
        ...
        """Wraps literal to a string"""

        return f'"{self.token().value()}"' \
            if self.token().is_string() else (self.prefix() + self.token().value())
    ...
    def execute(self, environment: dict, __=False) -> Any:  # pylint: disable=inconsistent-return-statements
        ...
        if self.property("quoted"):  # TODO: maybe implement handy shortcuts ###############################

            return self  # <------------------ return literal instance in case user have decided to quote it
