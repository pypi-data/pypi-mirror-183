...
class Lexer:
    ...
    def lex(self) -> None:  # pylint: disable=R0912, disable=R0915  # :(
        ...
        while self._has_next_symbol():
            ...
            elif self._current_symbol_is_at():
                self._tokens.append(Token(Token.At,               '@', self._pos()))
                self._advance()
                self._increment_char_number()  # <-- increment character num as well

            elif self._current_symbol_is_tilda():
                if self._has_next_symbol() and self._next_symbol_is_at():
                    self._tokens.append(Token(Token.TilAt,       '~@', self._pos()))
                    self._advance()
                    self._increment_char_number()
                else:
                    self._tokens.append(Token(Token.Tilda,        '~', self._pos()))
                self._advance()
                self._increment_char_number()   # <-- increment character num as well
            ...
            elif self._current_symbol_is_apostrophe():
                self._tokens.append(Token(Token.Apostrophe,       '`', self._pos()))
                self._advance()
                self._increment_char_number()  # <-- increment character num as well
    ...
    def _current_symbol_is_tilda(self) -> bool:

        """Returns whether current symbol is a tilda symbol"""

        return self._current_symbol() == '~'

    def _next_symbol_is_at(self) -> bool:

        """Returns whether next symbol is an at symbol"""

        return self._next_symbol() == '@'

    def _current_symbol_is_at(self) -> bool:

        """Returns whether current symbol is an at symbol"""

        return self._current_symbol() == '@'

    def _current_symbol_is_apostrophe(self) -> bool:

        """Returns whether current symbol is an apostrophe symbol"""
