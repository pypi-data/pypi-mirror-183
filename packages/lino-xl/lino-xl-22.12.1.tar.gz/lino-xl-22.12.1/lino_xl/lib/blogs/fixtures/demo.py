...
def objects():
    ...
    cross_roads = prepare_entry(title = title, body = body, pub_date = demo_date)
    yield cross_roads
    ...
    melancholy_house = prepare_entry(title=title, body=body, pub_date=demo_date - dt.timedelta(days=1))
    ...
    bleeding_words = prepare_entry(title=title, body=body, pub_date=demo_date - dt.timedelta(days=2))
    yield bleeding_words
    bleeding_words.after_ui_save(ar, None)
