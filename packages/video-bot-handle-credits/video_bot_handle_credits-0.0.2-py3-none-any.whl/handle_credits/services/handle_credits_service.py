...
from handle_credits.utils.helpers import get_attr, generate_id, set_attr
from handle_credits.utils.json import  from_json
from handle_credits.utils.file import read_binary_file
from handle_credits.utils.video import get_video_duration
from handle_credits.models.page import Sentence
from handle_credits.models.credits import Credits


class HandleCreditsService(BaseService):
    ...
    def __init__(self, config, executer, downloader, saver, external_services = []):
        ...
        self.credits_muted = get_attr(self.channel_info,  'credits_muted', False)
    ...
    def handle_credits(self, file_object):
        ...
        #add sentence
        sentences = []
        if (len(str(self.credits_text)) > 0):
            text = self.replace_placeholders(self.credits_text)                
        ...
        if ((self.use_narration == True )  and (self.credits_muted == False) ): 
        ...
        credits =  Credits(media_id = generate_id(), path = file_path, path_converted = '', video_time = video_time, sentences = sentences)
        return credits     
    ...
    def execute(self):
        ...
        for file in saved_files:
            result  = self.handle_credits(file)
