...
async def client(hub, ctx, client_class=AsyncClientSession) -> aiohttp.ClientSession:
    ...
    client_key = "client_" + _generate_key(auth=auth, auth_header=auth_header, **session_kwargs)
...
async def cookie_jar(
    ...
    if cookie_jar_key not in hub.tool.request.application.APP:
        hub.tool.request.application.APP[cookie_jar_key] = cookie_jar_class(**cookie_jar_kwargs)
...
async def resolver(
    ...
    if resolver_key not in hub.tool.request.application.APP:
        hub.tool.request.application.APP[resolver_key] = resolver_class(**resolver_kwargs)
