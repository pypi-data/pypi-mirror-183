...
class SpatialAnalysis:
  ...
  # REQUIRES: User does not cancel out with '1' input
  # MODIFIES self.c pandas dataframe
  # EFFECT: Deletes column from the .csv file that the user inputed to the SpatialAnalysis
  # NOTE1: INTENDED USE IS TO UNDO CHANGES FOR TUTORIAL
  def destroy_mods(self):
      assert(input('This function will destroy additions to the .csv file. Enter 1 to confirm or any input to cancel: ') == '1')
      del self.c['Is Outlier']
      del self.c['Region']
      del self.c['Distance from Stadium (miles)']
      del self.c['Latitude']
      del self.c['Longitude']
      self.c.to_csv(self.__path, index=False)

  # REQUUIRES: N/A
  # MODIFIES: self.c
  # EFFECT: does the same as initalization for SLD with harversine but user prompted
  # PURPOSE: To allow the user to maually enter lat and long values for GeoPy fetch fails
        # and update the .csv without reruning the enter SLDneed=True version of the
        # constructor
  def manualLatLong(self):
    for i in range(0, len(self.c)):
        if self.c['Latitude'][i] is not None and self.c['Longitude'] is not None:
            geoAdd = (float(self.c['Latitude'][i]), float(self.c['Longitude'][i]))
            self.c.loc[i, 'Distance from Stadium (miles)'] = hs.haversine(self.__centCords, geoAdd, unit=Unit.MILES)
            self.c.to_csv(self.__path, index=False)
            if self.c['Latitude'][i] > self.__LatPark and self.c['Longitude'][i] > self.__LongPark:
    ...
    for i in range(0, len(self.c)):
        if self.c['Distance from Stadium (miles)'][i] > self.__radSize or self.c['Distance from Stadium (miles)'][i] == 0:
            ...
            self.c['Longitude'][i] = np.nan
            self.c['Is Outlier'][i] = np.nan
        print('NUM ERRORS=', self.c['Distance from Stadium (miles)'].isna().sum(), 'Values.')
        if (self.c['Distance from Stadium (miles)'].isna().sum() != 0):
            print('Either manual entry of latitude and longitude was incomplete in some way, or some data should NOT have been included in the set.')
    self.c.to_csv(self.__path, index=False)
