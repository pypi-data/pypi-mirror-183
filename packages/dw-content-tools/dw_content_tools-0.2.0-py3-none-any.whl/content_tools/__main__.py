...
from .json_schemas import metadata_schema, docker_compose_device_schema
...
def __validate_metadata_yml(file_path):
    with file_path.open("r") as metadata_path:
        metadata = metadata_path.read()
        metadata = yaml.load(metadata, Loader=yaml.CLoader)

        try:
            validate(metadata, metadata_schema)
        except JSONSchemaValidationError as exc:
            raise (ValidationException(f"Invalid metadata.yml: {str(exc.message)}"))
