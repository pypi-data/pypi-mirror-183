...
def _find_configure_file() -> str:
    """ Recursively find a `configure.py` starting at current working directory and ascending from there """

    first_path = os.getcwd()
    path = os.getcwd()
    while path != os.sep:
        configure_path = os.path.join(path, CONFIGURE_FILE_NAME)
        if os.path.exists(configure_path):
            if path != first_path:
                os.chdir(path)
                logging.info("Switched to path %s to read `configure.py`", path)
            return configure_path

        path = os.path.abspath(os.path.join(path, '..'))

    return os.path.join(os.getcwd(), CONFIGURE_FILE_NAME)  # Fallback, the next tools will discover that there is config
