...
## file
filename_format = lambda x:list(x:=x.replace(z,'_') for z in '\\/:*?"<>|') and x
'''format filename'''
...
writealllines = lambda lines,src,srccoding="utf-8": open(src,'w',encoding=srccoding).writelines(list(line+'\n' for line in lines))
...
afterDecodeCP932 = lambda x, replis:list(x:=x.replace(y[0],y[1]) for y in [['〜','～'],['‖','∥'],['−','－']]) and x
'''fix CP932 content read with python'''
beforeEncodeCP932 = lambda x, replis:list(x:=x.replace(y[0],y[1]) for y in [['·','・'],['～','〜'],['∥','‖'],['－','−']]) and x
''''''
    ...
    if isinstance(json_data, list):
        ...
        for i in range(len(json_data)):
            if i % maxlen == 0 and i != 0:
                json_str += "\n"
            json_str += solve_value(json_data[i]) + ","
        json_str = json_str[:-1] + "]"
        ...
        for key, value in json_data.items():
            if json_str.count(",") % maxlen == 0 and json_str.count(",") != 0:
                json_str += "\n"
            json_str += '"' + str(key).replace('"','\\"') + '":' + solve_value(value) + ','
        json_str = json_str[:-1] + "}"
    else:
...
def readfile_by_line_max_N(file_name, N, func):
