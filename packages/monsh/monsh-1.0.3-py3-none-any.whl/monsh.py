...
def keep_alive(api_key, job_identifier, verbosity):
    ...
    try:
        logging_level = logging._nameToLevel[verbosity]
    except KeyError:
        pass
...
def run():
    ...
    try:
        ...
        print(f"Failed to coerce name provided '{verbosity}' to logging level.")
    
    logging.basicConfig(level=logging_level)
    logging.info(f"Starting mon job under workspace {workspace}: {job_start_time}/{job_identifier}")
    workspace_url_safe = urllib.parse.quote_plus(workspace)
    logging.info(f"->> https://www.mon.sh/job?id={workspace_url_safe}%2F{job_start_time}%2F{job_identifier}")
    ...
    last_write_time = time.time() # in UTC seconds since epoch
    events_buffer = []
    sequence_token = None
