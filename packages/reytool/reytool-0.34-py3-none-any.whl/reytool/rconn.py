...
class RConn(object):
    ...
    def __init__(
        ...
        only_once: bool = True,
        conn: Optional[Engine | Connection] = None
    ) -> None:
        """
    ...
    def create_conn(
        ...
        user: str = get_first_notnull(user, self.user)
        password: str = get_first_notnull(password, self.password)
        host: str = get_first_notnull(host, self.host)
        port: str | int = get_first_notnull(port, self.port)
        database: str = get_first_notnull(database, self.database)
        charset: str = get_first_notnull(charset, self.charset, default="utf8")
        only_once: str = get_first_notnull(only_once, self.only_once, default="utf8")

        if self.conn and [user, password, host, port, database] == [None] * 5:
            return self.conn
 
        try:
            url = f"fmysql+mysqldb://{user}:{password}@{host}:{port}/{database}?charset = {charset}"
            engine = create_engine(url)
        except ModuleNotFoundError:
            url = f"fmysql+pymysql://{user}:{password}@{host}:{port}/{database}?charset = {charset}"
            engine = create_engine(url)
    ...
    def fetch_table(self, data: LegacyCursorResult | Iterable, fields: Optional[Iterable] = None) -> list[dict,]:
        """
    ...
    def fetch_df(self, data: LegacyCursorResult | Iterable, fields: Optional[Iterable] = None) -> DataFrame:
        """
    ...
    def fetch_sql(self, data: LegacyCursorResult | Iterable, fields: Optional[Iterable] = None) -> str:
        """
    ...
    def execute_select(
            ...
            table: str,
            database: Optional[str] = None,
            fields: Optional[str | Iterable] = None,
            where: Optional[str] = None,
            order: Optional[str] = None,
            limit: Optional[int | str | Iterable] = None,
        ...
        limit : 'LIMIT' syntax content.
            ...
            - Iterable [str/int, ...] with length of 1 or 2 : Join as 'LIMIT int/str, ...'.
    ...
    def execute_update(
        self,
        data: LegacyCursorResult | List | Dict,
        table: str,
        database: Optional[str] = None,
