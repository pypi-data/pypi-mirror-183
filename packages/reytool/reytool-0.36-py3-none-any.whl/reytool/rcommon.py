...
from typing import Any, List, Tuple, Dict, Iterable, Literal, Optional
import os
import time
import random
...
def exc(title: str | None = "Error", to_print: bool = True) -> str:
    """
...
def log(records: List[Iterable] | List[Dict] | DataFrame | Series, fields: Optional[Iterable] = None, path: str = "log.csv") -> None:
    """
...
def randn(*thresholds: int | float, precision: Optional[int] = None) -> int | float:
    """
    Get random number.
...
def sleep(*thresholds: int | float, precision: Optional[int] = None) -> int | float:
    """
