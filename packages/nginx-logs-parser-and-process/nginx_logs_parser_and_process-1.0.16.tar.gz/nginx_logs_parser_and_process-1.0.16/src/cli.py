...
from src.infrastructure.LocalSqliteDatabase import LocalSqliteDatabase
...
def process_remote_nginx_logs():
    ...
    tb_nginx = Server(
        host="tbnginxasroot", source_path="/var/log/nginx/", name="TB Nginx Server", data_filter=tb_data_to_preserve, tables=tb_tables
    )
    seb_nginx = Server(
        host="tbnginxsebasroot", source_path="/var/log/nginx/", name="SEB Nginx Server", data_filter=seb_data_to_preserve, tables=seb_tables
    )
    remote_servers.append(tb_nginx)
    remote_servers.append(seb_nginx)
    local_server = Server(source_path=DI.logs_path(),
                          name="Local Server",
                          includes="seb.access.log*,seb.portalaluno.ops1.access.log*,seb.ops1.portaldoaluno.access.log*")
