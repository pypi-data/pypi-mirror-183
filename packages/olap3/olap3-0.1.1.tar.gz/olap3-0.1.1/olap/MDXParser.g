...
mdx_statement : (select_statement //-> ^(SELECTSTMT select_statement)
			 | create_formula_statement ) EOF
...
select_statement : (WITH formula_specification)? 
				  ...
				  (cell_props)? -> ^(SELECTSTMT formula_specification? axis_specification_list? cube_specification slicer_specification? cell_props?)
				;
...
axis_specification_list : axis_specification (COMMA axis_specification)* -> ^(AXESPEC axis_specification+)
			;
...
axis_specification	: (NON EMPTY)? expression (dim_props)? ON axis_name -> ^(AXISPEC expression axis_name)
			;
...
cube_specification: cube_name -> ^(CUBESPEC cube_name)
			;
