...
# TODO:
# 1. Test metaclass router specifier
# 2. enter_callback, error_callback as long as enter_message_text etc.

# TODO:
# Support for inline buttons and callback id?
# (?) use several field functions
# e. g. TextField for message fields
# CallbackField for inline fields
# allow using one or several choices
# List[CBDATA] | CBDATA
...
class FormMeta(ABCMeta):
    ...
    router: ClassVar[Optional[Router]] = None
...
class Form(ABC, metaclass=FormMeta):
    ...
    async def start(
        ...
        if cls.router is None:
            raise ValueError(f"{cls.__name__}.router should be set")
