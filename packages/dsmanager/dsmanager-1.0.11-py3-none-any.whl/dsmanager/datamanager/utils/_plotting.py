...
def scatter_matrix(df_: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
    """Plot a scatterplot matrix of the dataframe using plotly
        express scatter
...
def displot(df_: pd.DataFrame, name: str, **kwargs: Any) -> pd.DataFrame:
    """Plot a distplot for a given variable of a dataframe using plotly
        figure factory create_distplot
...
def corr(df_: pd.DataFrame,
    ...
    Args:
        df_ (pd.DataFrame): DataFrame to be returned
        squared (bool, optional): If True use squared correlation instead of
            the simple correlation. Defaults to False.
