...
try:
    ...
    print("DataSource 'SharepointSource' is not available because "
          f"'{module_error.name}' is missing")
try:
    from dsmanager.datamanager.datasources.sfsource import SFSource
    SOURCES["salesforce"] = SFSource
except ModuleNotFoundError as module_error:
    print("DataSource 'SFSource' is not available because "
          f"'{module_error.name}' is missing")
try:
    from dsmanager.datamanager.datasources.kagglesource import KaggleSource
    SOURCES["kaggle"] = KaggleSource
except ModuleNotFoundError as module_error:
    print("DataSource 'KaggleSource' is not available because "
          f"'{module_error.name}' is missing")
except (OSError, ValueError) as other_error:
    print("DataSource 'KaggleSource' is not available.\n"
          f"More details: {other_error}")
