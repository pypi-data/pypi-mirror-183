...
def _loop_pipe(
    df_: pd.DataFrame,
    generator: Generator[Tuple[Callable[..., Any], Any], None, None]
) -> pd.DataFrame:
    """Wrap chain function to a DataFrame using a generator to apply multiple
        functions ans theirs arguments in chain

    Args:
        df_ (pd.DataFrame): DataFrame that will be piped
        generator (Generator[Tuple[Callable[..., Any], Any], None, None]):
            Reversed generator of functions and their respective arguments and
            keyword arguments
...
def pipe_sklearn(df_: pd.DataFrame,
    ...
    Args:
        ...
        filter_func (Any, optional): Filter function to apply on original df_.
            Defaults to lambda df__ : df__.
        output_cols (List[str], optional): List of output columns expected.
            Defaults to None.
    ...
    if isinstance(res, np.ndarray) and len(index_list) == res.shape[0]:
        ...
        return (
            df_
            .drop(index_list)
            .pipe(lambda df__: pd.concat(
                (
                    df__,
                    filter_func(df_)
                    .assign(**{
                        output_cols[i]: res[:, i] for i in np.arange(res.shape[-1])
                    })
                )
            ))
            .reindex(df_.index)
...
def pipe_leaf(df_: pd.DataFrame,
              func: Any,
              *args: Any,
              datamanager: Any = None,
              verbose: int = 1,
...
def pipe_steps(df_: pd.DataFrame,
               steps: List[Tuple[Callable, Any]]) -> pd.DataFrame:
    """Wrap chain function to a DataFrame using a generator to apply multiple
        functions ans theirs arguments in chain

    Args:
        df_ (pd.DataFrame): DataFrame that will be piped
        steps (List[Tuple[Callable, Any]): List of functions and their
