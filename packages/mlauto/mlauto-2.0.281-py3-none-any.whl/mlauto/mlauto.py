    ...
    if save_function:
        ...
        if event == "call":
            ...
            if 'ipykernel' not in frame.f_code.co_filename:
                node(frame.f_code.co_name, frame.f_code.co_filename, frame.f_code.co_firstlineno, False)
            elif library_function == True:
                node(frame.f_code.co_filename.rsplit('/', 1)[1], frame.f_code.co_filename, frame.f_code.co_firstlineno, 1)
            else:
                node(frame.f_code.co_name, 'Jupyter notebook', 0, False)
