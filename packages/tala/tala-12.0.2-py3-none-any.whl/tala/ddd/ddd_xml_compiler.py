...
class DomainCompiler(XmlCompiler):
    ...
    def _compile_inform_element(self, element):
        ...
        forget_plan = [ForgetPlanItem(predicate)]
        ...
        end_turn_plan = [EndTurnPlanItem(float(expected_passivity))]

        plan = [AssumePlanItem(predicate_proposition),
                AssumeIssuePlanItem(question)]

        if insist == "true":
            plan = forget_plan + plan
        if generate_end_turn == "true":
            plan = plan + end_turn_plan
        return plan
