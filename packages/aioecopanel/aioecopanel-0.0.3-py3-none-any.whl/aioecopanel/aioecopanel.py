...
class Interface:
    ...
    async def listen(self, callback: Callable[[DeviceDict], None]):
        if not self._client or not self.connected or not self._device_dict:
            raise EcoPanelError("Not connected to a WLED WebSocket")
    ...
    async def request(self,uri: str = "",method: str = "GET", data: Union[dict[str, Any], None ]= None,) -> Any:
        ...
        try:
            ...
            if response.status // 100 in [4, 5]:
                ...
                raise EcoPanelError(response.status, {"message": contents.decode("utf8")})
    ...
    async def write_property(
        ...
        await self.request("/apiv1/{}/{}".format(deviceid,objectid), method="POST", data=data_to_send)
