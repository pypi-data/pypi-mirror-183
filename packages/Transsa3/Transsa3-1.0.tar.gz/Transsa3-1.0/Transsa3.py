...
def tabla_auxiliar(df1,df2,method,variables): #(df1 es la tabla de ventas, df2 es la tabla de tasaciones, 
...
def Muestra(df1,df2,cve):
    ...
    MuestraAVM=MuestraAVM.drop(['ingreso','avaluofiscal','ufm2(supCSII)','ufm2(supTSII)','supTSII/supCSII'],axis=1)
...
def Xrl_inter(remL,inter,indexs_max):
    if inter==1:
        indexaciones=['antiguedad','longitud','latitud','supCSII','supTSII']
        Xrlinter = np.array(remL[indexaciones])
        long=len(indexaciones)
    elif inter==10:
        indexaciones=['longitud', 'latitud', 'supCSII', 'supTSII']
        for j in range(0,len(indexs_max[0])):
            ins=str(indexs_max[0][j])
            string="Index_antiguedad_10_"+ins
            indexaciones.insert(j,string)
        Xrlinter = np.array(remL[indexaciones])
        long=len(indexaciones)
    elif inter==5:
        indexaciones=['longitud', 'latitud', 'supCSII', 'supTSII']
        for j in range(0,len(indexs_max[1])):
            ins=str(indexs_max[1][j])
            string="Index_antiguedad_5_"+ins
            indexaciones.insert(j,string)
        Xrlinter = np.array(remL[indexaciones])
        long=len(indexaciones)
