...
def InsertColumns(D): #(D es la tabla) 
    D=D.astype({'nro dormitorios': 'float64','nro banos':'float64','nro estacionamientos':'float64','supCSII':'float64',
                'supTSII':'float64','valor':'float64'})
...
def datosduplicados(tabla,T): #(tabla es la tabla, T es True entonces se muestra la cantidad de datos eliminados)
    ...
    tabla2 = tabla.drop_duplicates(subset=['nro dormitorios','nro banos','nro estacionamientos','longitud','latitud',
                                          'supCSII','supTSII'])
...
def atypicals_be_gone(df,pars,T,metodo,limit):
    ...
    colors=['mediumslateblue','brown','cadetblue','silver']
...
def tam_muestra(ofertas_comuna1,confianza):
    ...
    er=10/ofertas_comuna1['valor'].mean()
    Z=stats.norm.ppf(1-alpha/2)
    COV=ofertas_comuna1['valor'].std()/ofertas_comuna1['valor'].mean()
...
def Muestra(df1,df2,cve):
    ...
    MuestraAVM=pd.merge(df2,Muestras[1], how="right", 
                        on=["cve_comuna",'nro dormitorios','nro banos','nro estacionamientos',"longitud","latitud","supCSII","supTSII","valor"])
...
def Entrenamiento(df2, t_s):
    Xrlinter = np.array(df2[['nro dormitorios','nro banos','nro estacionamientos','longitud','latitud','supCSII','supTSII']])
...
def Escalas(cve, Xrlinter_train, Xrlinter_test):
    scl = StandardScaler().fit(Xrlinter_train[:,3:])
    Xrlinter_train[:,3:] = scl.transform(Xrlinter_train[:,3:])  
    Xrlinter_test[:,3:] = scl.transform(Xrlinter_test[:,3:])  
