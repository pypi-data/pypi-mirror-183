...
def setup(request, pytestconfig):
    ...
    if current_marker == CONFIG.MARKERS_NOGUI:
        ...
        driver = DM.get_browser(CONFIG.BROWSER_CHROME)
        ...
        G_REPORT_RUNTIME[CONFIG.URL] = env.url # pytestconfig.getoption(CONFIG.APP_LINK)
        G_REPORT_RUNTIME[CONFIG.APP_NAME] = env.app # pytestconfig.getoption(CONFIG.APP_NAME)
        G_REPORT_RUNTIME[CONFIG.USERNAME] = env.username # pytestconfig.getoption(CONFIG.USERNAME)
...
def pytest_html_report_title(report):
    ...
    report.title =  (env.app + CONSTANTS.SPACE + "automated test Report").title()
...
def pytest_configure(config):
    ...
    env.app = config.getoption(CONFIG.APP_NAME).capitalize()
    env.url = config.getoption(CONFIG.APP_LINK).lower()
    env.username = config.getoption(CONFIG.USERNAME)

    config._metadata = build_metadata(env.app, env.url ,env.username)
