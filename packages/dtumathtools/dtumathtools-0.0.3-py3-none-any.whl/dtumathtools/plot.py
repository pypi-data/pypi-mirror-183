...
from sympy.utilities.iterables import is_sequence, flatten
...
class Plot:
    ...
    def matplotlib(self):
        if hasattr(self, "_backend"):
            self._backend.close()
        mpl_backend = plot_backends["matplotlib"]

        mpl_backend = mpl_backend(self)
        mpl_backend.process_series()
        mpl_backend.fig.tight_layout()
        return mpl_backend.fig, mpl_backend.ax[0]
    ...
    def set_axis_equal(self, arg=True):
        self.axis_equal = arg
...
class SurfaceBaseSeries(BaseSeries):
    ...
    def __init__(self, **kwargs):
        ...
        # extra arguments to get all kwargs into the ax.plot_surface
        ...
        # opaque is own made argument to adjust antialiaseds, overwritten if antialiaseds is given
        self.opaque = kwargs.get("opaque", True)
        kwargs.pop("opaque", None)
        self.antialiaseds = kwargs.get("antialiaseds", not self.opaque)
        kwargs["antialiaseds"] = self.antialiaseds
        self.kwargs = kwargs
...
### Scatter plots
class ScatterBaseSeries(BaseSeries):
    """Representation for a scatter plot."""
...
class Scatter2dSeries(ScatterBaseSeries):
    """Representation for a scatter plot in 2D."""
    ...
    def __init__(self, x, y, **kwargs):
        ...
        if not "xlim" in kwargs:
            ...
            # Set limit slightly outside borders of points
            self._xlim = (
                self._xlim[0] - 0.05 * self.lenrange_x,
                self._xlim[1] + 0.05 * self.lenrange_x,
            )
        else:
            self._xlim = kwargs.pop("xlim")
        ...
        if not "ylim" in kwargs:
            ...
            # Set limit slightly outside borders of points
            self._ylim = (
                self._ylim[0] - 0.05 * self.lenrange_y,
                self._ylim[1] + 0.05 * self.lenrange_y,
            )
        else:
            self._ylim = kwargs.pop("ylim")

        super().__init__(**kwargs)


class Scatter3dSeries(ScatterBaseSeries):
    """Representation for a scatter plot in 2D."""
    ...
    def __init__(self, x, y, z, **kwargs):
        ...
        self._xlim = (min(self.x), max(self.x))
        self._ylim = (min(self.y), max(self.y))
        self._zlim = (min(self.z), max(self.z))
        self.lenrange_x = self._xlim[1] - self._xlim[0]
        self.lenrange_y = self._ylim[1] - self._ylim[0]
        self.lenrange_z = self._zlim[1] - self._zlim[0]
        ...
        if not "zlim" in kwargs:
            ...
            # Set limit slightly outside borders of points
            self._zlim = (
                self._zlim[0] - 0.05 * self.lenrange_z,
                self._zlim[1] + 0.05 * self.lenrange_z,
            )
        else:
...
class MatplotlibBackend(BaseBackend):
    ...
    def __init__(self, parent):
        ...
        self.inspect = import_module("inspect")
        ...
        for i, series in enumerate(series_list):
            ...
            if any(are_3D) and not all(are_3D):
                ...
                if not all([series[j].is_scatter for j in range(len(series))]):
    ...
    def _process_series(self, series, ax, parent):
        ...
        for s in series:
            ...
            if s.is_2Dline:
                ...
                if (
                    type(s.cmap) == str
                    and (s.cmap.lower() == "std" or s.cmap.lower() == "standard")
                ...
                if isinstance(s.surface_color, (float, int, Callable)):
                    ...
                    if not s.surface_color is None:
                ...
                ax.scatter(s.x, s.y, s.z, **s.kwargs)
                xlims.append(s._xlim)
                ylims.append(s._ylim)
                zlims.append(s._zlim)
        ...
        if not isinstance(ax, Axes3D):
            ax.autoscale_view(
                scalex=ax.get_autoscalex_on(), scaley=ax.get_autoscaley_on()
            )
        else:
            # XXX Workaround for matplotlib issue
            # https://github.com/matplotlib/matplotlib/issues/17130
        ...
        if parent.axis_equal:
            ...
            if isinstance(ax, Axes3D):
                ...
                zl, zh = 0, 0
            plot_radius = 0.5 * max([xh - xl, yh - yl, zh - zl])
            if isinstance(ax, Axes3D):
                ax.set_xlim3d(
                    [(xl + xh) / 2 - plot_radius, (xl + xh) / 2 + plot_radius]
                )
                ax.set_ylim3d(
                    [(yl + yh) / 2 - plot_radius, (yl + yh) / 2 + plot_radius]
                )
                ax.set_zlim3d(
                    [(zl + zh) / 2 - plot_radius, (zl + zh) / 2 + plot_radius]
                )
            else:
                ax.set_xlim([(xl + xh) / 2 - plot_radius, (xl + xh) / 2 + plot_radius])
                ax.set_ylim([(yl + yh) / 2 - plot_radius, (yl + yh) / 2 + plot_radius])
...
def scatter(*args, show=True, **kwargs):

    np = import_module("numpy")

    args = [[arg] if (not type(arg) == list) else arg for arg in args]
    args = [
    ...
    if len(args) == 2:
        ...
        series = Scatter3dSeries(*args, **kwargs)
    else:
        raise f"Error! Expected 2 or 3 arguments for scatter, but got {len(args)}!"

    # axis_center is hack, since we want box around datapoints, not lines in center for scatter
    scatter_plot = Plot(series, axis_center=None, axis_equal=axis_equal, **kwargs)
    if show:
        scatter_plot.show()
    return scatter_plot
