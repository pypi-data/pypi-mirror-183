...
class ImpedanceComputation:
    ...
    def __init__(self, frequencies, main_bore, holes_valves=list(),
                 ...
                 # Physical options
                 player=None, temperature=None, losses=True, nondim=True,
        ...
        if compute_method == 'modal':
            # force use of radiation model with additional dof
            use_rad1dof=True
    ...
    def _check_kwargs(kwargs):
        ...
        keys_geom = list(inspect.signature(InstrumentGeometry).parameters.keys())
        keys_phy = list(inspect.signature(InstrumentPhysics).parameters.keys())
        keys_freq = list(inspect.signature(FrequentialSolver).parameters.keys())
        keys_mesh = list(inspect.signature(Mesh).parameters.keys())[1:] + ['nb_sub']
        keys_solve = list(inspect.signature(FrequentialSolver.solve).parameters.keys())[1:]
