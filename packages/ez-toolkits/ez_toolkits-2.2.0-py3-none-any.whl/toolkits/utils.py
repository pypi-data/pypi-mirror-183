...
def list_sort(data=None, key=None, deduplication=None, **kwargs):
    '''
    列表排序
...
def list_print_by_step(data=None, number=None, separator=None):
    ...
    try:
        ...
        _list_length = len(data)

        if _list_length == 0 or _list_length <= number:

            print(data)

        else:

            _index_list = list(range(0, _list_length, number))

            if _index_list[-1] != _list_length:
                _index_list.append(_list_length)

            for _index in _index_list:
                if _index_list[-1] != _index:
                    '''
                    for code in analysis_result[i:i + 10]:
                        if code == analysis_result[i:i + 10][-1]:
                            print(code)
                        else:
                            print(code, end='  ')
                    '''
                    print(*data[_index:_index + number], sep=separator)
