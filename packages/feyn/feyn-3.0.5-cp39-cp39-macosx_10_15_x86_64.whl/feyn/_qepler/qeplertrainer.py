...
def fit_models(
    ...
    qepler_models = [_qepler.Model(dnames=m.names, fnames=m.fnames, params=m.params, sample_count=m._sample_count, loss=loss) for m in models]
    ...
    def fitting_thread():
        ...
        while True:
            ...
            qm = qepler_models[ix]
            new_losses[ix], new_params[ix] = qm._fit(xarray=sampled_data, yarray=sampled_data[output], sample_weights=sample_weights)
...
def _resample_data(df: DataFrame, sample_weights, n_samples):
    ...
    for col in df.columns:
        ...
        elif hasattr(df[col].values, "_data") and isinstance(df[col].values._data, np.ndarray):
            data[col] = df[col].values._data
        else:
...
def _compute_scales(data: DataFrame):
    ...
    for col in data.select_dtypes(include=[np.bool8]).columns.values:
        in_scales[col] = {
            "scale": 1,
            "scale_offset": 0.0,
            "detect_scale": 0
