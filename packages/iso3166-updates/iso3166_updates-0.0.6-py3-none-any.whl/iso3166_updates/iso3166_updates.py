...
def get_updates(alpha2_codes, year=[''], export_filename="iso3166-updates",
        export_json_filename="iso3166-updates", export_folder="iso3166-updates", 
        export_json=True, export_csv=False):
    """
    ...
    #if single str input then convert to array 
    if isinstance(alpha2_codes, str):
        # alpha2_codes = [alpha2_codes]
        alpha2_codes = alpha2_codes.split(',')
    ...
    if (export_json):
        if (all_changes):
            ...
            with open(os.path.join(export_folder, export_json_filename + ".json"), "w") as write_file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-export_csv', '--export_csv', type=bool, required=False, default=0, help='Whether to export all found updates to csv files in export folder.')
    ...
    get_updates(alpha2_codes, year, export_filename, export_json_filename,
        export_folder, export_json, export_csv)