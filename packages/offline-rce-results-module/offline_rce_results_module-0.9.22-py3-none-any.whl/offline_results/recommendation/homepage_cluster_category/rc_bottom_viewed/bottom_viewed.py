...
class BottomViewed:
    ...
    def __init__(self):
        BottomViewed.graph = ANGraphDb.new_connection_config().graph

    @staticmethod
    @custom_exception()
    def get_bottom_viewed_df(user_label, user_content, user_cluster_mapping):
        try:
            ...
            content_homepage_mapping = (
                RecommendationUtils.get_label_wise_homepage_for_contents(
                    BottomViewed().graph, user_label, homepage_id_wise=True
                )
            )
            ...
            cluster_homepage_mapped_user_content_df = (
                BottomViewedUtils.get_inner_merged_df(
            ...
            bottom_viewed_df = (
                BottomViewedUtils.get_cluster_wise_bottom_viewed_homepages(
                    cluster_homepage_mapped_user_content_df
                )
            )
            ...
            bottom_viewed_df = BottomViewedUtils.add_created_on_attribute(
                bottom_viewed_df
            )
            bottom_viewed_df = BottomViewedUtils.add_recommendation_type_attribute(
                bottom_viewed_df, BOTTOM_VIEWED_MODULE_NAME.upper()
            )
            ...
            Logging.error(
                f"Error while preparing bottom-viewed homepage_id rec for {user_label}, Error: {e}"
            )

    @staticmethod
    @custom_exception()
    def get_bottom_viewed_homepage_id():
        try:
            ...
            for user_label in list_label:
                Logging.info(
                    f"PREPARING BOTTOM-VIEWED HOMEPAGE_ID REC FOR {user_label}"
                )
                output_df = BottomViewed.get_bottom_viewed_df(
                    user_label, user_content_df, user_cluster_mapping
                )
                key_prefix = (
                    SERVICE_NAME
                    + ":"
                    + BOTTOM_VIEWED_MODULE_NAME.lower()
                    + ":"
                    + user_label
                )
            ...
            bottom_viewed_pay_tv_homepage_id_json = (
                BottomViewedUtils.get_json_format_output(
            ...
            Logging.error(
                f"Error while preparing bottom-viewed homepage_id rec, Error: {e}"
            )
