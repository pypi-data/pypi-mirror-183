...
class MVUtils:
    ...
    def get_cluster_wise_most_viewed_contents(ubd):
        try:
            ...
            for cluster_id in unique_cluster_id:
                ...
                content_views_df = content_views_df.head(
                    MOST_VIEWED_CONTENTS_LIMIT
                ).reset_index(drop=True)
                content_views_df = RecommendationUtils().get_recommendation_scores(
                    content_views_df)
                cluster_wise_most_viewed_contents = pd.concat(
                    [cluster_wise_most_viewed_contents, content_views_df], axis=0
                ).reset_index(drop=True)
            cluster_wise_most_viewed_contents = (
                cluster_wise_most_viewed_contents.sort_values(
            ...
            Logging.error(
                f"Error while Finding cluster-wise most-viewed contents, Error: {e}"
            )
        ...
        try:
            ...
            for cluster_id in unique_cluster_id:
                ...
                for homepage_id in unique_homepage_id:
                    ...
                    content_views_df = RecommendationUtils().get_recommendation_scores(
                        content_views_df)
                    homepage_wise_most_viewed_contents = pd.concat(
                        [homepage_wise_most_viewed_contents, content_views_df], axis=0
                    ).reset_index(drop=True)
                homepage_wise_most_viewed_contents[CLUSTER_ID] = cluster_id
                cluster_homepage_wise_most_viewed_contents = pd.concat(
                    [
                        cluster_homepage_wise_most_viewed_contents,
                        homepage_wise_most_viewed_contents,
                    ],
                    axis=0,
                ).reset_index(drop=True)
            cluster_homepage_wise_most_viewed_contents = (
                cluster_homepage_wise_most_viewed_contents.sort_values(
            ...
            Logging.error(
                f"Error while Finding cluster-and-homepage-wise most viewed contents , Error: {e}"
            )
    ...
    def get_default_most_viewed_contents(ubd):
        try:
            ...
            most_viewed_contents = content_views_df.sort_values(
                [VIEW_COUNT], ascending=False
            ).reset_index(drop=True)
            most_viewed_contents = most_viewed_contents.head(
                MOST_VIEWED_CONTENTS_LIMIT
            ).reset_index(drop=True)
            most_viewed_contents = RecommendationUtils().get_recommendation_scores(
                most_viewed_contents)
            return most_viewed_contents
        except Exception as e:
            Logging.error(
                f"Error while Finding FALLBACK most-viewed contents, Error: {e}"
            )
    ...
    def get_default_homepage_wise_most_viewed_contents(ubd):
        try:
            ...
            homepage_wise_most_viewed_contents = (
                homepage_wise_most_viewed_contents.sort_values(
