...
class CmdRepoCreate(CmdRepo):
    def run(self):
        ...
        result = subprocess.run(["gh", "repo", "create", "{}".format(repository_name), "--private", "--clone" ], capture_output=True, shell=True, env=dict(os.environ, GH_TOKEN=os.environ.get("GH_TOKEN")))
        print(result.stderr)
        print(result.stdout)
        ...
        result = subprocess.run(["dvc init"], capture_output=True, shell=True, cwd=repository_name, shell=True)
        print(result.stderr)
        print(result.stdout)
        ...
        result = subprocess.run(["dvc",  "remote", "add", "-d", "minio", "s3://{}".format(repository_name), "-f"], capture_output=True, shell=True, cwd=repository_name)
        print(result.stderr)
        print(result.stdout)
