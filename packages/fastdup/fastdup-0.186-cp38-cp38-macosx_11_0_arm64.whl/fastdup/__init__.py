...
def find_similarity_file(similarity_file, input_dir, work_dir, kwargs):
    ...
    if isinstance(similarity_file, pd.DataFrame):
        ...
        if similarity_file.endswith('.csv'):
            hierarchical_run = 'hierarchical' in similarity_file
            ...
            kwargs['hierarchical_threshold'] = os.path.basename(similarity_file).split('_')[-1].split('.')[0]
...
def create_duplicates_gallery(similarity_file, save_path, num_images=20, descending=True,
   ...
   '''
    ...
    try:
        ...
        similarity_file, input_dir = find_similarity_file(similarity_file, input_dir, work_dir, kwargs)
...
def create_duplicate_videos_gallery(similarity_file, save_path, num_images=20, descending=True,
   ...
   '''
    ...
    try:
        ...
        df, input_dir = find_similarity_file(similarity_file, input_dir, work_dir, kwargs)
...
def create_component_videos_gallery(work_dir, save_path, num_images=20, lazy_load=False, get_label_func=None,
    ...
    try:
        start_time = time.time()
        kwargs['is_video'] = True
        df, input_dir = find_similarity_file(work_dir, input_dir, work_dir, kwargs)
        df = remove_duplicate_video_distances(df, kwargs)
        if df is None:
            return 1
