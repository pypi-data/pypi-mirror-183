...
def extract_filenames(row, work_dir = None, save_path=None, kwargs = None):
    debug_hierarchical = kwargs is not None and 'debug_hierarchical' in kwargs and kwargs['debug_hierarchical']
    if kwargs is not None and 'hierarchical_run' in kwargs and kwargs['hierarchical_run']:
...
def prepare_hierarchy(df, work_dir, save_path, debug_hierarchical):
    ...
    return df, comp_images, comp_map
...
def do_create_duplicates_gallery(similarity_file, save_path, num_images=20, descending=True,
    ...
    print('hierarchical_run', hierarchical_run) # todo
    ...
    if 'hierarchical_run' in kwargs and kwargs['hierarchical_run']:
        ...
        df, comp_images, comp_map = prepare_hierarchy(df, work_dir, save_path, debug_hierarchical)
...
def load_one_image_for_outliers(args):
    ...
    impath1, impath2, dist, ptype = extract_filenames(row, work_dir, save_path)
    try:
...
def visualize_top_components(work_dir, save_path, num_components, get_label_func=None, group_by='visual', slice=None,
    ...
    for i,row in tqdm(top_components.iterrows(), total = len(top_components)):
        try:
            ...
            print('saving comp image to ', local_file) #todo remove
