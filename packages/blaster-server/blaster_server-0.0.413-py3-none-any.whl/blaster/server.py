...
class App:
	...
	# all dev specific headers go ito this
	def send_dev_headers(self, buffered_socket, request_headers):
		if(DEV_FORCE_ACCESS_CONTROL_ALLOW_ORIGIN):
			buffered_socket.send(
				b'Access-Control-Allow-Origin: ', 
				request_headers.get("origin", "*"), b'\r\n'
			)
	...
	def process_http_request(self, buffered_socket):
		...
		try:
			...
			if(response_headers or (body != I_AM_HANDLING_THE_SOCKET)):
				...
				if(IS_DEV):
					self.send_dev_headers(buffered_socket, headers)
			...
			if(IS_DEV):
				self.send_dev_headers(buffered_socket, headers or {})
