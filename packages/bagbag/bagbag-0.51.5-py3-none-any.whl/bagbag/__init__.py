...
if None not in [Os.Getenv("MATRIX_API_HOST"), Os.Getenv("MATRIX_API_PASS"), Os.Getenv("MATRIX_API_ROOM")]:
    ...
    vWR0AQ68tikimG50 = Tools.MatrixBot(Os.Getenv("MATRIX_API_HOST"), Os.Getenv("MATRIX_API_PASS")).SetRoom(Os.Getenv("MATRIX_API_ROOM"))
    fname = Os.Path.Basename(inspect.stack()[-1].filename)
    
    vWR0AQ68tikimG50.Send(msg + fname + " started")

    atexit.register(vWR0AQ68tikimG50.Send, msg + fname + " exit")