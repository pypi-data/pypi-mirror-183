...
class PassingAdjusted():
    ...
    def parse_passing_data(self, passing_data):
        """Sets the adjusted passing data in the object
    ...
    def get_wins(self):
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['wins'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['wins'].values[0])
        except ValueError as e:
    ...
    def get_losses(self):
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['losses'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['losses'].values[0])
        except ValueError as e:
    ...
    def get_ties(self):
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['ties'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['ties'].values[0])
        except ValueError as e:
    ...
    def get_passing_yards_per_attempt_index(self):
        """Get method for the index of the passing yards per attempt
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_yds_per_att_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_yds_per_att_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the passing yards per attempt.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_net_passing_yards_per_attempt_index(self):
        """Get method for the index of the net passing yards per attempt
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_net_yds_per_att_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_net_yds_per_att_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the net passing yards per attempt.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_adjusted_passing_yards_per_attempt_index(self):
        """Get method for the index of the adjusted passing yards per attempt
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_adj_yds_per_att_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_adj_yds_per_att_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the adjusted passing yards per attempt.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_adjusted_net_passing_yards_per_attempt_index(self):
        """Get method for the index of the adjusted net passing yards per attempt
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_adj_net_yds_per_att_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_adj_net_yds_per_att_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the adjusted net passing yards per attempt.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_completion_percentage_index(self):
        """Get method for the index of the completion percentage
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_cmp_perc_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_cmp_perc_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the completion percentage.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_touchdown_percentage_index(self):
        """Get method for the index of the touchdown percentage
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_td_perc_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_td_perc_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the touchdown percentage.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
    
    def get_interception_percentage_index(self):
        """Get method for the index of the interception percentage
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_int_perc_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_int_perc_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the interception percentage.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_sacked_percentage_index(self):
        """Get method for the index of the sacked percentage
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_sacked_perc_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_sacked_perc_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the sacked percentage.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
        
    def get_passer_rating_index(self):
        """Get method for the index of the passer's rating
        ...
        """
        try:
            return_value = self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_rating_index'].values
            if(len(return_value) > 1):
                return return_value
            else:
                return int(self.__passing_stats[self.__passing_stats['year'] == self.__year]['pass_rating_index'].values[0])
        except ValueError as e:
            error_message = ("Invalid value for the index of the passer's rating.\nYear:{0}\n\nException:{1}".format(self.__year, e))
            self.__logging(error_message)
            raise ValueError("{0}{1}".format(self.__generic_error_message, e))
