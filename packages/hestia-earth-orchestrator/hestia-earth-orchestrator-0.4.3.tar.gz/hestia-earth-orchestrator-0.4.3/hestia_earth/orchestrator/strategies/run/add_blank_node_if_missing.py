...
def should_run(data: dict, model: dict):
    ...
    run = (
        is_empty or (len(run_args.keys()) > 0 and all([v for _k, v in run_args.items()]))
    ) and _run_required(data, model.get('model'), term_id)

    logShouldRun(data, model.get('model'), term_id, run, key=key, value=term_id, is_empty=is_empty, **run_args)
