...
def main():
    ...
    wbi_config['MEDIAWIKI_API_URL'] = input("MEDIAWIKI API URL: ")# data['MEDIAWIKI_API_URL']
    wbi_config['SPARQL_ENDPOINT_URL'] = input("SPARQL ENDPOINT URL: ") # data['SPARQL_ENDPOINT_URL']
    wbi_config['WIKIBASE_URL'] = input("WIKIBASE URL: ") # data['WIKIBASE_URL']
    # login_instance = wbi_login.Login(user=data['USER'], password=data['PASSWORD'])
    login_instance = wbi_login.Login(user=input("USER: "), password=input("PASSWORD: "))

    # wbi_config['MEDIAWIKI_API_URL'] = input("MEDIAWIKI_API_URL: ")# data['MEDIAWIKI_API_URL']
    # wbi_config['SPARQL_ENDPOINT_URL'] = input("SPARQL_ENDPOINT_URL: ") # data['SPARQL_ENDPOINT_URL']
    # wbi_config['WIKIBASE_URL'] = input("WIKIBASE_URL: ") # data['WIKIBASE_URL']
    # # login_instance = wbi_login.Login(user=data['USER'], password=data['PASSWORD'])
    # login_instance = wbi_login.Login(user=input("USER: "), password=input("PASSWORD: "))
    ...
    try:
        ...
        def recImport(url, level):
            try:
                newGraph = Graph()
                newGraph.parse(url)
                g.parse(url)
                if level >= 1:
                    for s, p, o in newGraph.triples((OWL.Ontology, OWL.imports, None)):
                        recImport(o, level - 1)
                        print("imported:" + str(o))
            except Exception as recE:
                print(recE)


        if args.recursive:
            recDepth = int(args.recursive)
            for s, p, o in g.triples((OWL.Ontology, OWL.imports, None)):
                if recDepth >= 1:
                    recImport(o, recDepth - 1)
                    print("imported:" + str(o))




        print("Total triples to be imported: " + str(len(g)))
    ...
    for ontElement, elParams in ontElements.items():
        ...
        if len(sparqlResponse) > 0:
            ...
            mappingList.append([elParams["label"], elParams["reprID"]])
        else:
            if(elParams["type"] == "property"):
    ...
    if len(sparqlResponses) > 0:
        ...
        mappingList.append(["The " + oName + " ontology", newItem.id])
    ...
    def coinProp(el, dt, label):
        ...
        mappingList.append([label, dictEl["reprID"]])
        ...
        for el in [s,p,o]:
            ...
            if((isinstance(el, URIRef) or isinstance(el, BNode)) and not el in mapping):
                ...
                try:
                    ...
                    mappingList.append([genPrefixedURI(el, namespaces.items()), mapping[el]["Item"]["reprID"]])
                except Exception as e:
                    raise
    ...
    if(args.mapping):
        try:
            with open(args.mapping, 'w') as f:
                ...
                write = csv.writer(f)
                write.writerow(["Original value", "Representation in Wikibase"])
                write.writerows(mappingList)
