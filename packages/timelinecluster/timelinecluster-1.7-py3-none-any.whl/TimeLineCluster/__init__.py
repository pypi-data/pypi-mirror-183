...
class TimeLineClusterClass:
            ...
            if(self.status):
                ...
                if(len(source_data) > 0): # check source data for process
                    ...
                    for pk in self.pk_report_name:
                        for_drop_report.append(pk)
                    
                    for_drop_source = for_drop_report + [self.pk_report_start_date]
                    unique_source_data = source_data.drop_duplicates(for_drop_source) # duplicates source data for process
                    ...
                    for i , source in enumerate(unique_source_data.iloc): # self.pk_report_name, self.pk_report_start_date
                        ...
                        if(self.generic.process_status == False):
                            self.genLogErrorPk(last_update_source[self.generic.data_app_config["field_last_update_timestamp"]], for_drop_report, source)
                            return
                    ...
                    self.writeFileUpdate(last_update_source[self.generic.data_app_config["field_last_update_timestamp"]])
    ...
    def writeFileUpdate(self, last_update):
        if("field_last_update_timestamp" in self.generic.data_app_config):
            UtilsController.writeLastUpdateSurveyJson(self.store_for_time_line_cluster["last_update_path"] , last_update)

    def genLogErrorPk(self, last_update, for_drop_report, source):
        self.writeFileUpdate(last_update)
        self.generic.log.text_long += " | update_timestamp : " + str(last_update) + " "
