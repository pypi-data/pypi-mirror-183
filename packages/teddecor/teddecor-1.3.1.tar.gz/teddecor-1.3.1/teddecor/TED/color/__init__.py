...
from teddecor.TED.markup import TED
...
def _parse_color(color: COLOR, spec: str = "F"):
    ...
    if color is not None:
        if isinstance(color, str):
            ...
            code = f"@{spec} {','.join(Color.RGB(*color))}"
    return code
    ...
    *string,
    ...
    if isinstance(function, str):
        ...
        if len(function) == 2 and isinstance(function[0], str) and callable(function[1]):
            TED.define(function[0], function[1])
...
str and one Callable")
    ...
    # Parse with TED and return
    return TED.parse(
        f"[{' '.join(attr for attr in macro if attr.strip() != '')}]{head}{TED.escape(value)}"
    )
