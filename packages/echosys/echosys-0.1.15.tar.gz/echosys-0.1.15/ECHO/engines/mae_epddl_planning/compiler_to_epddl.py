...
def pred(_predicate: Predicate, prob=False) -> str:
    ...
    elif isinstance(_predicate, Forall):
        if _predicate.disequality_predicate is None:
            who = predicate_argument(_predicate.who)
        else:
            who = pred(_predicate.disequality_predicate)
        if _predicate.when is None:
            when = ''
        else:
            when = '(' + pred(_predicate.when) + ')'
        return f'forall ' + '(' + who + ')' + when

    elif isinstance(_predicate, disequality_predicate):
