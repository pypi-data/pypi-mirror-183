...
def app(request):
    ...
    if hasattr(request.cls, 'database_name'):
        ...
        app.config['SQLALCHEMY_DATABASE_URI'] =\
            'postgresql://postgres:@{}/{}'\
            .format(POSTGRES_HOST, request.cls.database_name)
...
def db(app, request):
    ...
    if 'scenario' in request.fixturename:
...
def rhodb(request):
    ...
    if request.param == 'postgres':
        ...
        os.environ['SQLALCHEMY_DATABASE_URI'] =\
            'mysql://root@{}/{}'.format(POSTGRES_HOST, request.cls.database_name)
    else:
...
def Person(db, request):
    ...
    with patch(import_obj, side_effect=import_mock):
        ...
        class Person(BaseWithIntPK, db.Model):
            ...
            __serializable_columns__ = [
                'id', 'given_name', 'family_name', 'birth_date'
            ]
