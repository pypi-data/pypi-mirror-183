...
class Base(object):
    ...
    changed_datetime = Column(DateTime,
                              default=func.now(),
    ...
    def _column_type(cls, kls, field):
        ...
        try:
            ...
            logger.info("Couldn't get property type on field {0}.{1}".format(
                kls, field))
    ...
    def transform_datetime_fields(cls, d):
        ...
        for field, field_value in iteritems(d):
            ...
            if 'datetime' in field:
                ...
                field_value = datetime.timedelta(seconds=field_value)
            d[field] = field_value
            ...
            if field_value is not None:
                try:
                    ...
                    logger.info("Couldn't set attribute {0} on object {1} to "
                                "{2}".format(field, entity,
                                             field_value))  # pragma: no cover
                except Exception as e:  # pragma: no cover
                    logger.exception(e)  # pragma: no cover
        return entity
    ...
    def _primary_key_columns(self):
        ...
        """
        return [
            a.key for a in self.__mapper__.primary_key
    ...
    def _columns(self):
        ...
        """
        return [
            a.key for a in self.__mapper__.attrs
    ...
    def to_dict(self, string_dates=True, extra_columns=None):
        ...
        retval = dict((k, getattr_plus(k)) for k in chain(cols, extra_columns))
        return retval
...
def run_db_commands(cmds, database, dialect):
    ...
    if dialect == 'postgres':
        ...
        import MySQLdb
        conn = MySQLdb.connect(host=POSTGRES_HOST,
                               user='root',
                               db=database,
                               autocommit=True)
    else:
...
def drop_database(dbname, dialect='postgres'):
    ...
    :param dbname: The name of the database.
    """
    run_db_commands('DROP DATABASE IF EXISTS {0}'.format(dbname), dialect,
                    dialect)
