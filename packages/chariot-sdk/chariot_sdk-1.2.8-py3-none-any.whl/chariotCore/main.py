...
def cmdline():
    ...
    parser.add_argument("-g", "--generate", help="生成插件，需要yaml或json类型的插件定义文件路径参数，建议每次改动插件定义文件时重新生成一次插件。注意，重新生成插件会覆盖不同组件的models.py校验文件。使用示例：chariot-plugin -g .\plugin.spec.yaml", action="append")
    parser.add_argument("-ag","--auto_generate",help="自动生成高度模板化插件，需要特殊格式的插件定义文件路径，此方法一般自动调用。",action="append")
