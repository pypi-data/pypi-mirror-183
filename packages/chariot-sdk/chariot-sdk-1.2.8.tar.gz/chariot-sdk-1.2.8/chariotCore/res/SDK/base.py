    ...
    try:
        if config_data:
            ...
            if config_data.get("log",{}).get("debug",False):
                logging.basicConfig(level=logging.DEBUG,format="[%(asctime)s] %(levelname)s\n  %(message)s",datefmt="%Y-%m-%d %H:%M:%S",force=True)
...
def transpondData(data:dict={},needCheck:bool=True,outputModel:BaseModel=None,dispatcher_url=""):
    """
    #   转发数据
    参数说明：
    data:dict,  #   需要转发的信息
