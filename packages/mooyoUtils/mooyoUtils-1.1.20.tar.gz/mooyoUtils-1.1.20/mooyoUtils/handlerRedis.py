...
class RedisHandler:
    ...
    def __init__(self, host, port, password, db, *args, **kwargs):
        ...
        self.rds = redis.Redis(connection_pool=self.pool)
        self.pipe = self.rds.pipeline()
        self.support_type = ['string', 'hash', 'list', 'set', 'zset', 'json']
    ...
    def exec(self):
        ...
        for command in commands:
            ...
            try:
                ...
                value = eval(rds_command)
                if path:
                    value = jsonpath(json.loads(value), expr='$..%s' % path)[0]
...
if __name__ == '__main__':
    config = {
        ...
        'commands': [
            ...
            {'type': 'string', 'operate': 'get', 'name': 'jackie', "path": "name"},
            ...
            {'type': 'json', 'operate': 'get_test', 'name': 'jackie'},
        ],
    }
