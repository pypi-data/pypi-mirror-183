...
def get_tianshou_envs(make_env, args):
    ...
    env = make_env(mode='training', args=args)
    ...
    return env, train_envs, train_envs2, test_envs
...
def get_encoder_and_decoder(dim, hidden_dim, z_dim):
    import numpy as np
    import torch
    import torch.nn as nn
    ...
    if isinstance(dim, tuple) and len(dim) == 3:  # img
        ...
        if len(dim) == 1:
            dim = dim[0]
        enc = MLP(input_dim=dim, output_dim=2 * z_dim, hidden_sizes=[hidden_dim] * 3)
