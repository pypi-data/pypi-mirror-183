...
phylum_lists = ['trading', 'direct', 'characteristic', 'feature', 'bond', 'financial', 'consensus', 'ths', 'industry']
class_lists = ['asharecalendar', 'ashareeodprices', 'ashareeodderivativeindicator', 'asharettmhis',
               ...
               'cbondcurvecnbd', 'cbondindex', 'extract', 'characteristic_exposure',
               'characteristic_return', 'characteristic_covariance', 'characteristic_idiosyncratic_variance',
...
class Checker(object):
    ...
    def check_params(cls, client_params):
        ...
        if client_params["class"].find('ths') >= 0:
            pass
        ...
        if client_params['class'] not in class_lists:
            if client_params['phylum'] == 'direct' or client_params['domain'] == 'sheet':
                pass
            else:
