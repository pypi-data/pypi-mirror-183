...
class CalculateAccuracy:
    ...
    def calculate_qinghai_day_2022(self):
        '''
            2022年10月13日青海调度每日通报 编制公式

            早高峰为 06:00-07:30
            中午低谷为 11:00-15:00
            晚高峰为 18:00-22:00

            准确率 = 1- abs(预测电量-实际电量）/实际电量
            偏差 = sum(abs(预测电量-实际电量））


            返回每日短期偏差积分电量
            FBI WARING
        '''
        df_qinghai = pandas.DataFrame([], columns=['time', 'power_real', 'power_forecast'])
        df_qinghai['time'] = self.date_time
        df_qinghai['power_real'] = self.power_real
        df_qinghai['power_forecast'] = self.power_forecast
        df_qinghai = df_qinghai[df_qinghai['power_real'] >= 0]
        df_qinghai = df_qinghai[df_qinghai['power_forecast'] >= 0]
        series_time = df_qinghai['time']
        df_qinghai['hour'] = series_time.str[11:16]
        if len(df_qinghai) > 0:
            # 取列
            dfs_time = df_qinghai['time']
            power_real = df_qinghai['power_real']
            power_forecast = df_qinghai['power_forecast']
            ymd = dfs_time.str[0:10]  # 选取 yyyy-mm-dd
            df_hour = dfs_time.str[11:16]  # pandas series
            cap = self.capacity

            morning_index = numpy.where(
                numpy.logical_and(df_hour >= '06:00', df_hour <= '07:30'))

            noon_index = numpy.where(
                numpy.logical_and(df_hour >= '11:00', df_hour <= '15:00'))

            evening_index = numpy.where(
                numpy.logical_and(df_hour >= '18:00', df_hour <= '22:00'))
            if sum(power_real)==0:
                day_acc_generating = 0
                day_sum_generating = abs(sum(power_forecast) - sum(power_real))
            else:
                day_acc_generating = 1 - abs(sum(power_forecast)-sum(power_real))/sum(power_real)  # 全天发电量准确率
                if day_acc_generating < 0:
                    day_acc_generating = 0
                day_sum_generating = abs(sum(power_forecast)-sum(power_real))  # 全天发电量偏差

            if sum(power_real[morning_index[0]]) ==0:
                morning_acc_generating = 0
                morning_sum_generating = abs(sum(power_forecast[morning_index[0]]) - sum(power_real[morning_index[0]]))
            else:
                morning_acc_generating = 1 - abs(sum(power_forecast[morning_index[0]])-sum(power_real[morning_index[0]]))/sum(power_real[morning_index[0]])
                if morning_acc_generating < 0:
                    morning_acc_generating = 0
                morning_sum_generating = abs(sum(power_forecast[morning_index[0]])-sum(power_real[morning_index[0]]))

            if sum(power_real[noon_index[0]]) ==0:
                noon_acc_generating = 0
                noon_sum_generating = abs(sum(power_forecast[noon_index[0]]) - sum(power_real[noon_index[0]]))
            else:
                noon_acc_generating = 1 - abs(sum(power_forecast[noon_index[0]])-sum(power_real[noon_index[0]]))/sum(power_real[noon_index[0]])
                if noon_acc_generating < 0:
                    noon_acc_generating = 0
                noon_sum_generating = abs(sum(power_forecast[noon_index[0]])-sum(power_real[noon_index[0]]))

            if sum(power_real[evening_index[0]]) ==0:
                evening_acc_generating = 0
                evening_sum_generating = abs(sum(power_forecast[evening_index[0]]) - sum(power_real[evening_index[0]]))
            else:
                evening_acc_generating = 1 - abs(sum(power_forecast[evening_index[0]])-sum(power_real[evening_index[0]]))/sum(power_real[evening_index[0]])
                if evening_acc_generating < 0:
                    evening_acc_generating = 0
                evening_sum_generating = abs(sum(power_forecast[evening_index[0]])-sum(power_real[evening_index[0]]))
        return morning_acc_generating, morning_sum_generating, noon_acc_generating, noon_sum_generating, \
               evening_acc_generating, evening_sum_generating, day_acc_generating, day_sum_generating