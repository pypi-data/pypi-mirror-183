    ...
    def _postprocess_logic_test(
        ...
        try:
            if self.model.postprocess_logic is not None:
                ...
                Handler = LogicConverter(user_defined_bytes=self.model.postprocess_logic).BytesToFunction(
...
class EurekaExpModelTest:
    ...
    def get_secrets(self, path, parse_data=True):
    ...
    def generate_configs(self, env, user):
        ...
        if env == "prd":
            mlsenv = MLSENV.PRD
        secrets = self.get_secrets(path="mls")
        config = dict(
            env=mlsenv,
    ...
    def get_mls_config(self, env, user):
        import concurrent.futures

        configs = self.generate_configs(env=env, user=user)
        e = concurrent.futures.ThreadPoolExecutor(max_workers=len(configs) + 1)
        fs = [e.submit(self.check_client_config, conf) for conf in configs]
    ...
    def get_mls_ml_model_client(self, env="stg", user="aa2"):
        from sktmls.models import MLModelClient

        config = self.get_mls_config(env, user)
        return MLModelClient(**config)

    def get_mls_experiment_client(self, env="stg", user="aa2"):
        from sktmls.experiments import ExperimentClient

        config = self.get_mls_config(env, user)
        return ExperimentClient(**config)

    def get_mls_profile_api_client(self, env="stg", user="aa2"):
        from sktmls.apis.profile_api import MLSProfileAPIClient

        config = dict({conf for conf in self.get_mls_config(env, user).items() if conf[0] in ["env", "runtime_env"]})
        return MLSProfileAPIClient(**config, **self.get_secrets("mls").get("reco_api"))

    def get_mls_recommendation_api_client(self, env="stg", user="aa2"):
        from sktmls.apis.recommendation_api import MLSRecommendationAPIClient

        config = dict({conf for conf in self.get_mls_config(env, user).items() if conf[0] in ["env", "runtime_env"]})
        return MLSRecommendationAPIClient(**config, **self.get_secrets("mls").get("reco_api"))

    def test_model_connect_experiment(self, prediction_type="online"):
        model_client = self.get_mls_ml_model_client(env=self.env, user=self.user)
        model = model_client.get_manual_model(name=self.model_name, version=self.model_version)

        experiment_client = self.get_mls_experiment_client(env=self.env, user=self.user)
        ...
        recommendation_api_client = self.get_mls_recommendation_api_client(env=self.env, user=self.user)
        profile_api_client = self.get_mls_profile_api_client(env=self.env, user=self.user)
