...
class K6_PulseCls:
	"""K6_Pulse commands group definition. 968 total commands, 15 Subgroups, 0 group commands"""
	...
	def riq(self):
		...
		if not hasattr(self, '_riq'):
			from .Riq import RiqCls
			self._riq = RiqCls(self._core, self._cmd_group)
		return self._riq
	...
	@property
	def sense(self):
		"""sense commands group. 17 Sub-classes, 0 commands."""
	...
	def clone(self) -> 'K6_PulseCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = K6_PulseCls(self._core, self._cmd_group.parent)
