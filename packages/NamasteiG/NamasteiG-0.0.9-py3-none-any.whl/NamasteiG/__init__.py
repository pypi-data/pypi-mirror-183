...
class Instagram:
    ...
    def Login(self):
        ...
        data = {
            'signed_body': 'SIGNATURE.{"jazoest":"'+str(generate_jazoest(str(IgFamilyDeviceId)))+'","country_codes":"[{\\"country_code\\":\\"1\\",\\"source\\":[\\"default\\"]},{\\"country_code\\":\\"91\\",\\"source\\":[\\"sim\\"]}]","phone_id":"'+str(IgFamilyDeviceId)+'","enc_password":"#PWD_INSTAGRAM:4:'+str(round(time.time()))+':'+str(password_encrypt(self.passw))+'","username":"'+str(self.user)+'","adid":"'+str(uuid.uuid4())+'","guid":"'+str(IgDeviceId)+'","device_id":"'+str(AndroidID)+'","google_tokens":"[]","login_attempt_count":"0"}',
        }
