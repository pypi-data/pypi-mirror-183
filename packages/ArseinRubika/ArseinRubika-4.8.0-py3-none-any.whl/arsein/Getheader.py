...
from arsein.PostData import method_Rubika,httpfiles
...
class Upload:
    ...
    def uploadFile(self, file):
        if not "http" in file:
            ...
            REQUES = self.methodUpload.methodsRubika("json",methode ="requestSendFile",indata = {"file_name": str(file.split("/")[-1]),"mime": file.split(".")[-1],"size": Path(file).stat().st_size},wn = clien.web).get("data")
