    ...
    async def sell_auto(self, price_map):
        ...
        for row in json.loads(self.strategy_df.to_json(orient='records')):
            try:
                ...
                if abs((buy_price - price) / price) > 0.0006:
                ...
                elif abs((buy_price - price) / price) > 0.0004:
                    logger.info(f"buy price {buy_price} vs {price}")
                    # self.sell_market(row['id'], symbol, amount)
                    self.sell_limit(row['id'], symbol, amount, buy_price * 1.0003)
                elif time.time() * 1000 - timestamp > 10 * 60 * 1000 and (price - buy_price) > 1.0001:
