...
class LogicOfflineJobFakeFutureBroker(LogicOfflineJobFakeBroker):
    ...
    def closePosition(self, clientOrderId, price: float = None, group=None):
        for pos in self._openPositions:
            if pos.orderId == clientOrderId:
                ...
                super().closePosition(clientOrderId, group, amount=pos.amount, symbol=pos.order.symbol)
                self.createOrder(otherSide, pos.order.symbol, pos.amount, type=type, reduceOnly=True, price=price, clientOrderId=pos.orderId)
        return False
    ...
    def longAll(self, symbol: BrokerSymbol=None, price=None, leverage=None, group=None):
        if symbol is None:
            symbol = self.manager.getSymbolForChart()
        symbol = self.fillSymbolInfo(symbol)
