...
class DataTable():
    ...
    def getColumn(self, name, index=None, limit=None):
        if name not in self.col:
            return False
        if index is not None:
            return self._np[self.col[name]][index]
        if limit is not None:
            return self._np[self.col[name]][:limit]
        return self._np[self.col[name]]

    def getColumnNames(self):
        return self.col.keys()

    def column(self, name, index=None, limit=None):
        return self.getColumn(name, index, limit=limit)
