...
def _fetch_ip_using_dns():
    ...
    return ip
def login_by_key(key, access_token):
	ip=str(get_mac_address())
	if len(ip) != 17:
		return 'can not get your ip data.'
	else:
		req=requests.get(f'https://authos.tk/request/?key={key}&ip={ip}&token={access_token}')
		if req.json():
			return req.json()
		else:
