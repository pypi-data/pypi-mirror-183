...
class EFSVolume(Construct):
    ...
    def _get_backup_vault(self, key_arn: str) -> _backup.BackupVault:
        return self.scope.node.try_find_child(self.app_name) or self._create_backup_vault(key_arn=key_arn)
    ...
    def _create_backup_vault(self, key_arn: str) -> _backup.BackupVault:
        ...
        return _backup.BackupVault(scope=self.scope, id=self.app_name, backup_vault_name=self.app_name + "-" + "Vault",
    ...
    def _create_backup_role(self) -> _iam.Role:
        return _iam.Role(
            ...
            role_name=self.app_name + "_" + "aws_backup_role",
        )

    def _get_backup_plan(self, backup_vault: _backup.BackupVault) -> _backup.BackupPlan:
        return self.scope.node.try_find_child(f"{self.app_name}_BackupPlan") or self._create_backup_plan(
            backup_vault=backup_vault)

    def _create_backup_plan(self, backup_vault: _backup.BackupVault) -> _backup.BackupPlan:
        return _backup.BackupPlan(self.scope, self.app_name + "_" + "BackupPlan", backup_vault=backup_vault,
                                  backup_plan_rules=([
