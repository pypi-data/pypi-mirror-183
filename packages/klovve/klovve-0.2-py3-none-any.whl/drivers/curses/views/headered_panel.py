...
class View(klovve.pieces.headered_panel.View, klovve.drivers.curses.View):
    ...
    def get_native(self, model, model_bind):
        ...
        def fddfdf():
            if model.state == model.State.BUSY:
                a = [viwid.widgets.busy_animation.BusyAnimation()]
            elif model.state == model.State.SUCCESSFUL:
                a = [viwid.widgets.label.Label(text="✓")]
            elif model.state == model.State.SUCCESSFUL_WITH_WARNING:
                a = [viwid.widgets.label.Label(text="TODO")]
            elif model.state == model.State.FAILED:
                a = [viwid.widgets.label.Label(text="×")]
            else:
                a = []
            indicbox.children = a
        ...
        @klovve.reaction(owner=box)
        def ddsfd():
            tsi.children = [x.view().get_native_stuff() for x in model.title_secondary_items]

        @klovve.reaction(owner=box)
        def ddd():
            hdlabel.text = model.title
        ...
        @klovve.reaction(owner=box)
        def fdddf():
            hdbox.background = model.title_label_background
...
#            else:
 #               title_label.set_css_classes([])
        ...
        @klovve.reaction(owner=box)
        def set_item():
            body_box.children = [model.body.view().get_native_stuff()] if model.body else []
