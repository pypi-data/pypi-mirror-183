...
class View(klovve.pieces.formatted_text.View, klovve.drivers.gtk.View):
    ...
    def get_native(self, model, model_bind):  # TODO scrolling?!
        text_view = self.gtk_new(klovve.drivers.gtk.Gtk.TextView, hexpand=True, vexpand=True, editable=False,
                                 cursor_visible=False, wrap_mode=klovve.drivers.gtk.Gtk.WrapMode.WORD)
...
#TODO
        ...
        def _text():
            xmldoc = xml.etree.ElementTree.fromstring("<body>"+model.text+"</body>")
            buf.set_text("")
            put(xmldoc)
