...
class ParamGenerator:
    ...
    async def result(cls, config) -> ResolvedParameters:
        ...
        try:
            ...
            LOG.debug(
                f'failed generate parameters for {config.test_name}, {ex}')
    ...
    async def _select_value(self, selector: Selector) -> dict:
        ...
        if not values:
            not_support_query = constraints[0].get('Behavior') == 'NotSupport'
            if not_support_query:
                raise IacerException(f'Parameter {key} is not support GetTemplateParameterConstraints.')
            prev_selector = selector.prev
            if not prev_selector:
                raise IacerException(error_msg)
            return await self._select_value(prev_selector)
