...
from iacer.config import TestConfig, IAC_OCTOPUS_NAME
...
SYS_TAGS = {'CreatedBy': f'{IAC_OCTOPUS_NAME}'}
...
class Stacker:
    ...
    def __init__(self,
                 ...
                 name_prefix: str = IAC_OCTOPUS_NAME,
                 tags: dict = None,
                 stacks: Stacks = None):
        self.tests = tests or []
        ...
        self._sys_tags = {
            f'{IAC_OCTOPUS_NAME}-id': self.uid,
            f'{IAC_OCTOPUS_NAME}-project-name': self.project_name,
        }
...
class Stack:
    ...
    async def create(cls, test: TestConfig, tags: dict = None, uuid: UUID = None) -> 'Stack':
        ...
        if not tags:
            tags = {}
        tags.update({f'{IAC_OCTOPUS_NAME}-test-name': name})
        region = test.region
        ...
        stack_name = f'{IAC_OCTOPUS_NAME}-{name}-{region}-{uuid4().hex[:8]}'
        try:
            ...
            stack_id = None
            stack = cls(region, stack_id, name, uuid, status_reason=ex.message, stack_name=stack_name)
