...
from pine.config.common_site_config import get_gunicorn_workers, update_config
...
def setup_redis_config(pine_info, pine_path):
	...
	pine_redis_socketio_template = pine.config.env().get_template(
		"systemd/pimelon-redis-socketio.service"
	)
	...
	pine_redis_socketio_config_path = os.path.join(
		pine_path,
		"config",
		"systemd",
		pine_info.get("pine_name") + "-redis-socketio.service",
	)
