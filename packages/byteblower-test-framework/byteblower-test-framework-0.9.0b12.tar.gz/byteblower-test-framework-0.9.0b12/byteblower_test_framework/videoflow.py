...
from .report.bufferanalyser import BufferAnalyser
...
class VideoFlow(TcpFlow):
    ...
    def __init__(
            ...
            segment_duration: Union[timedelta,
                                    float] = timedelta(seconds=2.500),
        ...
        self._bytes_per_ms = (self._segment_size /
                              self._segment_duration.total_seconds() / 1000
                              )  # video bitrate expressed in bytes/s
        bitrate = 8000 * self._bytes_per_ms
        logging.info("Video bitrate will be {BITRATE} MBit/s".format(
            BITRATE=bitrate / 1000000.0))
        buffering_goal_segments = (self._buffering_goal.total_seconds() /
        ...
        play_goal_bytes = int(
            ...
            self._segment_duration.total_seconds()) * self._segment_size
        self._bufferanalyser = BufferAnalyser(buffering_goal_bytes,
                                              play_goal_bytes)
    ...
    def _segment_download_required(self) -> bool:
        return (self._bufferanalyser.size + self._segment_size <
