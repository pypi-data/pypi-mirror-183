...
class utils():
    def simulation(self,_number,_nameof_msg_rpt, _namespace, _sincelast, _lst_patterns, _lst_target=[]):
        ...
        Q.logger(time7.currentTime7(),'      [ Begin simulation - {0} ] シミュレーション'.format(_number))
        ...
        Q.logger(time7.currentTime7(),'         - patterns :', _lst_patterns)
        Q.logger(time7.currentTime7(),'         - targets :', _lst_target)
        Q.logger(time7.currentTime7(),'      ', '.'*83)

        if len(_lst_target)!=0:
            for x in _lst_target:
                Q.logger(time7.currentTime7(),'       In Progress step (',x,')')
                time.sleep(1)
        Q.logger(time7.currentTime7(),'      ', '.'*83)
        Q.logger(time7.currentTime7(),'')
        Q.logger(time7.currentTime7(),'')
    ...
    def view_dictionary(self,lst_dict):
        try:
            if lst_dict:
                ...
                for row in lst_dict:
                    for record in row:
                        Q.logger(time7.currentTime7(),' '*3,record,':',row[record])
                    Q.logger(time7.currentTime7(),'-'*30)
                    time.sleep(1)
                Q.logger(time7.currentTime7(),'')
        except Exception as e:
        ...
        try:
            ...
            if allowArgs == True:
                ...
                if 'cip' in data[0].keys():
                    ...
                    Q.logger(time7.currentTime7(),'      Success get cip update : ({0})'.format(str(IntCipNow)))
                else:
    ...
    def pyairtable_detectCrashProcess_then_recovery(self, _lst_data, **kwargs):
        try:
            ...
            if '_argMaxSecondsTimeLogsWaiting' in kwargs:
                ...
                Q.logger(time7.currentTime7(),"      set _argMaxSecondsTimeLogsWaiting : ", str(maxSecondsTimeLogsWaiting))
            ...
            if len(_lst_data)!=0:
                ...
                for idx, r in  enumerate(_lst_data):
                    ...
                    Q.logger(time7.currentTime7(),'     -> check esc id :', r_id)
                    Q.logger(time7.currentTime7(),'     -> check esc status :', r_status)
                    Q.logger(time7.currentTime7(),'     -> check esc cip :', r_cip)
                    Q.logger(time7.currentTime7(),'     -> check esc last of log 1 :', r_lastOfLog1 )
                    Q.logger(time7.currentTime7(),'     -> check esc last of log 2 :', r_lastOfLog2 )
                    Q.logger(time7.currentTime7(),'     -> check esc last of log 3 :', r_lastOfLog3 )
                    ...
                    if len(lst_tLast_log)!=0 and int(r_cip)!=0 and str(r_status).strip()==const_status.InProgress.value:
                        ...
                        Q.logger(time7.currentTime7(),'     -> check start date :', r_start_date)
                        Q.logger(time7.currentTime7(),'     -> check max last of logs :', max_lastoflog)
...
class testing():
    ...
    def excute(self):
        ...
        if len(lst_data)!=0:
            ...
            ## skenario 5
            Q.logger(time7.currentTime7(),'[ Data is available ] 入手可能')
            ...
            Q.logger(time7.currentTime7(),'[ Data is unavailable ] 入手不可能')
