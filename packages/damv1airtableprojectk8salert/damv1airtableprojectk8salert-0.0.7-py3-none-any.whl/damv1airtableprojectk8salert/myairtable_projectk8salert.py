    ...
    def airtable_normalize_datafrom_loadAll_origin(self, _lst_data):
        ...
        try:
            for data in _lst_data:
                fdName = utils().airtable_escape_fields(data,'name','')
                fdTitle = utils().airtable_escape_fields(data,'title','')
                fdType = utils().airtable_escape_fields(data,'type','')
                fdNs = utils().airtable_escape_fields(data,'ns','')
                fdTargets = utils().airtable_escape_fields(data,'target contains','')     # dictionary values
                fdPatterns = utils().airtable_escape_fields(data,'patterns','')           # dictionary values
                fdEnable = utils().airtable_escape_fields(data,'Enable','')
                fdStatus = utils().airtable_escape_fields(data,'status','')
                fdStartDt = utils().airtable_escape_fields(data,'start date','')
                fdEndDt = utils().airtable_escape_fields(data,'end date','')
                fdDetected = utils().airtable_escape_fields(data,'detected','')           # dictionary values
                fdReport = utils().airtable_escape_fields(data,'report','')               # dictionary values
                fdMethod = utils().airtable_escape_fields(data,'exec method','')
                fdCip = utils().airtable_escape_fields(data,'cip','')
                fdLogLive1 = utils().airtable_escape_fields(data,'log live 1','')
                fdLogLive2 = utils().airtable_escape_fields(data,'log live 2','')
                fdLogLive3 = utils().airtable_escape_fields(data,'log live 3','')
...
#     # utils().simulation('Test title', 'test','24h',lst_patterns,target_podstr)
# excute()