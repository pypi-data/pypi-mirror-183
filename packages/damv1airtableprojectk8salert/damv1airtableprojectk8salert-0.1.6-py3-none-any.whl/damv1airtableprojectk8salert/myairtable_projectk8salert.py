...
class sandbox():
    ...
    def pyairtable_loadAll_by_OR_condition(self, _pattern):
        ...
        lst_output =[]
        print(time7.currentTime7(),'    - Loading airtable by OR contion on formula:')
        try:
            if len(_pattern)!=0:
                ...
                if r: 
                    lst_output = r
                    print(time7.currentTime7(),'      Successful load data')
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_loadAll_by_OR_condition"')    
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
        return lst_output
    ...
    def pyairtable__loadAll_by_rawformula(self, _raw):
        ...
        print(time7.currentTime7(),'    - Loading airtable by raw formula:')
        lst_output = []
        try:
            if _raw.strip()!= '':
                table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
            ...
            print(time7.currentTime7(),'Fail of function "pyairtable__loadAll_by_rawformula"')    
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
        return lst_output
...
### %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        try:
            ...
            if data:
                for row in data:
                    ...
                    table.delete(row['id'])
                print(time7.currentTime7(),'      Successful delete all data')
            else:
                print(time7.currentTime7(),'      Data is empty, abort delete')
            boolexecute = True
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_delete_all_rows"')    
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)    
        return boolexecute

    def pyairtable_create_batchrow(self):
        print(time7.currentTime7(),'    - Creating batch of row data:')
        boolexecute = False
        try:
            table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
            table.batch_create(\
            ...
            print(time7.currentTime7(),'      Successful create batch of row data')
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_create_batchrow"')    
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)    
        return boolexecute

    def pyairtable_loadAll_by_enable_ColParams(self, _enable=True):
        data = []
        print(time7.currentTime7(),'    - Loading airtable:')
        try:
            table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
            ...
            print(time7.currentTime7(),'Fail of function "pyairtable_loadAll_by_enable_ColParams"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
        return data
    ...
    def pyairtable_getFirstLstDict_by_name_and_enable(self, _table, _name, _Enable, _fields):
        ...
        try:
            ...
            print(time7.currentTime7(),'Fail of function "pyairtable_getFirstLstDict_by_name_and_enable"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
        return lst_data
        ...
        try:
            ...
            if len(lst_data)!=0:
                ...
                if '_argMaxThread' in kwargs:
                    ...
                    print(time7.currentTime7(),"      set _argMaxThread : ", str(maxThreadNumber))

                if id == str(_id).strip():
                    match _stepProcess:
                        case const_process.start.value: 
                            print(time7.currentTime7(),'      [ airtable - assign start process ]')
                            if cipInt<int(maxThreadNumber) : cipInt = cipInt + 1
                            field_date = 'start date'
                        case const_process.end.value: 
                            print(time7.currentTime7(),'      [ airtable - assign end process ]')
                    ...
                    print(time7.currentTime7(),'        successful assign ')
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_update_StartEnd_process"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
    ...
    def pyairtable_append_detected_and_report_process(self, _id, _name, _Enable, _sumDetected, _urlShareable):
        try:
            ...
            if len(lst_data)!=0:
                ...
                print(time7.currentTime7(),'      [ airtable - assign detected and report ] ')
                id = str(utils().escape_dict(lst_data[0],'id')).strip()
            ...
            print(time7.currentTime7(),'Fail of function "pyairtable_append_detected_and_report_process"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)

    def pyairtable_threadUpdate_lastoflog(self, _id, _numThread, _valueForUpdate = time7.currentTime7(), **kwargs):
        try:
            print(time7.currentTime7(),'    - Update last of log fields airtable:')
            table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
            allowArgs = False
            maxThreadNumber = 3 # Default ( デフォルト )
            if '_argMaxThread' in kwargs:
            ...
            if allowArgs == True:
                ...
                print(time7.currentTime7(),'      Success for update')
            else:
                print(time7.currentTime7(),'      Perboden Arguments')
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_threadUpdate_lastoflog"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
    
    def pyairtable_update_clearAll_lastoflog(self, _id, **kwargs):
        try:
            print(time7.currentTime7(),'    - Update clear all last of log fields airtable:')
            table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
            allowArgs = False
            maxThreadNumber = 3 # Default ( デフォルト )
            if '_argMaxThread' in kwargs:
            ...
            if allowArgs == True:
                for i in range(1,maxThreadNumber+1):
                    ...
                    table.update(_id,paramUpdate)
                print(time7.currentTime7(),'      Success for clear all update')
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_update_clearAll_lastoflog"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)

    def pyairtable_update_clearOneSelection_lastoflog(self, _id, _numThread, **kwargs):
        try:
            print(time7.currentTime7(),'    - Update clear one selection from last of log fields airtable:')
            table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
            allowArgs = False
            maxThreadNumber = 3 # Default ( デフォルト )
            if '_argMaxThread' in kwargs:
            ...
            if allowArgs == True:
                ...
                table.update(_id,paramUpdate)
                print(time7.currentTime7(),'      Success for clear one update')

        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_update_clearOneSelection_lastoflog"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)

    def pyairtable_getIntCip_now(self, _id, **kwargs):
        IntCipNow = 0
        try:
            print(time7.currentTime7(),'    - Get Integer Cip (Count in process) now:')
            allowArgs = False
            maxThreadNumber = 3
            if str(_id).strip()!= '':
                allowArgs = True
            ...
            if allowArgs == True:
                ...
                if 'cip' in data[0].keys():
                    ...
                    print(time7.currentTime7(),'      Success get cip update : ({0})'.format(str(IntCipNow)))
                else:
                    print(time7.currentTime7(),'      Not found cip key')

        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_getIntCip_now"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
        return IntCipNow
    ...
    def pyairtable_getFields_FirstThreadNumberAvailable(self, _id, **kwargs):
        ...
        try:
            print(time7.currentTime7(),'    - Get info first threads available:')
            allowArgs = False
            maxThreadNumber = 3
            if str(_id).strip()!= '':
                allowArgs = True
            ...
            if allowArgs == True:
                ...
                if len(lst_keyAvailable)>0:
                    ...
                    print(time7.currentTime7(),'      available of first key and value {', f'"{first_key}" : {first_value}','}')
                else:
                    print(time7.currentTime7(),'      [ not available thread ]')
        except Exception as e:
            print(time7.currentTime7(),'Fail of function "pyairtable_getFields_FirstThreadNumberAvailable"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
        return info_first_available
    
    def pyairtable_detectCrashProcess_then_recovery(self, _lst_data, **kwargs):
        try:
            print(time7.currentTime7(),'    - Checking for crash process:')
            maxThreadNumber = 3
            ...
            if '_argMaxSecondsTimeLogsWaiting' in kwargs:
                ...
                print(time7.currentTime7(),"      set _argMaxSecondsTimeLogsWaiting : ", str(maxSecondsTimeLogsWaiting))

            if len(_lst_data)!=0:
                u = utils()
                print(time7.currentTime7(),'.'*40)
                for idx, r in  enumerate(_lst_data):
                    ...
                    print(time7.currentTime7(),'     -> check esc id :', r_id)
                    print(time7.currentTime7(),'     -> check esc status :', r_status)
                    print(time7.currentTime7(),'     -> check esc cip :', r_cip)
                    print(time7.currentTime7(),'     -> check esc last of log 1 :', r_lastOfLog1 )
                    print(time7.currentTime7(),'     -> check esc last of log 2 :', r_lastOfLog2 )
                    print(time7.currentTime7(),'     -> check esc last of log 3 :', r_lastOfLog3 )
                    if str(r_status).strip()==const_status.InProgress.value and \
                        ...
                        print(time7.currentTime7(),'     Parameter 1 :')
                        print(time7.currentTime7(),'     [ Detected Crash Process ]')
                        table = Table(env.sandbox_airtable.api_key.value, env.sandbox_airtable.base_id.value, env.sandbox_airtable.table_name.value)
                        table.update(r_id,{'status':escape_quotes(const_status.ToDo.value), 'cip':0})
                        print(time7.currentTime7(),'       => Success for recovery')
                    lst_tLast_log = []
                    ...
                    if len(lst_tLast_log)!=0 and int(r_cip)!=0 and str(r_status).strip()==const_status.InProgress.value:
                        print(time7.currentTime7(),'     Parameter 2 :')
                        max_lastoflog=str(time7.maxdatetime_lstdict(lst_tLast_log)).replace(' ', 'T').replace('07:00','0700')
                        print(time7.currentTime7(),'     -> check start date :', r_start_date)
                        print(time7.currentTime7(),'     -> check max last of logs :', max_lastoflog)
                        diffseconds = time7.difference_datetimezone7_by_seconds_from_between(r_start_date,max_lastoflog)
                        print(time7.currentTime7(),'     [result check difference_datetimezone7_by_seconds_from_between : {}s ]'.format(str(diffseconds)))
            ...
            print(time7.currentTime7(),'Fail of function "pyairtable_detectCrashProcess_then_recovery"')
            print(time7.currentTime7(),'Error Handling ( エラー ):',e)
    ...
    def pyairtable_updateDateTime_CurrentNumberLastOfLog(self, _numberOfThread, _id, **kwargs):
        if "'int'" in str(type(_numberOfThread)) and str(_id)!='':
            print('Response number of thread ({0})'.format(str(_numberOfThread)))
            print('Response id airtable ({0})'.format(str(_id)))
...
class testing():
    ...
    def excute(self):
        ...
        if len(lst_data)!=0:
            ...
            print(time7.currentTime7(),'(2) - Processing')
