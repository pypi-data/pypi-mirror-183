...
class Queries(object): #TODO Statements
    ...
    def neware_cache_query(
        keys: Iterable, columns: Union[list, str] = '*', pulses: bool = False, condition: str = 'TRUE', connection: Union[MySqlConnection, str] = None,
        ...
        neware_pulses_cache = Queries.neware_cache_pulse_query(
            keys, columns=columns, condition=condition, connection=connection, cache=cache, ttl=ttl
        )
        
        if neware_pulses_cache.empty:
            return neware_cache

        group_keys = ['ip', 'dev_uid', 'unit_id', 'chl_id', 'test_id']

        def _replace_with_pulses(group):
            _group = query_df_by_dict(
                neware_pulses_cache,
                {key: value for key, value in zip(group_keys, group.name)}
            )
            
            return _group if not _group.empty else group

        return neware_cache.groupby(
            group_keys, as_index=False
        ).apply(_replace_with_pulses).droplevel(0)
