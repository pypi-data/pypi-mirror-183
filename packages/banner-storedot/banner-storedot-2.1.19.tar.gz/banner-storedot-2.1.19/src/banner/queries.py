...
class Queries(object): #TODO Statements
    ...
    def neware_cache_query(
        ...
        if not pulses: # Pulses were not requested
            return neware_cache
        ...
        if neware_pulses_cache.empty: # There are no pulses
            return neware_cache

        neware_pulses_cache['pulse'] = 1 # It is a pulse
        
        neware_cache = pd.concat(
            [neware_cache, neware_pulses_cache], ignore_index=True
        ) # Concat data
